CREATE Function GetOriginalID(@DocRef INT,@DocType INT)
...
When @RETAILINVOICE then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @RETAILINVOICEAMENDMENT then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @RETAILINVOICECANCELLATION then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @INVOICE then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @INVOICEAMENDMENT then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @INVOICECANCELLATION then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @SALESRETURN then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
...
When @PURCHASERETURN then (dbo.getvoucherprefix('STOCK ADJUSTMENT PURCHASE RETURN') + cast ((Select DocumentID from AdjustmentReturnAbstract where AdjustmentID=@DocRef) as nvarchar))
When @PURCHASERETURNCANCELLATION then (dbo.getvoucherprefix('STOCK ADJUSTMENT PURCHASE RETURN') + cast ((Select DocumentID from AdjustmentReturnAbstract where AdjustmentID=@DocRef) as nvarchar))
When @PURCHASERETURNAMENDMENT then (dbo.getvoucherprefix('STOCK ADJUSTMENT PURCHASE RETURN') + cast ((Select DocumentID from AdjustmentReturnAbstract where AdjustmentID=@DocRef) as nvarchar))
...
When @CREDITNOTE then (dbo.getvoucherprefix('CREDIT NOTE') + cast ((Select DocumentID from CreditNote where CreditID=@DocRef) as nvarchar))
...
When @MANUALJOURNALINVOICE then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
When @MANUALJOURNALSALESRETURN then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
...
When @MANUALJOURNALPURCHASERETURN then (dbo.getvoucherprefix('STOCK ADJUSTMENT PURCHASE RETURN') + cast ((Select DocumentID from  AdjustmentReturnabstract  where AdjustmentID=@DocRef) as nvarchar))
...
When @MANUALJOURNALCREDITNOTE then (dbo.getvoucherprefix('CREDIT NOTE') + cast ((Select DocumentID from CreditNote where CreditID=@DocRef) as nvarchar))
...
When @MANUALJOURNALOTHERCREDITNOTE then (dbo.getvoucherprefix('CREDIT NOTE') + cast ((Select DocumentID from CreditNote where CreditID=@DocRef) as nvarchar))
...
When @SALESRETURNCANCELLATION then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
...
When @CREDITNOTECANCELLATION then (dbo.getvoucherprefix('CREDIT NOTE') + cast ((Select DocumentID from CreditNote where CreditID=@DocRef) as nvarchar))
...
When @SALESRETURNAMENDMENT then (Select case isnull(InvoiceAbstract.GSTFlag,0) when 0 then dbo.getvoucherprefix('INVOICE')+ Cast(DocumentID As nvarchar) Else ISNULL(InvoiceAbstract.GSTFullDocID,'') End from InvoiceAbstract where InvoiceID=@DocRef )
...
When @CREDITNOTE_AMENDMENT then (dbo.getvoucherprefix('CREDIT NOTE') + cast ((Select DocumentID from CreditNote where CreditID=@DocRef) as nvarchar))
...
When @SERVICE_INWARD Then Cast((Select DocumentID From ServiceAbstract Where InvoiceID = @DocRef and ServiceType = 'Inward') as nvarchar)
When @SERVICE_OUTWARD Then Cast((Select DocumentID From ServiceAbstract Where InvoiceID = @DocRef and ServiceType = 'Outward') as nvarchar)
When @SERVICE_INWARD_CANCEL  Then Cast((Select DocumentID From ServiceAbstract Where InvoiceID = @DocRef and ServiceType = 'Inward') as nvarchar)
When @SERVICE_OUTWARD_CANCEL  Then Cast((Select DocumentID From ServiceAbstract Where InvoiceID = @DocRef and ServiceType = 'Outward') as nvarchar)
When @DAMAGE_INVOICE Then (Select GSTFullDocID From DandDInvAbstract Where DandDInvID = @DocRef)
End
End
Return @RetValue
End
