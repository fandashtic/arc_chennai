CREATE Function mERP_fn_SubSlabwiseSales_ITC(
	@FromMonth Datetime, 
	@ToMonth Datetime, 
	@Cat nVarchar(256), 
	@PCat nVarchar(256), 
...
If @Type = 1 
Begin
	Select @CustCount = Count(Distinct ia.CustomerID) From InvoiceAbstract ia, InvoiceDetail idl, Items its
	...
	Set @FOP = Cast(@CustCount As nVarchar)
End
Else If @Type = 2
Begin
...
Begin
	...
	Select "CustID" = ia.CustomerID, "ItemCount" = Sum((Case ia.InvoiceType When 4 Then -1 Else 1 End) * 
	...
	If @Type = 3
	Begin
		Select @CustCount = count(CustName) From @CustSaleCount 
		Where ItemCount Between @FromUOM And @ToUOM
