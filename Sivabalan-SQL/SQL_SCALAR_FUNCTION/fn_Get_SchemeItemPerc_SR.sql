Create Function dbo.fn_Get_SchemeItemPerc_SR(@SRInvoiceid int,@SchemeId int,@ProdCode nVarchar(255))
...
If isnumeric(@RefNum) = 0
	Set @InvoiceID = ( Select Top 1 InvoiceId from InvoiceAbstract where  DocumentId= ( select top 1 cast(ISnull(REVERSE(left(reverse(ReferenceNumber),PATINDEX(N'%[^0-9]%',Reverse(ReferenceNumber))-1)),0) as Integer) from InvoiceAbstract where invoicetype=4 and Invoiceid = @SRInvoiceID and isnull(referencenumber,'') <> '' and isnumeric(referencenumber) = 0 )) 
Else
	Set @InvoiceID = ( Select Top 1 InvoiceId from InvoiceAbstract where  DocumentId =  (Select ReferenceNumber from InvoiceAbstract where invoicetype=4 and Invoiceid = @SRInvoiceID and isnull(referencenumber,'') <> ''  )) 
...
select Product_Code,MultipleSchemeDetails,MultipleSplCategorySchDetail,Serial,FreeSerial,SplCatSerial,(Quantity * dbo.mERP_fn_GetMarginPTR(Product_code,InvoiceID,@Schemeid) ) SchValue,(Quantity * dbo.mERP_fn_GetMarginPTR(Product_code,InvoiceID,@Schemeid) + (Quantity * dbo.mERP_fn_GetMarginPTR(Product_code,InvoiceID,@Schemeid) * ((case when @Locality =1 then isnull(TaxCode,0)/100 else isnull(TaxCode2,0)/100 end )))) SchValueWithTax from invoicedetail where invoiceid=@Invoiceid
...
Begin
	...
	Begin
		...
		select Product_Code,Sum(Quantity * SalePrice),@tmpSchemeId ,@tmpSlabID ,((@tmpSchemeValue/sum(quantity * SalePrice)) * 100.) SchemePerc,((@tmpSchemeValueWithTax/sum(quantity * SalePrice)) * 100.) SchemePercWithTax,0 from InvoiceDetail where InvoiceId=@Invoiceid 
	...
	Begin
		...
		--Inserting Primary Item details
		Insert into @ItemBase(Product_code,SaleValue,SchemeID,SlabID,SchemePerc,SchemePercWithTax,SplCat) 
		select Product_Code,Sum(Quantity * SalePrice),@tmpSchemeId ,@tmpSlabID ,((@tmpSchemeValue/@tmpSchemeSaleValue) * 100.) SchemePerc,((@tmpSchemeValueWithTax/@tmpSchemeSaleValue) * 100.) SchemePercWithTax,1 from InvoiceDetail where InvoiceId=@Invoiceid 
		and Serial in  (select ItemValue from dbo.fn_SplitIn2Rows_Int(@SplCatSerial,','))
		group by Product_code
...
Begin
	...
	select distinct Product_code,(select sum(Quantity * Saleprice) from Invoicedetail where product_code= @Product_code and InvoiceID = @SRInvoiceID and (Isnull(MultipleschemeDetails,'')<>'' or isnull(MultipleSplCategorySchDetail,'')<>'' ) ) as SaleValue,SchemeId,SchemePerc,SchemePercWithTax,SplCat from @ItemBase where Product_Code=@Product_Code  and schemeid=@schemeid 
	Open Crsr_Prd
