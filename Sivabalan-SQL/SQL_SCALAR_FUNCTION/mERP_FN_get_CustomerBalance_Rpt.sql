Create Function mERP_FN_get_CustomerBalance_Rpt (@CusID nvarchar(255),@fromdate Datetime,@todate datetime)    
...
--C.DocumentDate between @fromdate and @todate And      
isnull(realised,0) not in(1)  And C.DocumentID not in(Select isnull(Representid,0) from ChequeCollDetails) and
C.DocumentID = CD.CollectionID and CD.DocumentType In (4,5)
...
Update T Set Flag=0 From @tmpC T,ChequeCollDetails CCD Where  T.DocID = CCD.RepresentID    
