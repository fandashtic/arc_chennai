CREATE Function [dbo].[GetTaxCompInfoWithBreakup_UT_NTD] (@InvNo int,@Product_Code nvarchar(255), @TaxID Int, @TaxAmount Decimal(18, 2))
Returns nvarchar(25)
As
Begin
...
Declare @CompPercentage nvarchar(6)
Declare @CompValue nvarchar(15)
...
(Case @check
When 4 then  (0 - Cast(((@TaxAmount * ITC.SP_Percentage) / @TaxComboSum) as Decimal(18,2)))
When 5 then  (0 - Cast(((@TaxAmount * ITC.SP_Percentage) / @TaxComboSum) as Decimal(18,
2)))
Else Cast(((@TaxAmount * ITC.SP_Percentage) / @TaxComboSum) as Decimal(18,2)) end) TaxCompValue
...
Set @Result = IsNull(@Result, N'') + ' ' + Space(6 -len(@CompPercentage)) + @CompPercentage + Space(8 -len(@CompValue)) + @CompValue
...
Else Cast(@TaxAmount as
 Decimal(18,2)) End
from InvoiceDetail where InvoiceID=@InvNo and Product_Code= @Product_Code
Set @Result = IsNull(@Result, N'') + ' ' + Space(6 -len(@CompPercentage)) + @CompPercentage + Space(9 -len(@CompValue)) + @CompValue --+ Space(6 -len(@CompPercentage1)) + @CompPercentage1 + Space(10 -len(@CompValue1)) + @CompValue1
End
