CREATE Function Dbo.FN_GetTaxSufferedAmt(        
...
@BatchCode int)
--This function returns the taxsuffered value for a given batch        
...
Begin            
  Select @nTax = (@TaxPercentage * @TaxPartOff) / 100  
      if @TaxApplicableOn = 1        
         select @nTaxableAmt = @Price  
      else if @TaxApplicableOn = 2        
         select @nTaxableAmt = @nStockist        
      else if @TaxApplicableOn = 3        
         select @nTaxableAmt = @nRetailer        
      else if @TaxApplicableOn = 4        
         select @nTaxableAmt = @nConsumer        
      else if @TaxApplicableOn = 5        
