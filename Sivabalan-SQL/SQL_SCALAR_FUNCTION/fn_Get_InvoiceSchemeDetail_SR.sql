CREATE Function dbo.fn_Get_InvoiceSchemeDetail_SR(@SRInvoiceID as integer,@ParamSchemeID as integer,@TaxConfigFlag as Integer)  
...
BEGIN
	...
	@SchValWithTax = Sum(isnull(Quantity,0) * isnull(dbo.mERP_fn_GetMarginPTR(Product_code,InvoiceID,@Schemeid),0) 
	+ (isnull(Quantity,0) * isnull(dbo.mERP_fn_GetMarginPTR(Product_code,InvoiceID,@Schemeid),0) * (case when @Locality =1 then isnull(TaxCode,0)/100 else isnull(TaxCode2,0)/100 end )) ) 	
	from InvoiceDetail where invoiceid = @Invoiceid
	and SchemeID=@Schemeid and FreeSerial = 0 
	And isnull(taxonQty,0)=0

	Select @SchValWithTax = @SchValWithTax +
	 Sum(isnull(Quantity,0) * isnull(dbo.mERP_fn_GetMarginPTR(Product_code,InvoiceID,@Schemeid),0) 
	+ (isnull(Quantity,0) * (case when @Locality =1 then isnull(TaxCode,0) else isnull(TaxCode2,0) end )))
	from InvoiceDetail where invoiceid = @Invoiceid
	and SchemeID=@Schemeid and FreeSerial = 0 
	And isnull(taxonQty,0)=1
