CREATE VIEW [dbo].[V_OL_Target_Achievement]  
AS  
Select T.DSID, T.CustomerID, T.PMProductName, Sum(isnull(T.Target,0)) as Target, 
	...
	From (Select Distinct PMMast.PMID, PMDS.DSTypeID as PMDSTYPEID, PMparam.ParamID, SM.SalesManID as DSID, 
	...
	Where PMMast.PMID = PMDS.PMID
		...
		And SM.SalesmanID in(Select Distinct SalesmanID From DSType_Master a, DSType_Details b 
							Where a.DSTypeID = b.DSTypeID and a.DSTypeCtlPos = 2 and a.DSTypeValue = 'Yes')) Tmp, PMOutlet PMO
	Where Tmp.CustomerID = PMO.OutletID
		And Tmp.PMID = PMO.PMID 
		And Tmp.PMDSTYPEID = PMO.DSTypeID 
		And Tmp.ParamID = PMO.ParamID
	Group By Tmp.DSID, Tmp.CustomerID, Tmp.PMProductName, Tmp.PMCatGrp, Tmp.PMID) T
	Left Outer Join  OLTargetAchievement OL On T.DSID = OL.DSID and T.CustomerID = OL.CustomerID and T.PMProductName = OL.PMProductName and T.PMCatGrp = OL.PMCatGrp and T.PMID = OL.PMID
