...
CREATE PROCEDURE sp_get_DocFooter
 @DocType nvarchar(10),  
 @DocID nvarchar(15)  
 AS  
BEGIN  
	...
	BEGIN  
		SELECT Product_Code,InvoiceID, Batch_Number,Serial, max(SalePrice) SalePrice, max(SaleID) SaleID,
			...
			max(flagWord) flagWord, max(freeSerial) freeSerial, max(SPLCATSerial) SPLCATSerial, max(SpecialCategoryScheme) SpecialCategoryScheme,
