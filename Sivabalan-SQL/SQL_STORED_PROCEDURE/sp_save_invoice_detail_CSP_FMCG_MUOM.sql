CREATE Procedure sp_save_invoice_detail_CSP_FMCG_MUOM(@INVOICE_ID int,
...
BEGIN
	...
	Amount, PurchasePrice, STPayable, MRP, TaxID, FlagWord, SaleID, TaxCode2, CSTPayable, TaxSuffered, TaxSuffered2, UOM, UOMQty, UOMPrice)
	VALUES (@INVOICE_ID, @ITEM_CODE, 0, N'', @REQUIRED_QUANTITY, @MODIFIED_PRICE, @TAXCODE, @DISCOUNTPERCENTAGE, @DISCOUNTAMOUNT, @AMOUNT, 
	@COST * @REQUIRED_QUANTITY, @STPAYABLE, @MRP, @TAXID, @FLAG, @SALEID, @TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice)
	GOTO ALL_SAID_AND_DONE
END
IF @TRACK_BATCHES = 1
	BEGIN
	SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products WHERE Product_Code = @ITEM_CODE AND ISNULL(Batch_Number, N'') = @BATCH_NUMBER 
	AND ISNULL(SalePrice, 0) = @SALE_PRICE AND (Expiry >= GetDate() OR Expiry IS NULL) And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow
	DECLARE ReleaseStocks CURSOR KEYSET FOR
	SELECT Batch_Number, Batch_Code, Quantity, PurchasePrice FROM Batch_Products
	WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, N'') = @BATCH_NUMBER and ISNULL(SalePrice, 0) = @SALE_PRICE AND ISNULL(Quantity, 0) > 0 AND (Expiry >= GetDate() OR Expiry IS NULL) And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow
	END
ELSE
	BEGIN
	SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products WHERE Product_Code = @ITEM_CODE AND ISNULL(SalePrice, 0) = @SALE_PRICE And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow
	DECLARE ReleaseStocks CURSOR KEYSET FOR
	SELECT Batch_Number, Batch_Code, Quantity, PurchasePrice FROM Batch_Products
	WHERE Product_Code = @ITEM_CODE AND ISNULL(SalePrice, 0) = @SALE_PRICE AND ISNULL(Quantity, 0) > 0 And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow
	END
...
BEGIN
	...
	END
        INSERT INTO InvoiceDetail(InvoiceID, Product_Code, Batch_Code, Batch_Number, Quantity, SalePrice, TaxCode, DiscountPercentage, DiscountValue, 
	Amount, PurchasePrice, STPayable, MRP, TaxID, FlagWord, SaleID, TaxCode2, CSTPayable, TaxSuffered, TaxSuffered2, UOM, UOMQty, UOMPrice)
	VALUES (@INVOICE_ID, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER, @REQUIRED_QUANTITY, @MODIFIED_PRICE, @TAXCODE, @DISCOUNTPERCENTAGE, @DISCOUNTAMOUNT, @AMOUNT, 
	@REQUIRED_QUANTITY * @COST, @STPAYABLE, @MRP, @TAXID, @FLAG, @SALEID, @TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice)
	...
	DiscountValue, Amount, PurchasePrice, STPayable, MRP, TaxID, FlagWord, SaleID, TaxCode2, CSTPayable, TaxSuffered, TaxSuffered2, UOM, UOMQty, UOMPrice)
	VALUES (@INVOICE_ID, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER, @QUANTITY, @MODIFIED_PRICE, @TAXCODE, @DISCOUNTPERCENTAGE, @DISCOUNTAMOUNT, @AMOUNT, 
	@COST * @QUANTITY, @STPAYABLE, @MRP, @TAXID, @FLAG, @SALEID, @TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice)
	SET @TAXCODE = 0
