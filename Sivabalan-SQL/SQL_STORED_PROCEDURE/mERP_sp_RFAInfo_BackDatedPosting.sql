Create Procedure mERP_sp_RFAInfo_BackDatedPosting(@InvoiceDate DateTime)
As
Begin
	...
	Create Table #RFAInfo(SR Int Identity , InvoiceID Int, BillRef nVarchar(255) collate SQL_Latin1_General_CP1_CI_AS, OutletCode nVarchar(255) collate SQL_Latin1_General_CP1_CI_AS,
        ...
        RebateValue_Tax Decimal(18,6),SRSchemeDetail_Tax nVarchar(1000) collate SQL_Latin1_General_CP1_CI_AS)
	...
	Create Table #RFADetail	(SR Int, Flagword Int, InvoiceID Int, SchemeID Int, 
		...
		SlabID Int, FreeBaseUOM nVarchar(255) collate SQL_Latin1_General_CP1_CI_AS, PrimarySerial Int, FreeSerial Int,RebateValue_Tax Decimal(18,6))
...
--						Flagword Int,InvoiceType Int)
	...
	Create Table #tmpRFADet(InvoiceID Int, InvoiceType Int, 
		...
		SlabID Int,FreeBaseUOM nVarchar(255) collate SQL_Latin1_General_CP1_CI_AS, PrimarySerial Int, FreeSerial Int)
	...
	Declare @SchInfo nVarchar(255)
	Create Table #Scheme(InvoiceID Int,Scheme Int)
	Create Table #SchInfo(InvoiceID Int,SchemeInfo nVarchar(255))
			...
			Begin
				...
				Begin/*Trade - Item based schemes - Start*/ 
					...
					Insert Into #RFAInfo(InvoiceId, BillRef, OutletCode, RCSID, LineType,
										 ...
										 SalePrice, InvoiceType, ReferenceNumber,[Doc No], TaxCode,SRSchemeDetail_Tax) 
					Select IA.InvoiceID, 
					...
					From InvoiceAbstract IA, Customer C,
					    (Select InvDt.InvoiceID, IsNull(InvDt.Flagword,0) Flagword, InvDt.Product_Code, InvDt.SalePrice, Max(InvDt.TaxCode) TaxCode, InvDt.Serial, Max(InvDt.PTR) PTR,
                         IsNull(InvDt.MultipleSchemeDetails, '') MultipleSchemeDetails, IsNull(InvDt.MultipleSplCategorySchDetail, '') MultipleSplCategorySchDetail, Sum(InvDt.Quantity) Quantity 
						 ...
						 Group By InvDt.InvoiceID, IsNull(InvDt.Flagword,0), InvDt.Product_Code, InvDt.SalePrice, InvDt.Serial, IsNull(InvDt.MultipleSchemeDetails, ''), IsNull(InvDt.MultipleSplCategorySchDetail, '')) ID  
					Where IA.InvoiceId = ID.InvoiceId
					And IA.InvoiceType In (1,3,4)        
					And (IA.Status & 128) = 0  
...
--					Deallocate SchemeOutletCur
					...
					Select Distinct InvoiceID, IsNull(SchemeDetail, ''), Serial, FlagWord, Sum(Amount) ,isNull(SRSchemeDetail_Tax,'')
					From #RFAInfo	
					Where SchemeOutlet = 0
					Group By InvoiceID, IsNull(SchemeDetail, ''), Serial, FlagWord,isNull(SRSchemeDetail_Tax,'')
					Open UpdateRebateCur	
					Fetch Next From UpdateRebateCur Into @InvoiceID, @SchemeDetail, @Serial, @FlagWord, @Amount,@SRSchemeDetail_Tax
					While (@@Fetch_Status = 0)
					Begin
						Set @SchemeAmt = 0
						Set @SlabID = 0
			...
			Select  0, IsNull(FlagWord, 0), InvoiceID, SchemeId, BillRef , OutletCode ,
					...
					RCSID , ActiveInRCS, LineType , SKUCode ,
					UOM, InvoiceType,[Doc No], SlabID,FreeBaseUOM,IsNull(FlagWord, 0)
			...
			Begin
				...
				Select InvoiceID, InvoiceType, SR, BillRef, OutletCode,
					...
					SlabID, FreeBaseUOM, PrimarySerial, FreeSerial
...
--					Deallocate UpdateLevelCur
					...
					Declare UpdateRebateCur Cursor For
					Select InvoiceID, InvoiceType, SchemeDetail, Serial, Amount, FlagWord, SchemeOutlet, SR,SRSchemeDetail_Tax  
					From #RFAInfo	
					Where SchemeOutlet = 0
					Open UpdateRebateCur	
					Fetch Next From UpdateRebateCur Into @InvoiceID, @InvoiceType, @SchemeDetail, @Serial, @Amount, @FlagWord, @SchemeOutlet, @InvSRID ,@SRSchemeDetail_Tax
					While (@@Fetch_Status = 0)
					Begin		
						If @FlagWord = 1
						Begin
					...
					Begin
						...
						Select  InvoiceID, InvoiceType, 0 as SR,
								...
								0 as BudgetedQty, 0 as BudgetedValue,Cust.Company_Name as 'OutletName',[Doc No], SlabID, FreeBaseUOM,
								 Max(Serial), Max(FreeSKUSerial)
								From #RFAInfo,Customer Cust   
								Where SchemeID = @SchemeID
								And Cust.CustomerID = #RFAInfo.OutletCode 
								Group By SchemeID, InvoiceID, InvoiceType, OutletCode, RCSID, ActiveInRCS, SKUCode, UOM,
								FreeBaseUOM, BillRef ,LineType,
								PriceExclTax, TaxPercentage, TaxAmount, PriceInclTax,Cust.Company_Name,[Doc No], SlabID, FreeBaseUOM
								Order By SKUCode
					End
--					Else
--					Begin
...
--			And T1.SKUCode = T2.SKUCode
		...
		RD.BillRef, RD.[Doc No] As DocID, RD.InvoiceType As InvoiceType, RD.SKUCode As Product_Code,
			...
			RD.RebateValue_Tax, RD.PromotedQty, RD.PromotedValue, 
			(Select UOM From UOM Where Description = IsNull(RD.FreeBaseUOM, '')) As RebateUOM,
			(Select UOM From UOM Where Description = IsNull(RD.UOM, '')) As PromotedUOM, 
	...
	If exists ((Select Top 1 * From tbl_merp_NonQPSData Where dbo.StripTimeFromDate(InvoiceDate) = dbo.StripTimeFromDate(@InvoiceDate)) ) 
