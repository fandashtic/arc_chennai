Create procedure dbo.spr_SalesReturnWithRFAReversal_Rpt_Detail ( @DivName nvarchar(1000), @Dt nvarchar(400)) 
as 
Begin 

set dateformat dmy
Declare @Delimeter char(1), @SplSchDet varchar(4000), @RptParam varchar(1000), @INV AS NVARCHAR(50)
, @InvoiceId Int, @prd_code varchar(4000), @serial Int, @SchItmData varchar(1000), @RFAflag Int
...
Join ( Select ItcDiv.CategoryId DivId, ItcDiv.Category_Name DivName, Itm.product_code  
...
where Itm.DivName = @DivName 


select #tmpIa.CustomerId, #tmpIa.CustomerId, #tmpIa.company_name CustomerName, schabs.ActivityCode
, schabs.Description ActivityDescription, schabs.ActiveFrom, schabs.ActiveTo
, Ia.InvoiceDate Original_InvDate, @INV + convert(varchar(100), #tmpIa.DocumentId) Ret_InvoiceNo, #tmpIa.InvoiceDate Ret_InvoiceDate
, ISch.prd_code SKUCode, (0 - ISch.quantity) ReturnSalesQty, ( 0 - ISch.amount) ReturnSalesValue
, IsNull(Null, 0) ReturnRebateQty, (0 - ISch.DscAmt) ReturnRebateValue
