Create Procedure [dbo].[sp_print_DandDInvAbs_CG_Intra](@DnDID INT)
AS
Set dateformat DMY
...
Exec sp_print_DandDInvItems_RespectiveUOM_CG_Intra @DnDID,1
...
Select TaxableValue=Sum(TaxableValue),SGSTPer=Max(Case when @UTGST_flag = 1 then UTGSTPer Else  SGSTPer End),
SGSTAmt =Sum(Case when @UTGST_flag = 1 then UTGSTAmt Else  SGSTAmt End ), CGSTPer=Max(CGSTPer),
CGSTAmt=SUM(CGSTAmt),
...
Total= SUM(Case when @UTGST_flag = 1 then UTGSTAmt Else  SGSTAmt End+CGSTAmt+CESSAmt+ADDLCESSAmt)
Into #GSTaxSummary
From #TempInvDet2
Group By TaxID
...
Set @GSTTaxComp = '  '+ Char(13) + Char(10)+'  |  Tax Summary/GST  Component' + space(62) + 'TaxableVal' + Space(10) + 'CGST' + Space(8) + Case when @UTGST_flag = 1 then 'UTGST' Else  '  SGST' End + Space(12) + 'Cess' +  Space(4) + 'AddlCess'+ + Space(6) + 'Total Tax' + Char(13) + Char(10)
...
+ '% + '+ Case when @UTGST_flag = 1 then 'UTGST' Else  'SGST  ' End + Replicate('  ',5-LEN(CAST(Cast(SGSTPer  As Decimal(5,2)) As nVarChar(5)))) + CAST(Cast(SGSTPer As Decimal(5,2)) As nVarChar(5))
...
Inner Join StateCode ToState On Inv.ToStateCode  = ToState.StateID
