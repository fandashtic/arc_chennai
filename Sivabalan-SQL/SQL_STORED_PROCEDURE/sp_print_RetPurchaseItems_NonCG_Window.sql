Create Procedure [dbo].[sp_print_RetPurchaseItems_NonCG_Window](@INVNO INT,@MODE INT=0)
AS
...
"Item Name" = Case When Duplicate = 1 Then
Items.Description
Else
cast(isnull(Items.HSNNumber,'') as nvarchar)
End,
"UOM" = Isnull(Case When Duplicate=1 Then
UOM.Description
Else
Cast(Cast(sum(AdjustmentReturnDetail.UOMQTY*AdjustmentReturnDetail.UOMPrice) as decimal(18,2))as nVarchar)
End,''),
"MRP" = Case When Duplicate = 1 Then
Cast(Cast(AdjustmentReturnDetail.MRPPerPack As Decimal(18,2)) as nVarchar(50))
Else Cast(Case AdjustmentReturnDetail.UOMPrice When 0 Then Cast(N'Free' as Nvarchar(50))
Else Cast(Cast(AdjustmentReturnDetail.UOMPrice As Decimal(18,2))As NVarChar)
End As nVarChar)
End,
"IGST" = Isnull(Case When Duplicate=1
Then Cast(Cast(max(Isnull(AdjustmentReturnDetail.IGSTPer,0)) As Decimal(18,2)) as nvarchar)
Else
Cast(Cast(sum(IGSTAmt) as decimal(18,2)) as nVArChar)
End,''),
"Quantity" = Isnull(Case When Duplicate=1
--Then Isnull(cast(Cast((Case When AdjustmentReturnDetail.UOMPrice <> 0  THEN Sum(1 * AdjustmentReturnDetail.UOMQty)  Else 0 	End) as decimal(18,2) )
Then Isnull(cast(Cast((Case When AdjustmentReturnDetail.UOMPrice <> 0  THEN Sum(1 * AdjustmentReturnDetail.UOMQty)  Else Sum(1 * AdjustmentReturnDetail.UOMQty) 	End) as decimal(18,2) )
as nVarchar),'')
Else cast(Case Cast(Sum(AdjustmentReturnDetail.Quantity) * AdjustmentReturnDetail.Rate  As Decimal(18,2))
When 0 then	'0'
Else	Cast(Sum(1 * AdjustmentReturnDetail.Quantity) * AdjustmentReturnDetail.Rate as Decimal(18,2))
End as nvarchar)
End,''),
"SGST/UTGST" =isnull(Case when Duplicate = 1
Then Cast(Cast(Max(SGSTPer) + Max(UTGSTPer) as decimal(18,2))as nVarchar)
Else Cast(Cast(Sum(SGSTAmt) + Sum(UTGSTAmt) as decimal(18,2))as nVarchar)
End,''),
"CGST Amt" = isnull(Case when Duplicate = 1
Then Cast(Cast(Sum(CGSTPer) as decimal(18,2))as nVarchar)
...
as nvarchar)
Else Cast(Cast(Sum(AdjustmentReturnDetail.CESSAmt) as decimal(18,2))as nVarchar)
End,''),

"Addl.CessRate" = isnull(Case when Duplicate = 1
Then Cast(Cast(Sum(AdjustmentReturnDetail.ADDLCESSPer) as decimal(18,2))as nVarchar)
Else Cast(Cast(Sum(AdjustmentReturnDetail.ADDLCESSAmt) as decimal(18,2))as nVarchar)
End,''),
"Total Tax" = isnull(Case when Duplicate = 1
Then Cast(cast(Sum(AdjustmentReturnDetail.TaxAmount) As Decimal(18,2))
as nvarchar)
Else Cast('' as nVarchar)
End,''),

"Total Amount" = isnull(Case when Duplicate = 1 Then
Cast(Cast(Sum(AdjustmentReturnDetail.Total_value) as decimal(18,2))as nVarchar)
Else
Cast('' as nVarchar)
End,''),
...
,AdjustmentReturnDetail.MRPPerPack
