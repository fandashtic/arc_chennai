CREATE Procedure [dbo].[mERP_SP_ListChannelMarginDetail](@Date DateTime = Null,@Level int,@Channel nvarchar(255),@RegFlag int)
As
Begin
Declare @CatID int
Declare @CategoryName nvarchar(100)
Declare @Parent int
Declare @ChannelCode nvarchar(15)

Select Distinct Top 1 @ChannelCode = Channel_Type_Code From tbl_mERP_OLClass Where Channel_Type_Desc = @Channel
...
Level,
cast(isNull(dbo.merp_fn_Get_CategoryChannelMargin(#TempCategory.Code,@Date,Level,'Percentage',@ChannelCode,@RegFlag),0) as decimal(18,6))
'Percentage',
dbo.merp_fn_Get_CategoryChannelMargin(#TempCategory.Code,@Date,Level,'EffectiveDate',@ChannelCode,@RegFlag)
'EffectiveDate',
dbo.merp_fn_Get_CategoryChannelMargin(#TempCategory.Code,@Date,Level,'RevokeDate',@ChannelCode,@RegFlag)
'RevokeDate',SystemSKU
from #TempCategory where Level <= @Level
order by Division,SubCategory,MarketSKU,SystemSKU+' ~ '+(select ProductName from Items where Product_Code=SystemSKU),Level
