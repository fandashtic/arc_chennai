CREATE PROCEDURE [dbo].[sp_print_InvAbstractSR](@INVNO INT)        
AS        
/*
...
Declare @AdjValueEx Decimal(18,6) -- To avoid the deduction of adjustmentvalue twice        

--To calculate the Sales Return Tax values 
Declare @SRTotalTax Decimal(18,6)
Declare @SRTaxSuffered Decimal(18,6)

 Create Table #Temp1 (InvID int identity(1,1), invno int,SR int)
 If  
 (  
 ...
 Begin    
  INsert into #Temp1(Invno,SR)     
  SELECT CollectionDetail.DocumentID,-1 FROM CollectionDetail,InvoiceAbstract    
  Where InvoiceAbstract.InvoiceId = @invno and   
  ISnull(InvoiceAbstract.PaymentDetails,0)=CollectionDetail.CollectionID    
  And CollectionDetail.DocumentType=1 And InvoiceAbstract.InvoiceType in (1,3)    
  Select @SRTotalTax = Sum(STPayable+CSTPayable), @SRTaxSuffered = Sum(TaxSuffAmount) from InvoiceDetail
  where InvoiceID in (Select InvNo from #Temp1)  
 End   
Drop Table #Temp1
...
create table #temp(taxsuffered Decimal(18, 6))
...
Select (Case InvoiceAbstract.TaxOnMRP When 1 then  
 ...
 Max(InvoiceDetail.TaxSuffered) / 100, 0) End)
From InvoiceDetail
...
Select @AdjValueEx = IsNull(Sum(CollectionDetail.AdjustedAmount), 0) From CollectionDetail, InvoiceAbstract        
Where CollectionID = Cast(PaymentDetails as int) And         
CollectionDetail.DocumentID <> @InvNo And        
InvoiceAbstract.InvoiceID = @InvNo And DocumentType not in (2,5)       
...
When 0 Then        
 ...
 When 4 Then        
 0 - ((NetValue + RoundOffAmount + AdjustmentValue) -  @AdjValueEx)     
 Else        
 (NetValue + RoundOffAmount + AdjustmentValue) -  @AdjValueEx
 End        
Else        
...
"SR Sales Tax" = @SRTotalTax,
"SR Tax Suffered" = @SRTaxSuffered
...
Inner Join VoucherPrefix SR on 
Inner Join VoucherPrefix Inv on SR.TranID = 'SALES RETURN'  
Inner Join VoucherPrefix InvA On InvA.TranID = 'INVOICE AMENDMENT'  
Salesman2
Salesman
