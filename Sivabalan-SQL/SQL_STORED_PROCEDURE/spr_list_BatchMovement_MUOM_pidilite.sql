CREATE procedure [dbo].[spr_list_BatchMovement_MUOM_pidilite](@Manufacturer nvarchar(2550),         
...
"Total Quantity" = Cast((  
   ...
   End) as nvarchar),         
"Conversion Factor" = SUM(IsNull(Quantity, 0) * IsNull(ConversionFactor, 0)),
-- CAST(CAST(SUM(Quantity * Items.ConversionFactor) AS Decimal(18,6)) AS nvarchar)        
-- + ' ' + CAST(ConversionTable.ConversionUnit AS nvarchar),        
"Reporting UOM" = SUM(IsNull(Quantity, 0) / Case IsNull(ReportingUnit, 1) When 0 Then 1 Else IsNull(ReportingUnit, 1) End),
-- Cast(dbo.sp_Get_ReportingUOMQty(#temp.product_code, SUM(IsNull(Quantity, 0))) As nvarchar)   
