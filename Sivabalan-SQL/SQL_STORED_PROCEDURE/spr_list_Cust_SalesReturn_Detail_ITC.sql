...
Create Procedure spr_list_Cust_SalesReturn_Detail_ITC(      
...
Set @pos = charindex(char(15), @CUSTOMER)
set @CustomerID = substring(@CUSTOMER, 1, @pos - 1)
set @BeatID = substring(@CUSTOMER, @pos + 1, Len(@CUSTOMER))

-------------------------------------
-- select  @pos , @customeriD, @BeatID 
-------------------------------------
...
Sum(InvoiceDetail.Quantity),Sum(InvoiceDetail.Amount),
Case IsNULL(InvoiceAbstract.GSTFlag ,0)
When 0 then Case When IsNull(InvoiceAbstract.DocumentID,0) = 0 Then '' Else @INVOICEPREFIX + Cast(InvoiceAbstract.DocumentID As nvarchar(50)) End 
...
And Customer.Company_Name = @customeriD
...
Group By Items.Product_Code,Items.Product_Code,Items.ProductName,InvoiceDetail.Batch_Number, InvoiceAbstract.DocumentID,InvoiceAbstract.GSTFlag,InvoiceAbstract.GSTFullDocID    
