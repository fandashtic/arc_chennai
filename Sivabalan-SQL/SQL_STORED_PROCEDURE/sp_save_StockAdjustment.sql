CREATE Procedure sp_save_StockAdjustment(@STOCKID INT,          
          ...
          @PRICE Decimal(18,6),          
          @ADJTYPE INT = 1, --0=StkAdj Damage, 1=StkAdj Others, 2=OP Stock Adjustment  
...
If @ADJTYPE = 2      
 IF @PriceOption = 1  
  ...
  AND IsNull(PTS, 0) = @PTS1 AND IsNull(PTR, 0) = @PTR1 AND IsNull(ECP, 0) = @ECP1  
...
BEGIN          
 ...
 BEGIN          
  IF @ORIGINAL_CODE = 0 SET @ORIGINAL_CODE = @BATCH_CODE          
         UPDATE Batch_Products SET PurchasePrice = @PRICE,Quantity = Quantity + @DIFF          
...
BEGIN          
 SELECT @EXPIRY = Expiry, @PTS = PTS, @PTR = PTR, @ECP = ECP, @Special_Price = Company_Price, @PKD_DATE = PKD, @GRN_ID = GRN_ID, @TAXSUFFERED = TaxSuffered,   
 ...
 SalePrice, PTS, PTR, ECP, Company_Price, Damage, DamagesReason, PKD, GRN_ID, TaxSuffered, Free, GRNTaxSuffered, GRNTaxID, GRNApplicableOn, GRNPartOff, ApplicableOn, PartofPercentage, Vat_Locality, DocType, DocId)          
 Values(@Batch_Number, @EXPIRY, @ITEM_CODE, ABS(@ADJUSTMENT_QTY), @ADJUSTMENT_PRICE,           
 @SALE_PRICE, @PTS, @PTR, @ECP, @Special_Price, 1, @REASON_ID, @PKD_DATE, @GRN_ID, @TAXSUFFERED, @Free, @GRNTaxSuffered, @GRNTaxId, @GRNApplicableOn, @GRNPartOff, @ApplicableOn, @PartOff, @Vat_Locality, 2, @STOCKID)          
