CREATE procedure sp_acc_gj_closepurchasereturn(@Adjustmentid integer,@BackDate datetime = Null)
as
...
declare @purchasetax decimal(18,6)
...
set @ndoctype=12    /* Variable to store the Document Type*/   
...
Create Table #TempBackdatedpurchasereturncancellation(AccountID Int) --for backdated operation
...
Begin
	...
	where [AdjustmentID]=@Adjustmentid and [Status]=192
End
Else
Begin
	select @nadjustmentid = [AdjustmentID],@dadjustmentdate = [AdjustmentDate],
	@nvalue = ISNULL([Value],0),@nvendorid=ISNULL([VendorID],0),
	@totalvalue = isnull(Total_Value,0) from AdjustmentReturnabstract
	where [AdjustmentID]=@Adjustmentid and [Status]=192
End
...
begin
	...
	If @Vat_Exists  = 1
	Begin
		set @totaltax = @totalvalue - @nvalue - @nVatTaxamount
	End
	Else
	Begin
		set @totaltax = @totalvalue - @nvalue
	End
	If @nvalue <> 0 
	Begin
		...
		begin 
			...
			Values(@documentid,@purchasereturn,@dadjustmentdate,@nvalue,0,@nadjustmentid,@ndoctype,'Purchase Return Cancellation',@uniqueid)  
			Insert Into #TempBackdatedpurchasereturncancellation(AccountID) Values(@purchasereturn)
		end
	
		if Round(isnull(@totaltax,0),2) <> 0 And Round((isnull(@totaltax,0) - isnull(@ILevelTaxValue,0)),2) <> 0  
		begin 
			insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
			[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])
			Values(@documentid,@purchasetax,@dadjustmentdate,isnull(@totaltax,0)-isnull(@ILevelTaxValue,0),0,@nadjustmentid,@ndoctype,'Purchase Return Cancellation',@uniqueid)  
			Insert Into #TempBackdatedpurchasereturncancellation(AccountID) Values(@purchasetax)
		end
					...
					begin
						...
						begin
							...
							begin
								insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
								[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])  
								Values(@documentid,@GST_receivable,@dadjustmentdate,isnull(@nGSTaxAmt,0)-isnull(@ILevelTaxValue,0),0,@nadjustmentid,@ndoctype,'Purchase Return Cancellation',@uniqueid)    
								Insert Into #TempBackdatedpurchasereturncancellation(AccountID) Values(@GST_receivable)  
							end
						end
						...
						begin
							...
							begin							 
								insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
								[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])  
								Values(@documentid,@GST_receivable,@dadjustmentdate,isnull(@nGSTaxAmt,0)-isnull(@ILevelTaxValue,0),0,@nadjustmentid,@ndoctype,'VAT Purchase Return Cancellation',@uniqueid)    
								Insert Into #TempBackdatedpurchasereturncancellation(AccountID) Values(@GST_receivable)  							
							end
						end
					end
		...
		Begin
	 		...
	 		begin  
				insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
				[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])
				Values(@documentid,@VAT_Receivable,@dadjustmentdate,isnull(@nVatTaxamount,0)-isnull(@ILevelTaxValue,0),0,@nadjustmentid,@ndoctype,'Purchase Return Cancellation',@uniqueid)  
				Insert Into #TempBackdatedpurchasereturncancellation(AccountID) Values(@VAT_Receivable)
			end
		End
		...
		begin 
			insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
			[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])
			Values(@documentid,@accountid,@dadjustmentdate,0,@totalvalue,@nadjustmentid,@ndoctype,'Purchase Return Cancellation',@uniqueid)
			Insert Into #TempBackdatedpurchasereturncancellation(AccountID) Values(@accountid)
		end
	end
End
...
Begin
	...
	Select AccountID From #TempBackdatedpurchasereturncancellation
...
End
Drop Table #TempBackdatedpurchasereturncancellation
