...
CREATE Procedure sp_acc_loadmanualjournal(@dfromdate datetime,@dtodate datetime,@accountid integer,@mode integer)
as
...
begin
	...
	and (isnull([Status],0)<>128 and isnull([Status],0)<>192)) and [DocumentType] in (26,37) order by TransactionID
end
else
if @mode =@all
begin
	select [GeneralJournal].[TransactionID],[GeneralJournal].[TransactionDate],[GeneralJournal].[AccountID],[GeneralJournal].[Debit],[GeneralJournal].[Credit],[GeneralJournal].[DocumentReference],[GeneralJournal].[DocumentType],
 	[GeneralJournal].[Status],[GeneralJournal].[ReferenceNumber],[GeneralJournal].[Remarks],
  	...
  	and @dtodate)
  	and [DocumentType] in (26,37) and (isnull([Status],0)<>128 and isnull([Status],0)<>192) order by TransactionID

   	/*  select [GeneralJournal].[TransactionID],[GeneralJournal].[TransactionDate],[GeneralJournal].[AccountID],[GeneralJournal].[Debit],[GeneralJournal].[Credit],[GeneralJournal].[DocumentReference],[GeneralJournal].[DocumentType],
	[GeneralJournal].[Status],[GeneralJournal].[ReferenceNumber],[GeneralJournal].[Remarks],
   	[AccountsMaster].[AccountName]from [GeneralJournal],[AccountsMaster] where [GeneralJournal].[AccountID]=[AccountsMaster].[AccountID] and [TransactionID] in (Select [TransactionID]from generaljournal where (dbo.stripdatefromtime([TransactionDate]) >=@d
