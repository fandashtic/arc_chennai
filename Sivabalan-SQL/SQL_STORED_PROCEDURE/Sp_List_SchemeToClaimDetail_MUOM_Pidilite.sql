Create Procedure Sp_List_SchemeToClaimDetail_MUOM_Pidilite(@SchemeId int)  
As  
Set dateformat dmy        
...
--PerUnit - 1 /Given Qty -0   
select @nApplyON = Isnull(ApplyON,0) from schemes where SchemeID=@SchemeId  
...
-- @bSelCustomers = 0 , @IsPer = 1,  @Flag = 2,  
-- @FromDate = '2007-07-14 00:00:00.000' , @ToDate = '2007-07-15 23:59:59.000', @bHasSlabs = 1   
...
Begin         
 ...
 Declare SchemeDiscount Cursor For        
  Select StartValue,EndValue,Freevalue, IsNull(PrimaryUOM,0) From SchemeItems        
  Where SchemeId=@SchemeID        
        
 Open SchemeDiscount        
  
 Fetch next From SchemeDiscount Into @FromLim,@ToLim,@Discount,@PrimaryUOM        
 While(@@fetch_status=0)        
 Begin        
  IF (@IsPer=1)        
  Begin        
   Update Custable Set DisAmt=@Discount,AllottedAmount=SalesValue * (@Discount /100)        
   Where Qty >= (@FromLim * (Case @PrimaryUOM When 0 Then 1  
     ...
     And Qty <= (@Tolim  * (Case @PrimaryUOM When 0 Then 1  
     When 1 Then (Select Uom1_conversion From Items Where Items.Product_code = Custable.Product_code)     
     When 2 Then (Select Uom2_conversion From Items Where Items.Product_code = Custable.Product_code) End))  
  End        
  Else        
  Begin        
   IF (@nApplyON=1)  -- Per-Unit  
     Begin  
       Update Custable Set DisAmt= Qty * @Discount, AllottedAmount= Qty * @Discount  
       Where Qty >= (@FromLim * (Case @PrimaryUOM When 0 Then 1  
     When 1 Then (Select Uom1_conversion From Items Where Items.Product_code = Custable.Product_code)     
     When 2 Then (Select Uom2_conversion From Items Where Items.Product_code = Custable.Product_code) End))  
      And Qty <= (@Tolim * (Case @PrimaryUOM When 0 Then 1  
...
Begin    
 ...
 End    
  ...
  Declare SchemeDiscount Cursor For        
   Select StartValue,EndValue,Freevalue,IsNull(PrimaryUOM,0) From SchemeItems        
   Where SchemeId=@SchemeID Group by StartValue,EndValue,FreeValue,PrimaryUOM       
         
  Open SchemeDiscount        
           
  Fetch next From SchemeDiscount Into @FromLim,@ToLim,@FreeQty,@PrimaryUOM        
  While(@@fetch_status=0)     
  Begin        
  IF (@bHasSlabs=1)        
  Begin        
  ...
  Else        
  Begin        
   Update Custable Set AllottedAmount = AllottedAmount + (Cast(Qty/(@FromLim * (Case @PrimaryUOM When 0 Then 1  
     When 1 Then (Select Uom1_conversion From Items Where Items.Product_code = Custable.Product_code)     
     When 2 Then (Select Uom2_conversion From Items Where Items.Product_code = Custable.Product_code) End) ) as int) * @FreeQty)        
   Where Qty/(@FromLim * (Case @PrimaryUOM When 0 Then 1  
     When 1 Then (Select Uom1_conversion From Items Where Items.Product_code = Custable.Product_code)     
