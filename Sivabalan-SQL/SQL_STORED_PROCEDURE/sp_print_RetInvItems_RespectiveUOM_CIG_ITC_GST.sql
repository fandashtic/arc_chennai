Create Procedure [dbo].[sp_print_RetInvItems_RespectiveUOM_CIG_ITC_GST](@INVNO INT,@MODE INT=0)
AS
Begin
...
--Case When Duplicate=1 then Cast((Case When InvoiceDetail.UOMPrice <> 0 THEN
Cast(Sum(#Temp1.SRQty * InvoiceDetail.UOMQty) as decimal(18,2))
...
End,
"Other Disc" = Case When Duplicate = 1 Then Cast(sum((InvoiceDetail.Quantity * InvoiceDetail.SalePrice)-InvoiceDetail.DiscountValue)*(InvoiceAbstract.AdditionalDiscount/100) as nvarchar) Else
cast(cast(Max(ADDLCESSPer) as decimal(18,2)) as nvarchar)
End,
--GST_Changes ends here
"Amount"=Cast(
...
+' | '+ Cast(Cast(Sum(InvoiceDetail.Quantity) As decimal(20,2)) As nvarchar(20)) + +' '+ Cast(InvoiceDetail.BaseUOMDescription  As nvarchar(5)) As nVarchar(50))
...
"Item MRP" =  IsNull(cast( Case When Duplicate=1 then
case isnull(InvoiceDetail.MRPPerpack,0) when 0 then isnull(Items.MRPPerPack,0) else isnull(InvoiceDetail.MRPPerpack,0) end
Else Cast(Cast((case when @utgst_flag = 1 then Max(UTGSTPer) Else Max(SGSTPer) End) as decimal(18,2))as nVarchar)  End
as nvarchar),''),
...
"Total Tax" = Case When  Duplicate = 1 Then cast(sum(isnull(InvoiceDetail.stpayable,0) + isnull(InvoiceDetail.cstpayable,0)) as nVarchar) Else
cast(cast(Sum(ADDLCESSAmt) as decimal(18,2)) as nvarchar)
End,
...
--InvoiceDetail.CESSAmt , InvoiceDetail.CESSPer
...
TaxDetailsWithBreakup = '',TaxDetails='',[Batch No.]='' ,--[Total Tax] = '',
[Discount%] = [Cess Rate] ,[Discount Value] = [Cess Amt] --,[Other Disc] = ''
Where Duplicate = 2
--GST_Changes ends here
