Create Procedure mERP_sp_get_DSPMetric_TargetDefn(@PMetricID Int, @ParamID Int, @EditLock Int = 0)
As
Begin 
...
set dateformat dmy
	...
	Begin
		...
		--FocusSKU, 
		0 as FocusID,CGMapID, 0 TargetDefnID , NULL SalesValue,GrowthPercentage,0 ProposedTargetValue,NULL,0,0,0
		From @tmpDSTarget
		Union 
		...
		isnull(PMTar.ProposedTargetValue,0) ProposedTargetValue,PMTar.TargetDefnDate,isnull(PMTar.AutopostFlag,0),0,OriginalTarget
		...
		BEGIN
			...
			Update T set T.Target = dbo.Fn_PM_GetRoundedNearest500(T1.Target)
			...
			update @Tempdata set ProposedTargetValue=NULL,OriginalTarget=NULL where salesmanid not in (select distinct DSID from #tmpOutletwiseData)
...
--		END
	...
	BEGIN
		...
		BEGIN
			...
			CGMapID,TargetDefnID,SalesValue,GrowthPercentage,isnull(ProposedTargetValue,0) as ProposedTargetValue,LastupdatedDate,AutoPostflag,SaveFlag,OriginalTarget
			from @Tempdata		
		END
	END
	ELSE
