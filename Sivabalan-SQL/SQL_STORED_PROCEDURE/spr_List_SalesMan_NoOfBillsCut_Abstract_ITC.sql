CREATE Procedure spr_List_SalesMan_NoOfBillsCut_Abstract_ITC
  (
...
and ProductCategorygroupAbstract.GroupName In 
(Select CategoryGroup COLLATE SQL_Latin1_General_CP1_CI_AS From #tempCategoryGroup)
...
If @DSTYPE= '%'           
   ...
   Union
   Select SalesmanID,'' from Salesman where SalesmanID not in (select SalesmanID from DSType_Details)
Else  
   Insert into #tempDSType 
...
(          
    Select isnull(Salesman.SalesManID,0) as SalesManID,isnull(Salesman.SalesMan_Name,'') SalesMan_Name ,--isnull(Beat.[Description],'') as Beat,        
           tDS.DSType as DSTypeValue,   
...
--                   and InvoiceDetail.SalePrice > 0 --Avoid diff product free items        
...
(      
SalesManID int,Salesman_Name NVarChar(500) COLLATE SQL_Latin1_General_CP1_CI_AS  default N'',      
DSType NVarChar(500) COLLATE SQL_Latin1_General_CP1_CI_AS  default N'',      
...
insert into #tmpTransformedData(SalesManID ,Salesman_Name,DSType, CatwiseTotalQty,CatwiseTotalValue)        
select SalesManID, Salesman_Name, DSTypeValue, sum(Quantity) as CatwiseTotalQty,sum(Netvalue) as CatwiseTotalValue      
from #tmpData         
group by SalesManID,Salesman_Name, DSTypeValue      
...
BEGIN
     ...
     if @ValueType = 'Value'
         Begin         
              ...
              set @tmp_sql = @tmp_sql + ' ( ' 
              set @tmp_sql = @tmp_sql + '      select SalesManID,sum(UpdateVal) as UpdateVal'
              set @tmp_sql = @tmp_sql + '      from'
              set @tmp_sql = @tmp_sql + '      ( '
              set @tmp_sql = @tmp_sql + '          select SalesManID,InvoiceID,case initParentCategoryID when ' + cast( @CategoryID as nvarchar(20)) + ' then '
              ...
              set @tmp_sql = @tmp_sql + '      group by SalesmanID'
              set @tmp_sql = @tmp_sql + ' ) tmp '
              set @tmp_sql = @tmp_sql + ' where #tmpTransformedData.SalesmanID = tmp.SalesmanID '
         End         
     else if @ValueType = 'Volume'
         Begin
              set @tmp_sql = 'Update #tmpTransformedData '
              ...
              set @tmp_sql = @tmp_sql + ' from #tmpTransformedData, '
              set @tmp_sql = @tmp_sql + ' ( '
              set @tmp_sql = @tmp_sql + ' select SalesManID,sum(case initParentCategoryID when ' + cast( @CategoryID as nvarchar(20)) + ' then '
              set @tmp_sql = @tmp_sql + ' isnull(#tmpData.Quantity,0) else 0 end) as UpdateVal '
              set @tmp_sql = @tmp_sql + ' from #tmpdata group by SalesManID '
              set @tmp_sql = @tmp_sql + ' ) tmp '
              set @tmp_sql = @tmp_sql + ' where #tmpTransformedData.SalesmanID = tmp.SalesmanID '
         End
     else if @ValueType = 'No.of Bills'
         Begin
              ...
              set @tmp_sql = @tmp_sql + ' from '
              set @tmp_sql = @tmp_sql + ' ( '
--              set @tmp_sql = @tmp_sql + ' select SalesManID,(case initParentCategoryID when ' + cast( @CategoryID as nvarchar(20)) + ' then  count(distinct #tmpData.InvoiceID) else 0 end) as UpdateVal  '
              set @tmp_sql = @tmp_sql + ' select SalesManID,(case initParentCategoryID when ' + cast( @CategoryID as nvarchar(20)) + ' then  count(distinct case when  #tmpdata.InvoiceType in (1,3) then #tmpdata.InvoiceID end) else 0 end) as UpdateVal  '
   
    
      
              set @tmp_sql = @tmp_sql + ' from #tmpdata group by SalesManID  ,initParentCategoryID '
              set @tmp_sql = @tmp_sql + ' ) tmp1 group by SalesManID '
              set @tmp_sql = @tmp_sql + ' ) tmp '
              set @tmp_sql = @tmp_sql + ' where #tmpTransformedData.SalesmanID = tmp.SalesmanID '
         end
...
--     (select SalesManID, count(distinct InvoiceID) as BillsCnt from #tmpData group by SalesManID) tmp
     (select SalesManID, count(distinct case when  #tmpdata.InvoiceType in (1,3) then #tmpdata.InvoiceID end) as BillsCnt from #tmpData group by SalesManID) tmp
     where #tmpTransformedData.SalesManID = tmp.SalesmanID
        
     set @Fields_sql = 'Select SalesManID,SalesMan_Name as [DS Name] ,DSType as [DS Type],
	' + @Fields_sql + ',
	sum(TotalQty) as [Cat.Grp Wise Total No.Of  Bills Cut], Sum(CatwiseTotalValue) as [Value]' + ' from
	#tmpTransformedData group by SalesManID,SalesMan_Name, DSType having sum(TotalQty) > 0'
...
Begin
     ...
     set @Fields_sql = 'Select SalesManID,SalesMan_Name as [DS Name] , DSType as [DS Type],
	' + @Fields_sql + ',
	Sum(CatwiseTotalValue) as [Value] ' + ' from
	#tmpTransformedData group by SalesManID, SalesMan_Name, DSType'
	exec sp_executesql @Fields_sql --Selects the data
End
...
Begin
     set @Fields_sql = 'Select SalesManID,SalesMan_Name as [DS Name] , DSType as [DS Type],
	' + @Fields_sql + ',
	Sum(CatwiseTotalQty) as [Total Volume],Sum(CatwiseTotalValue) as [Value] ' + ' from
	#tmpTransformedData group by SalesManID,SalesMan_Name, DSType'
	exec sp_executesql @Fields_sql --Selects the data        
End        
Handler:        
