Create Procedure [dbo].[sp_print_InvAbstract_Windows_GST_ASL](@INVNO INT)
AS
Set dateformat DMY
...
Declare @WDDL20 nVarChar(50)
Declare @WDDL21 nVarChar(50)

Select @WDDL20 = DL20, @WDDL21 = DL21,@WDFSSAINO =   Case when Setup.STRegn = '' then '' else 'FSSAI No. : ' + Setup.STRegn End From Setup
...
exec dbo.sp_print_RetInvItems_RespectiveUOM_MED_ITC_GST @INVNO,1
--Select  1
...
Set @GSTaxCompHead = 'Rate' + space(10) +  'TaxableVal' + SPACE(10) + 'CGST' + SPACE(10) +Case when @UTGST_flag = 1 then 'UTGST' Else  '  SGST' End  + SPACE(6) + 'Total Tax'
Set @GSTaxCompHead_DOS = 'Rate'+ SPACE(2)+'TaxableVal' + SPACE(7) + 'CGST' + SPACE(6) +Case when @UTGST_flag = 1 then 'UTGST' Else  ' SGST' End  + SPACE(2) + 'Total Tax'
Set @GSTaxCompDet = ''
Set @GSTaxCompDet_DOS  = ''
--Select TaxableValue=Sum(TaxableValue),Rate =Convert(Decimal,SGSTPer)+Convert(Decimal , CGSTPer) ,SGSTAmt =Sum(SGSTAmt), CGSTAmt=Sum(CGSTAmt) , Total=Sum(CGSTAmt+SGSTAmt)
--Into #GSTTaxCompDet
----, IGSTPer=Max(IGSTPer),IGSTAmt=SUM(IGSTAmt)
--From #TempInvDet2 where Convert(Decimal,SGSTPer)+Convert(Decimal , CGSTPer) > 0
--Group By SGSTPer, CGSTPer
...
SGSTAmt =Sum(Case when @UTGST_flag = 1 then UTGSTAmt Else  SGSTAmt End ), CGSTAmt=Sum(CGSTAmt) , Total=Sum(CGSTAmt+Case when @UTGST_flag = 1 then UTGSTAmt Else  SGSTAmt End)
Into #GSTTaxCompDet
--, IGSTPer=Max(IGSTPer),IGSTAmt=SUM(IGSTAmt)
...
"DL 20" = IsNull(DLNumber,''),
"DL 21" = IsNull(DLNumber21,''),
"WDDL20" = IsNull(@WDDL20,''),
"WDDL21" = IsNull(@WDDL21,''),
