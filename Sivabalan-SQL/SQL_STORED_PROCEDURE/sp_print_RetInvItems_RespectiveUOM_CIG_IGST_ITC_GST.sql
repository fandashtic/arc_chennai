Create Procedure [dbo].[sp_print_RetInvItems_RespectiveUOM_CIG_IGST_ITC_GST](@INVNO INT,@MODE INT=0)
AS
Begin
...
Cast((Case When InvoiceDetail.UOMPrice <> 0  THEN Sum(#Temp1.SRQty * InvoiceDetail.UOMQty)  Else 0 	End) as decimal(18,2) )
--Else Cast(sum(IGSTAmt) as decimal(18,2)) 	End
as nVarchar),''),
...
Else Cast(Cast(Sum(ADDLCESSAmt) as decimal(18,2))as nVarchar)
End,
...
"Item MRP" =  isnull(Case when Duplicate = 1 then
cast(case isnull(InvoiceDetail.MRPPerpack,0) when 0 then isnull(Items.MRPPerPack,0) else isnull(InvoiceDetail.MRPPerpack,0) end as nvarchar)
Else Cast(Cast(Max(IGSTPer) as decimal(18,2))as nVarchar) End,''),
...
"IGST Rate" = Cast(max(Isnull(InvoiceDetail.IGSTPer,0)) as nvarchar),
...
TaxDetailsWithBreakup = '',TaxDetails='',[Batch No.]='',[Total Tax] = '',[Other Disc] = ''
Where Duplicate = 2
--GST_Changes ends here
