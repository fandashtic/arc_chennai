CREATE Procedure mERP_spr_TMDOutletSalDmg_XMLSplitUp(@FromDate DateTime, @ToDate DateTime)
As
set dateformat dmy
...
From #OLClassMapping olcm, Customer C
...
Select "WD_Code" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(WDCode, '')), 
	"WD_Dest_Code" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(WDDest, '')), 
	"From_Date" = Convert(nVarchar(10),@FromDate,103)  ,
	"To_Date" = Convert(nVarchar(10),@ToDate,103), 
	"Customer_ID" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(CustomerID, '')), 
	"RCS_ID" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(RCSID, '')),
	"System_SKU_Code" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(SKUCode, '')), 
	"Sales" = Sum(IsNull(Sales,0)), 
	"Sales_Return_Damages" = Sum(IsNull(SRDmg, 0)),
	"Value" = Sum(IsNull(Val, 0)), 
	"Active_in_RCS" = dbo.mERP_fn_FilterSplChar_ITC(IsNull([Active in RCS], '')),
	"Sales_With_GV" = Sum(IsNull(SalesWithGV, 0)),
	"Value_With_GV" = Sum(IsNull(ValueWithGV, 0)), 
	"Customer_Type" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(CustomerType, '')),
	"Channel_Type" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(ChannelType, '')),
	"Outlet_Type" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(OutletType, '')),
	"Loyalty_Program" = dbo.mERP_fn_FilterSplChar_ITC(IsNull(LoyaltyProgram, '')),
	"Base_GOI_Market_ID" = dbo.mERP_fn_FilterSplChar_ITC(IsNull([Base GOI Market ID], '')),
	"Base_GOI_Market_Name" = dbo.mERP_fn_FilterSplChar_ITC(IsNull([Base GOI Market Name], ''))
