CREATE procedure [dbo].[spr_list_invoices_by_salesman_abstract] (@Salesman nvarchar(2550),  
...
SELECT  isnull(Salesman.SalesmanID, 0 ), "Salesman" = case isnull(Salesman.SalesmanID, 0 ) when 0 then @OTHERS else Salesman.Salesman_Name end,     
 "Net Value (%c)" = Sum(Case InvoiceType When 4 then -NetValue Else NetValue End) - Sum(Case InvoiceType When 4 then -Freight Else Freight End),    
 "Balance (%c)" = Sum(Case InvoiceType When 4 then -Balance Else Balance End)    
