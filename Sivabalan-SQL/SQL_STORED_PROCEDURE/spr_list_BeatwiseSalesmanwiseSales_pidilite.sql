CREATE Procedure spr_list_BeatwiseSalesmanwiseSales_pidilite (@BeatName nVarChar(2550), 
@SalesmanName nVarChar(2550), @Category nVarchar(2550), 
...
"Salesman Name", GoodsValue "Goods Value", ia.DiscountValue + AddlDiscountValue "Discount",      
...
0 "Credit Invoice"       
From InvoiceAbstract ia Join InvoiceDetail ids On ia.InvoiceID = ids.InvoiceID Join
Items its On ids.Product_Code = its.Product_Code Left Join Beat be On       
...
And IsNull(its.CategoryID, 0) In (select CategoryID From #tempCategory)
