CREATE procedure  [dbo].[sp_ser_rpt_ServiceInvoiceAbstract](@Fromdate datetime,@Todate datetime)                                        
As          
...
(case isnull(Serviceinvoiceabstract.PaymentMode,'') when 0 then 'Credit' when 1 then 'Cash' when 2 then 'Cheque' when 3 then 'DD'  when 4 then 'Credit Card' when 5 then 'Coupon' else '' end) as 'Payment Mode',                                          
...
'Collected Amount' =(isnull(serviceinvoiceabstract.Netvalue,0) + isnull(roundoffamount,0)-isnull(balance,0)),              
