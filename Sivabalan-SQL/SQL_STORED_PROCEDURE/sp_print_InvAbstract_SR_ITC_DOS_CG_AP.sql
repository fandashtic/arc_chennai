...
Create Procedure [dbo].[sp_print_InvAbstract_SR_ITC_DOS_CG_AP](@INVNO INT)     
AS          
Set Dateformat DMY
...
exec sp_print_RetInvItems_RespectiveUOM_SR_ITC_AP @INVNO,1
--Select 1 
...
"Net Amount Payable" = cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then 0 - ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as Nvarchar(13)),
...
"InvoiceTotals" = 
				  ...
				  '|Net Amt. Payable     :' + Space(15-Len(cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then 0 - ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as Nvarchar(13)) + Char(13) + Char(10)))
											+ cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then 0 - ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as Nvarchar(13)) + Char(13) + Char(10),
...
"TIN/NON TIN" = ' I N V O I C E' ,
"Disclaimer" = 'This is a Second sale in the State of Andhra Pradesh.',
