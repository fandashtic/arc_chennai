CREATE Procedure sp_save_invoice_detail_CSP_van_pharma_MUOM_ITC(@INVOICE_ID int,
...
DECLARE @InventoryMRPPERPACK Decimal(18,6)
...
SELECT @COST = Purchase_Price, @MRP = MRP, @SALEID = SaleID, @PTS = PTS, @PTR = PTR,@InventoryMRPPERPACK = ISNULL(MRPPerPack,0), @HSNNumber = isnull(HSNNumber,''), @CategorizationID = isnull(CategorizationID,0)
...
BEGIN
	...
	VALUES (@INVOICE_ID, @ITEM_CODE,@TEMPID, @BATCH_NUMBER, @REQUIRED_QUANTITY, @MODIFIED_PRICE, 
	...
	@MultiSchID, @TotSchAmount,@MultiSchIDAndCost,@MultipleRebateID,@RebateRate,@MultipleRebateDet,@GroupID,@InventoryMRPPERPACK,
	@TAXONQTY,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)
	GOTO ALL_SAID_AND_DONE
END
IF @TRACK_BATCHES = 1
	...
	IF @CUSTOMERTYPE = 1
		...
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK And ISNULL(Batch_Products.Free,0) = @FreeRow

		DECLARE ReleaseStocks CURSOR KEYSET FOR
		SELECT VanStatementDetail.Batch_Number, VanStatementDetail.Batch_Code, Pending, VanStatementDetail.PurchasePrice, [ID], 
		VanStatementDetail.PTS, VanStatementDetail.PTR, VanStatementDetail.ECP ,Batch_Products.MRPPERPACK
		FROM VanStatementDetail, Batch_Products
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		END
	ELSE IF @CUSTOMERTYPE = 2
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.PTR, 0) = @GenericPTR AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK And ISNULL(Batch_Products.Free,0) = @FreeRow

		DECLARE ReleaseStocks CURSOR KEYSET FOR
		SELECT VanStatementDetail.Batch_Number, VanStatementDetail.Batch_Code, VanStatementDetail.Pending, 
		...
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.PTR, 0) = @GenericPTR AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		END
	ELSE IF @CUSTOMERTYPE = 3
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.SpecialPrice, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK And ISNULL(Batch_Products.Free,0) = @FreeRow
		...
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.SpecialPrice, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		END
	ELSE
		BEGIN
		SELECT @TOTAL_QUANTITY = ISNULL(SUM(Pending), 0) FROM VanStatementDetail, Batch_Products
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.SalePrice, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK And ISNULL(Batch_Products.Free,0) = @FreeRow
		...
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.SalePrice, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		END
	END
ELSE
	BEGIN
	IF @CUSTOMERTYPE = 1
		...
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		
		DECLARE ReleaseStocks CURSOR KEYSET FOR
		SELECT VanStatementDetail.Batch_Number, VanStatementDetail.Batch_Code, Pending, 
		VanStatementDetail.PurchasePrice, [ID], VanStatementDetail.PTS, VanStatementDetail.PTR, VanStatementDetail.ECP ,Batch_Products.MRPPERPACK FROM 
		VanStatementDetail, Batch_Products
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		END
	ELSE IF @CUSTOMERTYPE = 2
		BEGIN
		SELECT @TOTAL_QUANTITY = ISNULL(SUM(Pending), 0) FROM VanStatementDetail, Batch_Products
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.PTR, 0) = @GenericPTR And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK 
		...
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.PTR, 0) = @GenericPTR AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		END
	ELSE IF @CUSTOMERTYPE = 3
		BEGIN
		SELECT @TOTAL_QUANTITY = ISNULL(SUM(Pending), 0) FROM VanStatementDetail, Batch_Products
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.SpecialPrice, 0) = @SALE_PRICE And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		...
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.SpecialPrice, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		END
	ELSE
		BEGIN
		SELECT @TOTAL_QUANTITY = ISNULL(SUM(Pending), 0) FROM VanStatementDetail, Batch_Products
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.SalePrice, 0) = @SALE_PRICE And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		...
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.SalePrice, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow AND ISNULL(Batch_Products.MRPPERPACK, 0) = @MRPPERPACK
		END
	END
OPEN ReleaseStocks
...
--		SELECT @BATCHMRPPERPACK = ISNULL(MRPPerPack,0) FROM Items WHERE Product_code = @ITEM_CODE
	...
	PurchasePrice, STPayable, MRP, TaxID, FlagWord, SaleID, TaxCode2, CSTPayable, 
	TaxSuffered, TaxSuffered2, PTS, PTR, UOM, UOMQTY, UOMPRICE, OtherCG_Item,QuotationID, 
	MultipleSchemeID, TotSchemeAmount,MultipleSchemeDetails,MultipleRebateID,RebateRate,
	...
	@REQUIRED_QUANTITY * @COST, @STPAYABLE, @MRP, @TAXID, @FLAG, @SALEID, 
	@TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @PTS, @PTR, @UOM, @UOMQTY,
	@UOMPRICE, @OtherCG_Item,@QuotationID,@MultiSchID, @TotSchAmount,@MultiSchIDAndCost,@MultipleRebateID,@RebateRate
	,@MultipleRebateDet,@GroupID,@BATCHMRPPERPACK,@TAXONQTY,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)
	...
	MultipleSchemeID, TotSchemeAmount,MultipleSchemeDetails,MultipleRebateID,RebateRate,MultipleRebateDet,GroupID,MRPPerPack,
	TAXONQTY,GSTFlag,GSTCSTaxCode,HSNNumber,CategorizationID)
	VALUES (@INVOICE_ID, @ITEM_CODE, @ROWID, @BATCH_NUMBER, @QUANTITY, 
	@MODIFIED_PRICE, @TAXCODE, @DISCOUNTPERCENTAGE, @DISCOUNTAMOUNT, @AMOUNT, 
	@COST * @QUANTITY, @STPAYABLE, @MRP, @TAXID, @FLAG, @SALEID, @TAXCODE2, 
	@CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @PTS, @PTR, @UOM, @UOMQTY, @UOMPRICE, @OtherCG_Item,@QuotationID,
...
Begin
	...
	--		Values(@ITEM_CODE, @PRIMARY_QUANTITY, @ORIGINAL_QTY, @MODIFIED_PRICE * @ORIGINAL_QTY, @SCHEME_COST, @SCHEMEID, @INVOICE_ID, 0, @ORIGINAL_QTY)
