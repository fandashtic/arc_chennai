Create PROCEDURE mERP_spr_StockControlRpt_ITC_Upload
(
...
Begin
If ((Select dbo.StripTimeFromDate(LastInventoryUpload) From Setup) >= dbo.StripTimeFromDate(DateAdd(d, 1, @TODATE)))
Set @DayClosed = 1
End
...
--Select InvoiceID, InvoiceType, Status, NewReference,  Cast( IsNull(Reverse(left(reverse(IsNull(NewReference,'')),
--        Case When PATINDEX( N'%[^0-9]%', Reverse(IsNull(newreference, ''))) > 0 Then PATINDEX( N'%[^0-9]%',Reverse(IsNull(NewReference, ''))) -1
--    Else Len(IsNull(NewReference, '')) End )) , 0)
--         As bigint) NRef, IsNull(GSTFullDocID,'') SRInvoiceID, CustomerID Into #InvoiceGST_tmp
--From InvoiceAbstract
--Where InvoiceAbstract.InvoiceDate BETWEEN @FROMDATE AND @TODATE AND (InvoiceAbstract.Status & 128) = 0
...
"ClosingStockD&DValue"= Cast(0 as Decimal(18,6)),
...
--PTS,
Batch_products.TaxSuffered, DocDate, isnull(TOQ,0),Isnull(CS_TaxCode,0) ,Isnull(GRNTaxID,0) ,Isnull(GSTTaxType,0),ISNULL(Items.Sale_Tax,0)
...
Group by Batch_Products.Product_Code,Batch_Products.Batch_Code,Batch_Products.QuantityReceived,Batch_Products.Quantity,Batch_Products.PTS,
Batch_Products.TaxSuffered,Batch_Products.DocDate,Batch_Products.TOQ,Tax.CS_TaxCode,Batch_Products.GRNTaxID,Batch_Products.GSTTaxType,Batch_Products.PFM,Items.Sale_Tax
...
Where  dbo.StripTimeFromDate(DocDate) <= @OpeningDate
) as Upd
Where Upd.Batch_Code = #Batch.Batch_Code


Update T Set [ClosingStockD&DValue] = Temp.CSDandDValue
...
)Destroy On BP.Product_Code = Destroy.Product_Code And BP.Batch_Code = Destroy.Batch_Code
Left Outer Join (
Select Product_Code, Batch_Code, AdjQty From #StkAdj
)StkAdj On BP.Product_Code = StkAdj.Product_Code And BP.Batch_Code = StkAdj.Batch_Code
...
---- *newly added Closing Stock D&D Tax Value*-----------
...
[Closing Stock - Saleable] > 0 Or [Closing Stock - D&D] > 0 Or  [ClosingStockD&DValue] > 0 or
[ClosingStockD&DTaxValue] > 0 Or   [Total Closing Stock] > 0 Or [SIT] > 0) Or [Active] = 1
