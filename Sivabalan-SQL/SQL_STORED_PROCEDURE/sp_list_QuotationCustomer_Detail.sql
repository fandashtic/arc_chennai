...
Create PROCEDURE sp_list_QuotationCustomer_Detail(      
@QuotationID INT,      
...
Begin    
	 SELECT "CustomerID" = QuotationCustomers.CustomerID, "CustomerName" = Company_Name, QuotationName FROM Customer, QuotationAbstract, QuotationCustomers              
	 ...
	 SELECT "CustomerID" = QuotationCustomers.CustomerID FROM Customer, QuotationAbstract, QuotationCustomers              
	         WHERE QuotationCustomers.CustomerID = Customer.CustomerID And QuotationCustomers.QuotationID = QuotationAbstract.QuotationID              
	         And QuotationAbstract.QuotationID = @QuotationID     
	 Union    
	     
	 SELECT QuotationCustomers.CustomerID FROM QuotationCustomers, QuotationAbstract       
	 WHERE QuotationAbstract.Active = 1 And ((QuotationAbstract.ValidFromDate  between @FromDate And @Todate or dbo.StripDateFromTime(QuotationAbstract.ValidToDate) between @FromDate And @Todate) or      
...
Begin  
	 ...
	 order by company_name asc                    
End  
--customers filtered based on beat and segment
Else if @BeatID<>'%' And @ChannelType = '%'  
Begin  
	 SELECT "CustomerID" = QuotationCustomers.CustomerID, "CustomerName" = Company_Name, QuotationName FROM Customer, QuotationAbstract, QuotationCustomers              
	 WHERE QuotationCustomers.CustomerID = Customer.CustomerID And QuotationCustomers.QuotationID = QuotationAbstract.QuotationID              
	 ...
	 order by company_name asc                    
End  
--Customers filtered based on beat,channel and segment
Else  
Begin  
	 SELECT "CustomerID" = QuotationCustomers.CustomerID, "CustomerName" = Company_Name, QuotationName FROM Customer, QuotationAbstract, QuotationCustomers              
	 WHERE QuotationCustomers.CustomerID = Customer.CustomerID And QuotationCustomers.QuotationID = QuotationAbstract.QuotationID              
