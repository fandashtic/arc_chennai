Create Procedure Sp_GetCGLeafCategories_ITC
		...
		if @Category=N'%%' or @Category = N'%' 
			Begin
				  Insert into #tmpCat 
					...
					where CategoryID in (Select CatID From dbo.fn_GetCatFromCatGroup_ITC(@CategoryGroup,@Division,@Delimeter))
					and ParentID in (Select CatID From dbo.fn_GetCatFromCatGroup_ITC(@CategoryGroup,@ProductHierarchy,@Delimeter))
			End
		Else
			Begin
				 Insert into #tmpCat 
...
Else
	Begin	                   
      ...
      select Category_Name from ItemCategories              
		  Where CategoryID in 
      (Select CatID From dbo.fn_GetCatFromCatGroup_ITC(@CategoryGroup,@ProductHierarchy,@Delimeter)) 
		End
		Else    
		Begin               
			 Insert into #tmpCat 
       select Category_Name from ItemCategories  
		   where Category_Name in (select * from dbo.sp_SplitIn2Rows(@Category,@Delimeter))  
		   and CategoryID in (Select CatID From dbo.fn_GetCatFromCatGroup_ITC(@CategoryGroup,@ProductHierarchy,@Delimeter))      
		End
  End
