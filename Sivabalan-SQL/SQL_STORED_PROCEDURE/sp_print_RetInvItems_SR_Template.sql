CREATE PROCEDURE [dbo].[sp_print_RetInvItems_SR_Template](@INVNO INT)    
AS    
/*
Declare @FREE As NVarchar(50)
Declare @F As NVarchar(50)
Declare @S As NVarchar(50)
Declare @SRSALEABLEORDAMAGES as NVarchar(50)
Declare @cnt as int
...
Set @SRSALEABLEORDAMAGES = dbo.LookupDictionaryItem(N'Sales Return Saleable/Damages', Default)
...
SELECT "Item Code" = InvoiceDetail.Product_Code, "Item Name" = Items.ProductName,     
	...
	"Excise duty" = IsNull(InvoiceDetail.ExciseDuty, 0), "TaxComponents"='TaxComponents' into #tmp3
	...
	InvoiceDetail.ExciseDuty, Items.TrackPKD, Items.Virtual_Track_Batches
	...
	Insert into #tmp4([Item Code]) values(N'')
	Insert into #tmp4([Item Code]) values(@SRSALEABLEORDAMAGES)
