CREATE Procedure sp_save_Invoicedetail_CSP_MUOM(@INVOICE_ID int,  
...
IF @TRACK_BATCHES = 1  
 ...
 SELECT @TOTAL_QUANTITY = SUM(Quantity) FROM Batch_Products   
 WHERE Product_Code = @ITEM_CODE AND ISNULL(Batch_Number, N'') = @BATCH_NUMBER   
 AND ISNULL(PTR, 0) = @BATCH_PRICE And ISNULL(Damage, 0) = 0   
 AND (Expiry >= GetDate() OR Expiry IS NULL) And isnull(Free, 0) = @FreeRow  
 ...
 ELSE IF @CUSTOMER_TYPE = 2  
  ...
  FROM Batch_Products  
  WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, N'') = @BATCH_NUMBER   
  and ISNULL(PTR, 0) = @BATCH_PRICE AND ISNULL(Quantity, 0) > 0   
  And ISNULL(Damage, 0) = 0 AND (Expiry >= GetDate() OR Expiry IS NULL) And isnull(Free, 0) = @FreeRow  
  END  
 ELSE IF @CUSTOMER_TYPE = 3  
  BEGIN  
...
ELSE  
 ...
 ELSE IF @CUSTOMER_TYPE = 2   
 SELECT @TOTAL_QUANTITY = SUM(Quantity) FROM Batch_Products   
 WHERE Product_Code = @ITEM_CODE AND ISNULL(PTR, 0) = @BATCH_PRICE   
 And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow  
 ELSE IF @CUSTOMER_TYPE = 3   
 SELECT @TOTAL_QUANTITY = SUM(Quantity) FROM Batch_Products   
 ...
 ELSE IF @CUSTOMER_TYPE = 2  
  ...
  SELECT Batch_Number, Batch_Code, Quantity, PurchasePrice, PTR, PTS, ECP  
  FROM Batch_Products  
  WHERE Product_Code = @ITEM_CODE AND ISNULL(PTR, 0) = @BATCH_PRICE AND   
  ISNULL(Quantity, 0) > 0 And ISNULL(Damage, 0) = 0   
  And isnull(Free, 0) = @FreeRow  
  END  
 ELSE IF @CUSTOMER_TYPE = 3  
...
BEGIN  
 ...
 @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)  
 SET @AMOUNT = 0  
 SET @DISCOUNT_PER = 0  
