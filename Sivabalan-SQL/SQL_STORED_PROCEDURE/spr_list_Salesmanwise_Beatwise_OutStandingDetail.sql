CREATE procedure [dbo].[spr_list_Salesmanwise_Beatwise_OutStandingDetail](@SalesmanBeatID nvarchar(50),
							@FromDate datetime,
							@ToDate datetime)
as
...
WHERE   ISNULL(Balance, 0) > 0 AND DocumentDate Between @FromDate AND @ToDate AND
	CreditNote.CustomerID IS NOT NULL And CreditNote.CustomerID *= Beat_Salesman.CustomerID
...
WHERE ISNULL(Balance, 0) > 0 AND DocumentDate Between @FromDate AND @ToDate AND
	DebitNote.CustomerID IS NOT NULL And DebitNote.CustomerID *= Beat_Salesman.CustomerID
