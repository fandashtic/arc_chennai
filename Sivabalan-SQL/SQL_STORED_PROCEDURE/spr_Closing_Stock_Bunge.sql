CREATE procedure [dbo].[spr_Closing_Stock_Bunge]          
(          
@Given_Date DateTime,           
@UOM nVarchar(256)          
)          
...
If @UOM = N'Sales UOM'          
  ...
  "Damaged Value" = sum( Case When isnull(Damage,0)<>0 then Quantity * PurchasePrice Else 0 END) ,        
    ...
    Else        
      ...
      "Saleable Stock" = Isnull(Cast(Opening_Quantity - Damage_Opening_Quantity as nVarchar),0), --+ ' ' + UOM.Description,         
      "Free Stock" = Isnull(Cast(Free_Opening_Quantity as nVarchar),0), --+ ' ' + UOM.Description         
   "Damaged Stock" = Isnull(Damage_opening_Quantity,0),    
      "Damaged Value" = Isnull(Damage_OPening_Value,0),    
   "Closing Value" = Isnull(Opening_Value,0),        
      "Forum Code" = Items.Alias         
...
Else        
select OpeningDetails.Product_Code, "Item Code" = OpeningDetails.Product_Code,         
"Saleable Stock" = Cast(Isnull(Cast((Opening_Quantity - Damage_Opening_Quantity) * (CASE IsNull(Items.ConversionFactor,0) WHEN 0 THEN 1 ELSE IsNull(Items.ConversionFactor,0) END)as Decimal(18,6)),0) as nVarchar), --+ ' ' + Isnull(ConversionTable.ConversionUnit, ''),
"Free Stock" = Cast(Isnull(Cast(Free_Opening_Quantity * (CASE IsNull(Items.ConversionFactor,0) WHEN 0 THEN 1 ELSE IsNull(Items.ConversionFactor,0) END)as Decimal(18,6)),0)  as nVarchar), -- + ' ' + Isnull(ConversionTable.ConversionUnit, '')          
"Damaged Stock" = Cast(Isnull(Cast(Damage_opening_Quantity * (CASE IsNull(Items.ConversionFactor,0) WHEN 0 THEN 1 ELSE IsNull(Items.ConversionFactor,0) END)as Decimal(18,6)),0)  as nVarchar),    
"Damaged Value" = Isnull(Damage_OPening_Value,0),    
"Closing Value" = Isnull(Opening_Value,0),        
"Forum Code" = Items.Alias    
...
Else        
select OpeningDetails.Product_Code, "Item Code" = OpeningDetails.Product_Code,         
"Saleable Stock" = Isnull((Opening_Quantity - Damage_Opening_Quantity)/ (Case Isnull(Reportingunit,0) When 0 then 1 else ReportingUnit End),0),  
"Free Stock" = Isnull((Free_Opening_Quantity)/ (Case Isnull(Reportingunit,0) When 0 then 1 else ReportingUnit End),0),        
"Damaged Stock" = Isnull((Damage_opening_Quantity / (Case Isnull(Reportingunit,0) When 0 then 1 else ReportingUnit End)),0),    
"Damaged Value" = Isnull(Damage_OPening_Value,0),    
"Closing Value" = Isnull(Opening_Value,0),        
"Forum Code" = Items.Alias         
