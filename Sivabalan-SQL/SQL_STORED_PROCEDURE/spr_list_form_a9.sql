create Procedure dbo.spr_list_form_a9  
(   
@Items nvarchar(256) ,
@FromDate DateTime,   
@ToDate DateTime   
...
Begin  
	...
	t2.product_code as ItemCode,
	t3.productname as ItemName,
	t1.vendorid As VendorId,
	t4.vendor_name as VendorName,
	t4.Address as Address,
	Isnull(t4.CST,N'') as CSTRegno,
	IsNull(t4.TNGST,N'') as STRegno,
	"BillNo"=case Isnull(t1.billreference,0)
	when 0 then @bprefix + cast(t1.documentid as nvarchar) 
		else @baprefix + cast(t1.documentid as nvarchar) 
	End,
	t1.invoicereference as Invoiceno,
	t1.billdate as Billdate,
	sum(t2.quantity) as Quantity,
	sum(t2.amount)+ sum(t2.taxamount) as TotalValue   
...
group by t2.product_code,t3.productname,t1.vendorid,t4.vendor_name,t4.Address,t1.documentid,
	t2.quantity,t1.billdate,t4.CST,t4.TNGST,t1.billreference,t1.invoicereference
...
if(@Items=N'%')  
Begin  
select t2.product_code,t2.product_code as ItemCode,
...
where t1.billid=t2.billid  
	...
	and (t1.status & 128=0)  
	and t1.billdate between @FromDate and @ToDate  
group by t2.product_code,t3.productname,t1.vendorid,t4.vendor_name,t4.Address,t1.documentid,t2.quantity,t1.billdate,t4.CST,t4.TNGST,
t1.billreference,t1.invoicereference  
order by t1.documentid  
End  
