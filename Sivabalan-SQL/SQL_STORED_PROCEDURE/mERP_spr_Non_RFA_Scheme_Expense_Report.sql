Create Procedure [dbo].[mERP_spr_Non_RFA_Scheme_Expense_Report]
(
 @FromDate Datetime,
 @ToDate Datetime,
...
CREATE TABLE [dbo].[#tmpFinalData](
	...
	Create Table #FreeInfo(InvoiceID Int, BillRef nVarchar(255) collate SQL_Latin1_General_CP1_CI_AS, 
		...
		UOM nVarchar(255) collate SQL_Latin1_General_CP1_CI_AS)
	if object_id('tempdb..#tmpPayout') is not null
...
CREATE CLUSTERED INDEX IDX_C_TmpTSRInvAbs_InvoiceID ON #TmpTSRInvAbs(InvoiceID)
...
CREATE TABLE [dbo].[#TmpTSRInvDtl](
	...
	[GroupID] [int] NULL,
	[TAXONQTY] [int] Not Null Default 0
	) ON [PRIMARY]
CREATE NonCLUSTERED INDEX IDX_NonC_TmpTSRInvDtl_InvoiceID ON #TmpTSRInvDtl(InvoiceID)
...
--Select '1',* from #tmpActivityCode
--Select '2',* from #tmpScheme
...
--				and  PayoutPeriodFrom >= @FromDate and PayoutPeriodTo <= @ToDate  and isnull(Status,0) <> 0  
				...
				Select Schemeid,PayoutID,PayoutPeriodFrom,PayoutPeriodTo from #tmpPayout1
				Open @clu_0
	...
	End
--*************************************************************************************************
	select * from #TmpSchememaster*/
			--	delete #TmpSchememaster
Begin
--				 
				...
				--Select '4',* from #TmpSchememaster
				
				If OBJECT_ID('tempdb..#NonQpsSchemes') is not null
					Drop Table #NonQpsSchemes
				...
				--in (select Distinct SchemeID	from tbl_merp_NonQPSData where dbo.striptimefromdate(Invoicedate) between @FromDate and @ToDate)
				And SA.SchemeID = #tmpScheme.SchemeID
				
				--Select '5',* from #TmpSchememaster
...
--Set @TimeLine = '1-' + convert(nvarchar,getdate()-@StartTime,114) 

--If OBJECT_ID('tempdb..#tbl_merp_QPSAbsData') is not null
--	Drop Table #tbl_merp_QPSAbsData 

--Select QA.* Into #tbl_merp_QPSAbsData 
--From tbl_merp_QPSAbsData QA
--Join #TmpSchememaster TSA On TSA.Schemeid = QA.SchemeID 

--If OBJECT_ID('tempdb..#tbl_merp_QPSDtlData') is not null
--	Drop Table #tbl_merp_QPSDtlData 

--Select QD.* Into #tbl_merp_QPSDtlData
--From tbl_merp_QPSDtlData QD
--Join #tbl_merp_QPSAbsData QA On QA.SchemeID = QD.SchemeID 
...
--Set @TimeLine = @TimeLine + ' , ' + '2-' + convert(nvarchar,getdate()-@StartTime,114) 
	...
	--Select '5A',* from #tmpLesGrtDate
	--Select '5B',* from #TmpTSRInvAbs 
	--Select '5C',* from #TmpTSRInvDtl 
--Set @TimeLine = @TimeLine + ' , ' +'3-' + convert(nvarchar,getdate()-@StartTime,114) 
--			set @Counter = 0						
...
--	If @szPayoutID > 0
--		Begin
--			Select @LesserDate = (select PayoutPeriodFrom From tbl_mERP_SchemePayoutPeriod Where Id = @szPayoutID)
--			Select @GreaterDate = (Select PayoutPeriodTo From tbl_mERP_SchemePayoutPeriod Where Id = @szPayoutID)
--		End
--	Else
--		Begin
--			Select @LesserDate = dbo.StripTimeFromDate(Min(PADateFrom)) From #tmpLesGrtDate Where SchemeID = @SchemeID 
--			Select @GreaterDate = dbo.StripTimeFromDate(Max(PADateTo)) From #tmpLesGrtDate Where SchemeID = @SchemeID 
...
*/
	...
	--Set @TimeLine = @TimeLine + ' , ' +'4-' + convert(nvarchar,getdate()-@StartTime,114) 
	...
	--Select '5*',LDt=@LesserDate , GDt=@GreaterDate,IFDt=@InvFrom ,ITDt = @InvTo, ActF=@ActiveFrom,ActT=@ActiveTo,PFDt=@PayoutFrom ,PTDt=@PayoutTo
	--Select '5A',* from #tmpLesGrtDate
	--Select '5B',* from #TmpTSRInvAbs 
	--Select '5C',* from #TmpTSRInvDtl 
	--select '5D',* from #tmpSales
	
	If (select count(*) from #tmpSales) = 0
	Goto NoScheme
	
	--Set @TimeLine = @TimeLine + ' , ' +'5-' + convert(nvarchar,getdate()-@StartTime,114) 	
	--select '5D',* from #tmpSales
	...
	/* ForFor schemes which are already submitted take it directly from RFAAbstract & RFADetail table */
		...
		--Set @TimeLine = @TimeLine + ' , ' +'6-' + convert(nvarchar,getdate()-@StartTime,114) 	

	  --Set @TimeLine = @TimeLine + ' , ' +'7-' + convert(nvarchar,getdate()-@StartTime,114) 	
	...
	--Set @TimeLine = @TimeLine + ' , ' +'8-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
--                 Else TaxAmount End),0) as TaxAmount,
				IsNull((Case  When InvoiceType = 4 and [Type] IN (0,2,3) Then (Case Isnull(TOQ,0) When 0 Then (PromotedValue * isNull(TaxPercent,0)/100 ) 
								Else (IsNull(PromotedQty, 0) * isNull(TaxPercent,0)) End)	
                                  When InvoiceType = 1  and [Type] IN (0,2) Then (Case Isnull(TOQ,0) When 0 Then (PromotedValue * isNull(TaxPercent,0)/100 ) 
								Else (IsNull(PromotedQty, 0) * isNull(TaxPercent,0)) End)		
                               When InvoiceType = 3 and [Type] IN(0,2) Then (Case Isnull(TOQ,0) When 0 Then (PromotedValue * isNull(TaxPercent,0)/100 ) 
								Else (IsNull(PromotedQty, 0) * isNull(TaxPercent,0)) End)		
                 Else TaxAmount End),0) as TaxAmount,
	...
	--Set @TimeLine = @TimeLine + ' , ' +'9-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
-- If A Invoice having More than One item applied QPS Scheme then That all Items are Listed in the Report.(Before Only one Item Listed)
	...
	Insert into #tmpSKU Select Distinct Product_code From tbl_merp_QPSdtldata Where  SchemeID =  @SchemeID
		...
		Begin
		    ...
		    Select IA.InvoiceID, @InvPrefix + Cast(IA.DocumentID as nVarchar) as BillRef, 
				...
				1, 1, @SchemeID, @PayoutID, @MarginPTR, Max(ID.TaxCode), (Case Max(Isnull(ID.TAXONQTY,0)) When 0 Then 
	...
	--Set @TimeLine = @TimeLine + ' , ' +'10-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
--*********************************************************************************************************************************************
--Set @TimeLine = @TimeLine + ' , ' +'11-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
Begin
	...
	Close @cur_9
	Deallocate @cur_9
--Set @TimeLine = @TimeLine + ' , ' +'12-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
--		Drop Table #tmpQtySaleValue
	...
	--Set @TimeLine = @TimeLine + ' , ' +'13-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
--Set @TimeLine = @TimeLine + ' , ' +'14-' + convert(nvarchar,getdate()-@StartTime,114) 	
...
--			@ExpiryDate = SA.ExpiryDate,	
		...
		Select 	ID.Product_Code as SKUCode,
			...
			And IA.CustomerID = C.CustomerID
			And (Case IA.InvoiceType
						When 3 Then (Select dbo.StripTimeFromDate(Min(InvoiceDate)) From #TmpTSRInvAbs (NoLock) Where DocumentID = 
						IA.DocumentID
						And InvoiceType = 1 And isnull(CancelDate,'') <> '' and CustomerID = IA.CustomerID)
						Else dbo.StripTimeFromDate(IA.InvoiceDate)
						End) Between @ActiveFrom And @ActiveTo
			And (Case IA.InvoiceType
					When 3 Then (Select dbo.StripTimeFromDate(Min(InvoiceDate)) From #TmpTSRInvAbs (NoLock) Where DocumentID = IA.DocumentID
						And InvoiceType = 1 And isnull(CancelDate,'') <> '' and CustomerID = IA.CustomerID) 
					Else dbo.StripTimeFromDate(IA.InvoiceDate)
					End) Between @PayoutFrom And @PayoutTo
	...
	--Set @TimeLine = @TimeLine + ' , ' +'15-' + convert(nvarchar,getdate()-@StartTime,114) 		
		/* Explore QPS Scheme */
		...
		Begin
			...
			Begin
				...
				Begin
					...
					Select IA.InvoiceID, @InvPrefix + Cast(IA.DocumentID as nVarchar) as BillRef, IA.CustomerID, @SKUCode,Sum(Quantity),
						...
						(Case Max(ID.TAXONQTY) When 0 Then  @MarginPTR + (@MarginPTR * (Max(ID.TaxCode) / 100)) Else (@MarginPTR + Max(ID.TaxCode)) End ) ,
						'Free',
					    IA.DocReference
						From #TmpTSRInvAbs IA, #TmpTSRInvDtl ID
				...
				Close OffTakeSKUCur
				Deallocate OffTakeSKUCur
			--Set @TimeLine = @TimeLine + ' , ' +'16-' + convert(nvarchar,getdate()-@StartTime,114) 		
			...
			Begin /*Offtake - Free Item Based Scheme  Starts*/
				If Exists(Select * from tbl_mERP_QPSDtlData (NoLock) 
				Where Schemeid = @SchemeID and payoutid = @PayoutID And QPSAbsDataID = 2 
			...
			Begin
				...
				Begin
					...
					Select  InvoiceID, SchemeId, BillRef , QPS.CustomerID ,
							...
							InvDocRef
							From tbl_mERP_QPSDtlData QPS (NoLock)
							,Customer Cust (NoLock)
							Where 
							QPS.SchemeID = @SchemeID
							And PayoutID = @PayoutID
							And (isNull(Rebate_Qty,0) > 0 Or isNull(Rebate_Val,0) > 0 Or IsNull(RFARebate_Val,0) > 0)
	...
	--Set @TimeLine = @TimeLine + ' , ' +'17-' + convert(nvarchar,getdate()-@StartTime,114) 		
		Select @QPS = 2
	...
	Begin
		...
		End
	End 
--Set @TimeLine = @TimeLine + ' , ' +'18-' + convert(nvarchar,getdate()-@StartTime,114) 				
if (select Distinct SlabType from tbl_mERP_SchemeSlabDetail Where schemeid = @schemeid) in (2,3)
Begin
	...
	--Set @TimeLine = @TimeLine + ' , ' +'19-' + convert(nvarchar,getdate()-@StartTime,114) 				
--	Update T Set T.SaleQty = M.Qty,T.SaleValue =  M.Svalue From
--	(Select D.Product_Code AS Product_Code ,Sum(Isnull(D.Quantity,0)) As Qty,Isnull(sum(D.Quantity * D.SalePrice),0) as Svalue From 
--	 #TmpTSRInvDtl D, #TmpTSRInvAbs A,#tmpFinaldata T where A.Invoicedate Between @InvFrom and @InvTo And  A.InvoiceType  in (1,3) and (A.Status & 128) = 0 
--	and D.InvoiceId = A.InvoiceId And  D.Product_Code IN (Select Distinct  Code from #tmpFinaldata Where [Type] = 'MAIN') 
--	Group by D.Product_Code) AS M,#tmpFinaldata AS T		
--	Where T.[Type] Like '%MAIN%' And  T.Code  = M.Product_Code
...
Begin    
  ...
  --Set @TimeLine = @TimeLine + ' , ' +'20-' + convert(nvarchar,getdate()-@StartTime,114) 				
  ...
  Begin    
    --If @ApplyOn = Line And @ItemGrp = Spl.Category  Or @ItemGrp = Direct  
      ...
      Begin    
        ...
        --SchAbs.ApplicableOn, Convert(nVarchar(10), .CreationTime,103)    
...
End
--Set @TimeLine = @TimeLine + ' , ' +'21-' + convert(nvarchar,getdate()-@StartTime,114) 				
...
Close Cur_6
Deallocate  Cur_6
--Set @TimeLine = @TimeLine + ' , ' +'22-' + convert(nvarchar,getdate()-@StartTime,114) 				
...
--Set @TimeLine = @TimeLine + ' , ' +'23-' + convert(nvarchar,getdate()-@StartTime,114) 				
--Select @TimeLine 

	select WdCode,[WD Code],[WD Dest],[From Date],[To Date],ActivityCode,Description,[Applicable From Date],[Applicable To Date],[RFA From Date],[RFA To Date],Division,Code,Name,UOM,Type,
	SaleQty,SaleValue,PromotedQty,PromotedValue,TaxPercentage,TaxAmount,RebateQty,RebateValue From #TempOutputtable

Outside:
	Drop Index #TmpTSRInvAbs.IDX_C_TmpTSRInvAbs_InvoiceID
	Drop Index #TmpTSRInvDtl.IDX_NonC_TmpTSRInvDtl_InvoiceID
