Create Procedure Sp_GGRRFinalDataPost_RPT(@FirstDate DateTime = NULL,@ToDate DateTime = Null)  
As  
Begin  
  Set DateFormat DMY  
  Declare @Receive Int
  Set @Receive = NULL
  Declare @DummyToDate Datetime  
  Set @DummyToDate=@ToDate  
  Set @ToDate=@FirstDate  
  
  While @ToDate<=@DummyToDate  
  BEGIN  
   ...
   Set @LastdaycloseDate = Convert(Nvarchar(10),@DummyToDate,103)  
   ...
   Create Table #TmpDSRpt (SalesmanID Int)  
     
   Create Table #TmpBeatRpt (BeatID Int)  
     
   Create Table #TmpDSTypeRpt (DSType Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS NULL)  
     
   Create Table #TmpCustomerRpt (  
    SalesmanID Int,  
 ...
 [Status] nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,  
   ...
   Create Table #TmpCustCat(CustomerID Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,SalesmanID Int,CategoryGroup Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,ProdDefnID Int,PMCategory Nvarchar(255) 
  COLLATE SQL_Latin1_General_CP1_CI_AS NULL)  
   ...
   Create Table #Statusval (DSID Int, DSType Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,CustomerID Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,CatGroup Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,Status Nvarchar(255) 
	COLLATE SQL_Latin1_General_CP1_CI_AS,Curstatus Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,Fstatus Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,Actual Decimal(18,6),PMCategory Nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS)  
   ...
   Begin  
    ...
    Else  
    Begin   
     --Truncate Table GGRRFinalDataRPT  
     Delete From GGRRFinalDataRPT Where Fromdate >= '01-04-2014'  
  ...
  --  If Exists(Select 'X' From GGDRDataRPT Where Convert(Nvarchar(10),InvoiceDate,103) = Convert(Nvarchar(10),@ToDate,103))  
  --  Begin     
     If Exists(Select 'X' From GGDROUtlet Where @Todate Between ReportFromdate and ReportTodate And Active = 1 )--And OutletID in (Select Distinct RetailerCode From GGDRDataRPT Where InvoiceDate = @ToDate))  
     Begin  
      Truncate Table #tmbGGRRMonth  
      Insert into #tmbGGRRMonth (MonthName)  
      Select @GGDRmonth  
      ...
      Select Distinct RetailerCode From GGDRDataRPT Where Convert(Nvarchar(10),InvoiceDate,103) = Convert(Nvarchar(10),@ToDate,103)  
     End  
  --  End  
   End  
   ...
   Select Distinct ProdDefnID,Product_Code,MarketSKU,SubCategory,Division,CategoryGroup From TmpGGDRSKUDetails  
  ...
  --  Goto SkipMonth  
   ...
   Truncate Table #TmpCustomerRpt  
   Truncate Table #TmpDSRpt  
   Truncate Table #TmpBeatRpt  
   Truncate Table #TmpOut  
   Truncate Table #TmpDSTypeRpt  
  
   If @DS = '%'  
   Begin  
    Insert Into #TmpDSRpt Select SalesmanID From Salesman  
   End  
   Else  
   Begin  
    Insert Into #TmpDSRpt      
    Select SalesmanID From Salesman Where Salesman_Name In (Select * From dbo.sp_splitin2Rows(@DS,@Delimeter))      
   End  
  
   If @Beat = '%'  
   Begin  
    Insert Into #TmpBeatRpt Select BeatID From Beat  
   End  
   Else  
   Begin  
    Insert Into #TmpBeatRpt      
    Select BeatID From Beat Where Description In (Select * From dbo.sp_splitin2Rows(@Beat,@Delimeter))      
   End  
  
   If @RecdDSType = '%'  
   Begin  
    Insert Into #TmpDSTypeRpt Select Distinct DSTypevalue From DSType_Master  
   End  
   Else  
   Begin  
    Insert Into #TmpDSTypeRpt      
    Select  DSTypevalue From DSType_Master Where DSTypevalue In (Select * From dbo.sp_splitin2Rows(@RecdDSType,@Delimeter))      
   End  
  
   Insert Into #TmpCustomerRpt (SalesManId,BeatID,CustomerID)  
   Select Distinct BS.SalesManId,BS.BeatID,BS.CustomerID From Beat_SalesMan BS, #TmpDSRpt S, #TmpBeatRpt B   
   ...
   Insert Into #TmpCustomerRpt (CustomerID,SalesManId,DSType)  
   Select Distinct RetailerCode,DSID,DSType From GGDRDataRPT   
   Where InvoiceDate Between  @MonthFirstdate And @MonthTodate  
     
  
   Update T Set T.Customername = C.Company_name From Customer C, #TmpCustomerRpt T  
   Where C.CustomerID = T.CustomerID  
  
   Update T Set T.SalesmanName = S.Salesman_Name From SalesMan S, #TmpCustomerRpt T  
   Where S.SalesmanID = T.SalesmanID  
  
   Insert Into #TmpCustomer1 (SalesmanID,SalesmanName,BeatId,DSType,CustomerID,Customername)  
   select Distinct SalesmanID,SalesmanName,isnull(BeatId,0),DSType,CustomerID,Customername from #TmpCustomerRpt  
   Truncate Table #TmpCustomerRpt  
   Insert Into #TmpCustomerRpt (SalesmanID,SalesmanName,BeatId,DSType,CustomerID,Customername)  
   ...
   Insert Into #DSTypeMaster (DSType)  
   Select Distinct DSType From #TmpCustomerRpt Where DSType Not In (Select Distinct DSType From #DSTypeMaster)  
  
   Update #TmpCustomerRpt Set DSType = Null Where DSType Not In (Select Distinct DSType From #DSTypeMaster)  
  
   If @DS <> N'%' And @Beat = N'%'  
   Begin  
    Delete From #TmpCustomerRpt Where Isnull(SalesManID,0) not In (Select Distinct SalesManID From #TmpDSRpt)  
   End  
   Else If @DS <> N'%' And @Beat <> N'%'  
   Begin  
    Delete From #TmpCustomerRpt Where Isnull(SalesManID,0) not In (Select Distinct SalesManID From #TmpDSRpt)  
    And Isnull(BeatID,0) not In (Select Distinct BeatID From #TmpBeatRpt)  
   End   
   Else If @DS = N'%' And @Beat <> N'%'  
   Begin  
    Delete From #TmpCustomerRpt Where Isnull(BeatID,0) not In (Select Distinct BeatID From #TmpBeatRpt)  
   End  
  
   Delete From #TmpCustomerRpt Where DSType Not In (Select Distinct DSType From #TmpDSTypeRpt)  
  
   Update T Set T.Fromdate = G.FromDate,T.Todate = G.Todate From #TmpCustomerRpt T,  
   (Select Distinct G.OutletId,  
   ...
   G.ProdDefnID,  
   G.PMCatGroup  
   from GGDROutlet G, #TmpCustomerRpt C  
  ...
  /* Remove Unwanted Customer For DayClose DataPosting */  
   ...
   Begin  
    Delete From #TmpOut Where ID in (Select Distinct Id From #TmpOut T,GGRRFinalDataRPT G Where G.[Month] = @GGDRmonth  
  ...
  /* Status Validation Start */  
   ...
   Create Table #output (Salesmanid int,DSType nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,CustomerID nvarchar(15) COLLATE SQL_Latin1_General_CP1_CI_AS, CTarget Decimal(18,6),CTargetUOM Int,CsalesValue Decimal(18,6),ProdDefnID int,SDProductCode 
   Nvarchar(256) COLLATE SQL_Latin1_General_CP1_CI_AS,SDProductLevel int,Target decimal(18,6),TargetUOM int,MTDSales decimal(18,6),ProductFlag Nvarchar(256) COLLATE SQL_Latin1_General_CP1_CI_AS,CategoryGroup Nvarchar(256) COLLATE SQL_Latin1_General_CP1_CI_AS,
OutletStatus Nvarchar(256) COLLATE SQL_Latin1_General_CP1_CI_AS,CurrentStatus Nvarchar(256) COLLATE SQL_Latin1_General_CP1_CI_AS,PMCategory nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS)  
   ...
   Create Table #ProductwiseDet (Salesmanid int,DSType nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,CustomerID nvarchar(15) COLLATE SQL_Latin1_General_CP1_CI_AS, ProdDefnID int,SDProductCode nvarchar(256) COLLATE SQL_Latin1_General_CP1_CI_AS,
   SalesValue decimal(18,6),CSalesValue decimal(18,6))  
   ...
   From GGDROutlet G,GGDRDataRPT GD  
   Where   
  ...
  /* Remove Unwanted Customer For Receive & Process DataPosting */  
   ...
   From GGDROutlet G,GGDRDataRPT GD,#HHDS HS,GGDRProduct GP  
   Where   
   G.OutletID = GD.RetailerCode And  
  ...
  -- GD.ProdDefnID = G.ProdDefnID And  
   ...
   From GGDRDataRPT GD,GGDRProduct GP, #output O,#Items I,GGDROutlet G  
   Where   
   G.OutletID = GD.RetailerCode And  
   G.ProdDefnID = GD.ProdDefnID And  
  ...
  /* Transaction Based Salesman dataPosting Validation removed. */  
   ...
   Where isnull(Active,0) = 0 )-- And SalesManId Not In (Select Distinct DSID From GGDRDataRPT Where InvoiceDate BetWeen @MonthFirstdate And @MonthTodate))  
...
/* EG Category Changes Ends */
   ...
   Select Distinct [DetailID],@GGDRmonth [Month],[CustomerID],[Customer Name],[DS ID],[DS Name],[DS Type],'' [Beat],[Status],[Target],[TargetUOM],[Cat GRP],[OCG],(Case When [Actual] <> 0 Then [Actual] Else 0 End) [Actual],[Current Status],
  [Last Day Close Date],[FromDate],[ToDate],[ProdDefnID],Null,'',PMCategory  
  ...
  /*  Loop End */  
    If Isnull(@Receive,0) = 1  
    Begin  
     Delete From GGRRFinalDataRPT Where [Month] = @GGDRmonth And CustomerID in (Select Distinct CustomerID From PendingGGRRFinalDataPost Where FromDateMonth = @GGDRmonth)  
  ...
  /* Detail part Start */  
   ...
   Select Distinct [DetailID],[Month],[CustomerID],[Customer Name],[DS ID],[DS Name],[DS Type],[Beat],[Status],[Target],[TargetUOM],[Cat GRP],[OCG],[Actual],[Current Status],[Last Day Close Date],[FromDate],[ToDate],[ProdDefnID],MaxProdDefnID,
CustomerCategory,PMCategory From #TmpOutAbstract  
   Order By [Month],[CustomerID] Asc  
   Open CUR  
   Fetch from CUR into @D_DetailID,@D_Month,@D_CustomerID,@D_CustomerName,@D_DSID,@D_DSName,@D_DSType,@D_Beat,@D_Status,@D_Target,@D_TargetUOM,@D_CatGRP,@D_OCG,@D_Actual,@D_CurrentStatus,@D_LastDayCloseDate,@D_FromDate,@D_ToDate,@D_ProdDefnID,
@D_MaxProdDefnID,@D_CustomerCategory,@D_PMCategory   
   While @@fetch_status =0  
    Begin  
     Set DateFormat DMY  
     Set @DSTypeId = 0  
    ...
    /* For Inculede Items */  
     ...
     While @@fetch_status =0  
      Begin  
       ...
       from GGDRDataRPT G, #TmpItems TI,#Items I   
     ...
     Insert Into #TmpOutDetail(TProductCode,ProductCode,[Product Description],ProductLevel,Target,TargetUOM,IsExcluded,Actual)  
     Select  Distinct [ProductCode] as [TProductCode],[ProductCode],[Product Description],  
      ...
      isnull([Actual],0) as [Actual] from #D_TmpOut  
  
     Delete From GGRRFinalDataRPT Where [Month] = @D_Month And DSID = @D_DSID and DSType = @D_DSType and [CustomerID] = @D_CustomerID And [ProdDefnID] = @D_ProdDefnID and status= @D_Status and isnull([catGRp],'') = isnull(@D_CatGRP,'') and isnull([OCG],''
) = isnull(@D_OCG,'')  
  
     Insert Into GGRRFinalDataRPT(Month,FromDate,ToDate,CustomerID,CustomerName,DSID,DSName,DSType,Status,Target,TargetUOM,CatGRP,OCG,Actual,CurrentStatus,DetailID,ProdDefnID,D_ProductCode,D_ProductDescription,D_ProductLevel,D_Target,D_TargetUOM,D_IsExcluded,D_Actual,LastDayCloseDate,CreationDate,MaxProdDefnID,CustomerCategory,PMCategory)  
     Select Distinct @D_Month,@D_FromDate,@D_ToDate,@D_CustomerID,@D_CustomerName,@D_DSID,@D_DSName,@D_DSType,@D_Status,@D_Target,@D_TargetUOM,@D_CatGRP,@D_OCG,@D_Actual,@D_CurrentStatus,@D_DetailID,@D_ProdDefnID,[ProductCode],[Product Description],[ProductLevel],[Target],[TargetUOM],[IsExcluded],[Actual],@D_LastDayCloseDate,Getdate(),@D_MaxProdDefnID,@D_CustomerCategory,@D_PMCategory From #TmpOutDetail  
  --NextID:  
     Fetch Next from CUR into @D_DetailID,@D_Month,@D_CustomerID,@D_CustomerName,@D_DSID,@D_DSName,@D_DSType,@D_Beat,@D_Status,@D_Target,@D_TargetUOM,@D_CatGRP,@D_OCG,@D_Actual,@D_CurrentStatus,@D_LastDayCloseDate,@D_FromDate,@D_ToDate,@D_ProdDefnID,@D_MaxProdDefnID,@D_CustomerCategory,@D_PMCategory   
    End  
   Close CUR  
   Deallocate CUR  
  ...
  /* Detail part End */  
   ...
   Update GGRRFinalDataRPT Set LastdaycloseDate = Convert(Nvarchar(10),@LastdaycloseDate,103)   
  
   If @ToDate IS NOT NULL  
   Begin  
    Update GGRRFinalDataRPT Set ToDate = @Todate Where [Month] = @GGDRmonth  
   End  
   ...
   Drop Table #TmpCustomerRpt  
   Drop Table #TmpDSRpt  
   Drop Table #TmpBeatRpt  
   Drop Table #TmpOut  
   Drop Table #TmpDSTypeRpt  
