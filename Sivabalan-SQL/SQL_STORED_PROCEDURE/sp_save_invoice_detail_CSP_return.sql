CREATE Procedure sp_save_invoice_detail_CSP_return(@INVOICE_ID int,    
...
BEGIN    
SET @RETVAL = 1    
IF @DAMAGES = 0 --Sales Return Saleable   
	BEGIN
...
ELSE    
	...
	IF @PURCHASEPRICE = 0    
	    BEGIN    
		Select @PURCHASEPRICE = PurchasePrice From Batch_Products Where Product_Code = @ITEM_CODE And PurchasePrice > 0    
		Set @PURCHASEPRICE = IsNull(@PURCHASEPRICE, 0)    
	    END     
	
	INSERT INTO Batch_Products(Product_Code, Batch_Number, Expiry, Quantity,     
	PurchasePrice, SalePrice, PTS, PTR, ECP, Company_Price, Damage,     
	DamagesReason, PKD, TaxSuffered, DocType, DocID) VALUES(@ITEM_CODE, @BATCH_NUMBER, @EXPIRY,     
	@REQUIRED_QUANTITY, @PURCHASEPRICE, @ECP, @PTS, @PTR, @ECP,     
	@SPECIAL_PRICE, 2, @REASON_ID, @PKD_DATE, @TAXSUFFERED_ORIG, 1, @INVOICE_ID)    
