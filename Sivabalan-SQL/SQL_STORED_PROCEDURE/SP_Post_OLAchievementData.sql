Create Procedure SP_Post_OLAchievementData
AS
BEGIN
	Set Dateformat DMY
	...
	--Select @DayCloseFromDate = Min(dbo.StripDateFromTime(DayCloseDate)) From DayCloseTracker Where isnull(Status,0) = 0 and Module = 'OLTAchievement'
	--Select @DayCloseToDate = Max(dbo.StripDateFromTime(DayCloseDate)) From DayCloseTracker Where isnull(Status,0) = 0 and Module = 'OLTAchievement'

	Select @FromDate = Convert(DateTime, '01' +  right(convert(varchar(11), dbo.Striptimefromdate(GetDate()), 103), 8))
	--Select @ToDate = dbo.stripdatefromtime(GetDate())
	...
	Insert into #CustomerList(CustomerID) select Distinct OutletID from PMOutlet where PMID in (select Distinct PMID from tbl_merp_PMMaster 
	where Period = @PMMonth and isnull(active,0)=1)
	
	/* To post data in Temp Table from Invoice */
	Insert into #OLAchievement (InvoiceID,InvoiceDate,CustomerID,DSID, DStypeID, GroupID,ItemCode,SalesValue)
	Select distinct ID.InvoiceID,IA.InvoiceDate,IA.CustomerID,IA.SalesmanID,IA.DStypeID,ID.GroupID,ID.Product_code,Case When Max(IA.InvoiceType) in (4,5) then -Sum(ID.Amount) else Sum(ID.Amount) end From InvoiceAbstract IA,InvoiceDetail ID
	...
	Truncate table OLTargetAchievement
...
--							Where a.DSTypeID = b.DSTypeID and a.DSTypeCtlPos = 2 and a.DSTypeValue = 'Yes')
		

	/* To get Target value from PMoutlet */
	Insert Into #TmpOutput (DSID, CustomerID, PMProductName, PMCatGrp, Target, PMID, DSTypeID)
	Select Tmp.DSID, Tmp.CustomerID, Tmp.PMProductName, Tmp.PMCatGrp, Sum(IsNull(PMO.Target, 0)) as Target, Tmp.PMID, Tmp.DSTypeID
	From #TmpDS Tmp, PMOutlet PMO
	...
	Begin
		...
		If @ProductLevel = 0
		Begin
			Insert Into #TmpSales(DSTypeID, DSID, CustomerID, PMProductName, PMCatGrp, PMID, SaleValue)
			Select OLA.DSTypeID, OLA.DSID, OLA.CustomerID, @PMProductName, @PGCatGrp, @PMID, Sum(IsNull(SalesValue, 0))
			...
			Group By OLA.DSTypeID, DSID, CustomerID
		End				
	...
	Update T Set Achievement = Sa.SalesValue
	From #TmpOutput T, (Select DSID, DSTypeID, CustomerID, PMProductName, PMCatGrp, PMID, Sum(SaleValue) as SalesValue From #TmpSales 
	...
	Insert Into OLTargetAchievement(DSTypeID, DSID, CustomerID, PMProductName, PMCatGrp, Target, Achievement, PMID)
	...
	/* For Reprocessing: If PMOutlet alone is received and PM is not received for a month then Dataposting should not happen */
--	If (select count(*) From #CustomerList)>0
--	BEGIN
--		Update DayCloseTracker Set Status = 1 Where dbo.StripDateFromTime(DayCloseDate) Between @DayCloseFromDate and @DayCloseToDate and Module = 'OLTAchievement'
--	END

	Update DayCloseModules Set DayCloseDate = @Todate Where Module = 'Outlet Level Target'
