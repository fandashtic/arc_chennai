Create Procedure sp_save_vanstatementdetail_FMCG_MUOM(@Document_ID int,  
   ...
   if @TrnType<>3   		  --Godown to van
	   begin
		...
		begin     	
			 ...
			 @REQUIRED_QUANTITY, @SALE_PRICE, @SALE_PRICE * @REQUIRED_QUANTITY, @BFQTY, @COST, @UOM, @UOMQTY, @UOMPRICE, @REQUIRED_QUANTITY, @DocSerial, @Serial)  
		end
 ...
 END  
    if @TrnType<>3   		  --Godown to van
		...
		begin     	
		     ...
		     Batch_Number, Quantity, Pending, SalePrice, Amount, BFQty, PurchasePrice,UOM, UOMQty, UOMPrice,TransferQty,VanTransferID,TransferItemSerial)   
		     VALUES (@Document_ID, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER,   
		     @REQUIRED_QUANTITY, @PENDING_QTY, @SALE_PRICE, @TOTAL_AMOUNT, @BFQTY, @COST, @UOM, @UOMQTY, @UOMPRICE,@REQUIRED_QUANTITY, @DocSerial, @Serial)  
		end
        GOTO OVERNOUT  
 END  
    ELSE  
