CREATE Procedure sp_save_invoice_detail_CSP_return_FMCG_MUOM(@INVOICE_ID int,  
...
BEGIN  
	...
	IF @DAMAGES = 0  
   	...
   	INSERT INTO InvoiceDetail(InvoiceID, Product_Code, Batch_Code, Batch_Number,   
		...
		@CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @REASON_ID, @UOM, @UOMQty, @UOMPrice)  
		END  
	ELSE  
		BEGIN  
		IF @COST = 0  
...
IF @SCHEMEID <> 0  
BEGIN  
	Insert Into SchemeSale(Product_Code, Quantity, Free, Value, Cost, Type, InvoiceID, Claimed, Pending) Values(@ITEM_CODE, @PRIMARY_QUANTITY, @ORIGINAL_QTY,   
	@MODIFIED_PRICE * @ORIGINAL_QTY, @SCHEME_COST, @SCHEMEID, @INVOICE_ID, 0, @ORIGINAL_QTY)  
END  
SELECT @RETVAL  
