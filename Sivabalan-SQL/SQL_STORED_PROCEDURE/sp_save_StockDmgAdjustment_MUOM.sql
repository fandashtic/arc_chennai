CREATE Procedure sp_save_StockDmgAdjustment_MUOM(@STOCKID INT,
...
@PTS1 decimal(18,6)=0, @PTR1 decimal(18,6)=0, @ECP1 decimal(18,6)=0, @PKD1 DATETIME = NULL,
@TAXSUFFERED1 decimal(18,6)=0, @APPLICABLEON1 int=0,
@PARTOFF1 decimal(18,6)=0,
@TaxType Int= 1, @MRPPERPACK1 decimal(18,6)=0,
@nUOM int,@UOMQty decimal(18,6), @UOMPrice decimal(18,6) )
AS
...
Declare @TotalVal Decimal(18,6)

Select @AdjDate=AdjustmentDate from StockAdjustmentAbstract where AdjustmentID=@STOCKID

Select @PriceOption = Price_Option FROM ItemCategories, Items WHERE ItemCategories.CategoryID = Items.CategoryID AND Items.Product_Code = @ITEM_CODE
SET @ORIGINAL_CODE = 0


/* While Doing StockAdjustment Others When a new item is added and new batch is added through F9 option
then the below procedure updates the openingdetails for the newly added item */
...
AND (IsNull(Expiry,'') = IsNull(@EXPIRY_DATE,''))
...
AND IsNull(PTS, 0) = @PTS1 AND IsNull(PTR, 0) = @PTR1 AND IsNull(ECP, 0) = @ECP1 And IsNull(MRPPerPack, 0) = @MRPPERPACK1
-- AND (PKD = @PKD1 OR PKD IS NULL)
AND IsNull(PKD,'') = IsNull(@PKD1,'')
AND IsNull(TAXSUFFERED, 0) = @TAXSUFFERED1
AND IsNull(APPLICABLEON, 0) = @APPLICABLEON1 AND IsNull(PARTOFPERCENTAGE, 0) = @PARTOFF1
And IsNull(TaxType,1) =  @TaxType
ORDER BY Batch_Code
Else
DECLARE GetStocks CURSOR KEYSET FOR
SELECT Batch_Number,  Batch_Code , Quantity, IsNull(Damage,0) FROM Batch_Products
...
And IsNull(TaxType,1) =  @TaxType
ORDER BY Batch_Code
Else --If StkAdj Others or Damages
IF @PriceOption = 1
DECLARE GetStocks CURSOR KEYSET FOR
SELECT Batch_Number,  Batch_Code , Quantity, IsNull(Damage,0) FROM Batch_Products
WHERE Product_Code = @ITEM_CODE AND isnull(Batch_Number, N'') = @BATCH_NUMBER
...
UPDATE Batch_Products SET PurchasePrice = Case When @NEW_QTY = 0 And @PRICE = 0 Then PurchasePrice Else @PRICE End, Quantity = Quantity + @DIFF
...
BEGIN
SELECT @EXPIRY = Expiry, @PTS = PTS, @PTR = PTR, @ECP = ECP, @PKD_DATE = PKD, @GRN_ID = GRN_ID, @TAXSUFFERED = TaxSuffered,
...
@MRPPERPACK = IsNull(MRPPerPack, 0) , @PFM = isnull(PFM,0)
FROM Batch_Products WHERE Batch_Code = @ORIGINAL_CODE
SELECT @UOM = UOM From Items Where Product_Code = @ITEM_CODE

SELECT @TOQ = IsNull(TOQ,0), @GSTTaxType = GSTTaxType From  Batch_Products  where Batch_Code = @BATCH_CODE

INSERT INTO Batch_Products(Batch_Number, Expiry, Product_Code, Quantity, PurchasePrice,
SalePrice, PTS, PTR, ECP, Damage, DamagesReason, PKD, GRN_ID, TaxSuffered, UOM,
UOMQty, UOMPrice, Free, GRNTaxSuffered, GRNTaxID, GRNApplicableOn, GRNPartOff, ApplicableOn, PartofPercentage, Vat_Locality, DocType,
DocId, TaxType,DocDate,QuantityReceived,MRPPerPack,TOQ,GSTTaxType,PFM)
Values(@Batch_Number, @EXPIRY, @ITEM_CODE, ABS(@ADJUSTMENT_QTY), @ADJUSTMENT_PRICE,
@SALE_PRICE, @PTS, @PTR, @ECP, 1, @REASON_ID, @PKD_DATE, @GRN_ID, @TAXSUFFERED, @UOM,
ABS(@ADJUSTMENT_QTY), @ADJUSTMENT_PRICE, @Free, @GRNTaxSuffered, @GRNTaxId, @GRNApplicableOn, @GRNPartOff, @ApplicableOn, @PartOff, @Vat_Locality, 2,
@STOCKID, @TaxType,@AdjDate,ABS(@ADJUSTMENT_QTY),@MRPPERPACK,@TOQ,@GSTTaxType,@PFM)

INSERT INTO StockAdjustment (SerialNO, Product_Code, Batch_Code, Batch_Number,
Quantity, Rate, ReasonID, OldQty, OldValue, UOM, UOMQty, UOMPrice) VALUES (@STOCKID, @ITEM_CODE, @@IDENTITY,
@BATCH_NUMBER, ABS(@ADJUSTMENT_QTY), ABS(@ADJUSTMENT_QTY) * @ADJUSTMENT_PRICE, @REASON_ID,
@QUANTITY, @QUANTITY * @OLDPRICE,@nUOM ,@UOMQty ,@UOMPrice )

Select @TotalVal = SUM(Rate) from StockAdjustment Where SerialNO =  @STOCKID
Update  StockAdjustmentAbstract set AdjustmentValue =  @TotalVal Where AdjustmentID =@STOCKID
END
CLOSE GetStocks
...
Exec Sp_Update_Opening_TaxSuffered_Percentage @OpeningDate, @Item_Code, @ORIGINAL_CODE
End
