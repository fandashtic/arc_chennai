Create Procedure dbo.mERP_spr_DailyGGRRDetail(@Dtldata Nvarchar(Max),@AsonDate Datetime,@Dummy1 Nvarchar(255),@Dummy2 Nvarchar(255),@Dummy3 Nvarchar(255))
As
Begin
	Set DateFormat DMY
	
--	Declare @GGDRmonth Nvarchar(10)
--	select * into #GGDRData from GGDRData where 1=2
--	If @AsOnDate > (Select isnull(LastinventoryUpload,getdate()) from Setup)
--	BEGIN
--		Select @GGDRmonth=CAST(DATENAME(month,@AsOnDate) as nvarchar(3)) + '-' + Right(Year(@AsOnDate),4)
--		Insert into #GGDRData Select * From GGDRData Where Month(InvoiceDate)=Month(@AsOnDate) and Year(InvoiceDate)=Year(@AsOnDate)
--	END
--	Declare @FirstDate Datetime
--	Select @FirstDate = dateadd(d,1,isnull(LastinventoryUpload,getdate())) from Setup
--	exec mERP_SP_initGGDRDataposting_RPT @FirstDate,@AsonDate
