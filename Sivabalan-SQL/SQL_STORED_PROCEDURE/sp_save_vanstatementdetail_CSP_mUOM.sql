Create Procedure sp_save_vanstatementdetail_CSP_mUOM(@DOCUMENT_ID int,
		...
		begin
			Insert into VanTransferDetail
					(DocSerial, Product_Code, BatchCode, BatchNumber,
					 Quantity, Saleprice, Value,Serial,UOMID,UOMQty,UOMPrice,MRPPerPack)
			Values(
					@DocSerial, @ITEM_CODE, 0, @BATCH_NUMBER, 
					@REQUIRED_QUANTITY, @PTR,@TOTAL_AMOUNT,@Serial,@UOM,@UOMQTY,@UOMPRICE,@MRPPerPack)
		end
		if @TrnType<>2
		begin     	
			...
			SpecialPrice, UOM, UOMQty, UOMPrice,TransferQty,VanTransferID,TransferItemSerial, MultipleSchemeID, MRPPerPack)
			VALUES (@DOCUMENT_ID, @ITEM_CODE, 0, @BATCH_NUMBER, @REQUIRED_QUANTITY, 
			@PENDING_QTY, @SALE_PRICE, @TOTAL_AMOUNT, @BFQTY, @COST, @PTS, @PTR, @ECP,
			@SPECIALPRICE,@UOM, @UOMQTY, @UOMPRICE,@REQUIRED_QUANTITY, @DocSerial, @Serial, @MultiSchID, @MRPPerPack)
		end
...
BEGIN
	if (@TrnType=1 Or @TrnType=2)
		...
		WHERE Product_Code = @ITEM_CODE AND ISNULL(Batch_Number, N'') = @BATCH_NUMBER 
		AND ISNULL(PTR, 0) = @ORIG_PRICE AND (Expiry >= GetDate() OR Expiry IS NULL) 
	...
	if (@TrnType = 1 or @TrnType=2)    --Cursor for VanstatemntDetail
		...
		WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, N'') = @BATCH_NUMBER 
		and ISNULL(PTR, 0) = @ORIG_PRICE AND ISNULL(Quantity, 0) > 0 
...
ELSE
	...
	if (@TrnType=1 Or @TrnType=2)
		SELECT @TOTAL_QUANTITY = ISNULL(SUM(Pending), 0) FROM VanstatementDetail
		WHERE Product_Code = @ITEM_CODE AND DocSerial=@FromVan AND ISNULL(PTR, 0) = @ORIG_PRICE 
		and isnull(MRPPerPack, 0) = @BatchMRPPerPack
	Else
		SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products 
		WHERE Product_Code = @ITEM_CODE AND ISNULL(PTR, 0) = @ORIG_PRICE 
		And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow
		and isnull(MRPPerPack, 0) = @BatchMRPPerPack
		...
		begin
			Insert into VanTransferDetail
					(DocSerial, Product_Code, BatchCode, BatchNumber,
					 Quantity, Saleprice, Value,Serial,UOMID,UOMQty,UOMPrice,MRPPerPack)
			Values(
					@DocSerial, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER,@QUANTITY,@PTR,@TOTAL_AMOUNT,@Serial,@UOM,@UOMQTY,@UOMPRICE,@MRPPerPack) 
--					@REQUIRED_QUANTITY, 
 		end
	if @TrnType<>2
		begin
			...
			Quantity, Pending, SalePrice, Amount, BFQty, PurchasePrice, PTS, PTR, ECP,
			SpecialPrice , UOM, UOMQty, UOMPrice,TransferQty,VanTransferID,TransferItemSerial, MultipleSchemeID, MRPPerPack) 
			VALUES (@DOCUMENT_ID, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER, @QUANTITY, 
			@PENDING_QTY, @SALE_PRICE, @TOTAL_AMOUNT, @BFQTY, @COST, @PTS, @PTR, @ECP, 
			@SPECIALPRICE ,@UOM, @UOMQTY, @UOMPRICE,@QUANTITY, @DocSerial, @Serial, @MultiSchID, @MRPPerPack)
		end	
