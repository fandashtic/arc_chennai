CREATE PROCEDURE spr_top_customers_by_brand(@BrandID int,    
         @FROMDATE datetime,    
...
INSERT INTO #TopCustomers SELECT Cast(CustomerID As nVarchar),     
--"Customer" =     
...
WHERE ISNULL((select SUM(Amount) from InvoiceAbstract, InvoiceDetail, Items    
  ...
  CustomerID = Customer.CustomerID AND     
  InvoiceDate BETWEEN @FROMDATE AND @TODATE AND     
  InvoiceType in(5,6) AND IsNull(CustomerID,N'') <> N'' AND (Status & 128) = 0), 0) <> 0    
...
SELECT Top 25 * FROM #TopCustomers ORDER BY TotalSales    
