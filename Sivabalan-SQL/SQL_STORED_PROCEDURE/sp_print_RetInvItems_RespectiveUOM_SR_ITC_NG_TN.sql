Create Procedure [dbo].[sp_print_RetInvItems_RespectiveUOM_SR_ITC_NG_TN](@INVNO INT,@MODE INT = 0)
AS
Begin
...
End,
"TaxDetails" = dbo.GetTaxCompInfo(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code,Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.STPayable)),
"TaxDetailsWithBreakup" = dbo.GetTaxCompInfoWithBreakup(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code,Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.STPayable)),
...
InvoiceDetail.UOM, Items.Soldas,InvoiceAbstract.InvoiceType,InvoiceDetail.Batch_Number,Batch_Products.PKD,Batch_Products.Expiry 
Order By serial
...
-- Below changes done to display sales Item first then salesreturn Items 
--Select * From #TmpInvDet Order By serial
select * into #t2 from #TmpInvDet where  1=2

SET IDENTITY_INSERT #t2 ON
ALTER TABLE #t2 ALTER COLUMN [Item MRP] nVarchar(100)
insert into #t2 (id1,Description) values (@srl +1,'**S A L E S - R E T U R N')
SET IDENTITY_INSERT #t2 OFF

if exists (select top 1 * from #tmpInvDet where quantity <0)
	Begin
	--selecting only invoice items
	select * from #tmpInvdet where quantity >0 or free > 0
	Union
	--To Print Sales - Return text
	select * from #t2
	Union
	--Selecting sales return items
	select * from #tmpInvdet where quantity <0	
	End
else
	--select * from #tmpInvdet 
   Begin
		IF @MODE = 0
			select * from #tmpInvdet 
		ELSE
