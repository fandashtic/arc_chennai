CREATE Procedure sp_acc_gj_stocktransferinCancel (@DocSerial INT,@BackDate DATETIME=Null)
AS
...
Set @VAT_Receivable  = 115  /* Constant to store the VAT Receivable (Output Tax) AccountID*/     
Set @Vat_Exists = 0 
...
Begin
	...
	Begin
		...
		execute sp_acc_insertGJ @TransactionID,@AccountID,@DocumentDate,@NetValue,0,@DocSerial,@AccountType,"Stock Transfer In Cancellation",@DocumentNumber
	...
	Begin
		...
		If Isnull(@FLSTTaxType ,0) in (3,4)
			...
			End
		execute sp_acc_insertGJ @TransactionID,@AccountID1,@DocumentDate,0,@PurchaseValue,@DocSerial,@AccountType,"Stock Transfer In Cancellation",@DocumentNumber
		Insert Into #TempBackdatedAccounts(AccountID) Values(@AccountID1)
	End
	If @TaxAmount<>0
	Begin
		-- Entry for TaxonStocktransfer Account
		execute sp_acc_insertGJ @TransactionID,@AccountID2,@DocumentDate,0,@TaxAmount,@DocSerial,@AccountType,"Stock Transfer In Cancellation",@DocumentNumber
	...
	Begin
		...
		if @TaxType = 2 
		Begin
			execute sp_acc_insertGJ @TransactionID,@PurchaseTax,@DocumentDate,0,@nVatTaxamount,@DocSerial,@AccountType,"Stock Transfer In Cancellation",@DocumentNumber
		...
		Begin
			If Isnull(@FLSTTaxType ,0) Not in(3,4)
				Begin
					execute sp_acc_insertGJ @TransactionID,@VAT_Receivable,@DocumentDate,0,@nVatTaxamount,@DocSerial,@AccountType,"Stock Transfer In Cancellation",@DocumentNumber
