CREATE procedure sp_acc_prn_groupwiseledgerdetail_all(@fromdate datetime,@todate datetime ,@parentid  integer, @DocRef integer,@doctype integer,@mode integer,@Info nvarchar(4000) = Null,@State Int=0,@Hide0BalAC Int =0)
as        
...
-- -- 	if (select count(1) from #AllGroupwiseledgerdetail) > 0 
-- -- 	Begin
		Select 'Date'=TransactionDate,'Transaction ID'=OriginalID,          
		'Description'=Type,'Particular'=AccountName,'AccountID'=AccountID,Fromdate,Todate,          
		'DocRef'= Docref,'DocType'=DocType,ColorInfoParam,'Particular'=AccountName,'Debit'=Debit,
		'Credit'=Credit,'Balance'=Balance,'Narration' = Narration,'High Light'=HighLight 
		from #AllGroupwiseledgerdetail
		-- Select * from #AllGroupwiseledgerdetail
-- -- 	End
