Create Procedure mERP_sp_FindSplCatSchemeID_New(@InvoiceDate Datetime,@OutletID nVarchar(500),    
...
-- Set @CreationDate = Cast(@CreationDate as Datetime)    
 ...
 Begin    
  ...
  Begin    
    ...
    insert into #t1 Select distinct SchemeID from #tmpCurScheme  
	
	
    Insert into #tmpPrdtScope
	Select SCPD.SchemeID, SCPD.Product_Code 
	from SchemeProducts  SCPD, #t1,
	tbl_mERP_SchemeProductScopeMap SPSM Where SCPD.Product_Code in (select Product_Code From #tmpInvProducts)
	And SCPD.SchemeID = #t1.Schemeid And  SPSM.SchemeID = #t1.Schemeid 
	And SCPD.ProductScopeID = SPSM.ProductScopeID And SCPD.ACtive = 1
