CREATE procedure [dbo].[spr_list_stock_ledger_by_items_fmcg_pidilite](@FROM_DATE datetime, 
...
-- "Reporting UOM" = 
--    SubString(
--    CAST(CAST(ISNULL(OpeningDetails.Opening_Quantity, 0) / (CASE Items.ReportingUnit WHEN 0 THEN 1 ELSE Items.ReportingUnit END) AS Decimal(18,6)) AS nvarchar), 1, 
--    CharIndex('.', CAST(CAST(ISNULL(OpeningDetails.Opening_Quantity, 0) / (CASE Items.ReportingUnit WHEN 0 THEN 1 ELSE Items.ReportingUnit END) AS Decimal(18,6)) AS nvarchar)) -1)
--   + '.' + 
--   CAST(Cast(ISNULL(OpeningDetails.Opening_Quantity, 0) As Int) % Cast((CASE Items.ReportingUnit WHEN 0 THEN 1 ELSE Items.ReportingUnit END) As Int) AS nvarchar),
--  + ' ' + @ReportingUOM,

--  "Reporting UOM" = dbo.sp_Get_ReportingUOMQty(Items.product_Code, OpeningDetails.Opening_Quantity), 
  "Reporting UOM" = ISNULL(OpeningDetails.Opening_Quantity, 0) / (CASE Items.ReportingUnit WHEN 0 THEN 1 ELSE Items.ReportingUnit END),
...
-- 	 isnull((Isnull(openingdetails.Damage_Opening_Quantity, 0) * Isnull(Items.Company_Price, 0)), 0)        
 ...
 WHERE          
  ...
  and Items.Active = 1 --HAVING ISNULL(SUM(Opening_Quantity), 0) > 0
 END        
 ELSE      
 BEGIN      
 Select          
...
--  "Reporting UOM" = dbo.sp_Get_ReportingUOMQty(I1.product_Code, SUM(Quantity)), 
  "Reporting UOM" = Sum(Quantity) / (CASE I1.ReportingUnit WHEN 0 THEN 1 ELSE I1.ReportingUnit END),
