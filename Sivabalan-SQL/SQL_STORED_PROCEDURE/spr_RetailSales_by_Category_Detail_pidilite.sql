CREATE procedure [dbo].[spr_RetailSales_by_Category_Detail_pidilite] (@InvID nVarchar(512))              
AS              
...
begin      
 ...
 When InvoiceAbstract.InvoiceType>=4 and InvoiceAbstract.InvoiceType<=6 Then 0 - InvoiceDetail.Quantity         
 Else InvoiceDetail.Quantity           
 End)/ Case IsNull(ReportingUnit, 1) When 0 Then 1 Else IsNull(ReportingUnit, 0) End),  
 "Conversion Factor" =  sum((Case         
 When InvoiceAbstract.InvoiceType>=4 and InvoiceAbstract.InvoiceType<=6 Then 0 - InvoiceDetail.Quantity         
 Else InvoiceDetail.Quantity           
 End)* IsNull(ConversionFactor, 0)),  
...
--  End) / (Case IsNull(Items.ReportingUnit, 0) When 0 Then 1 Else Items.ReportingUnit End),      
 "UOM Description" = IsNull((Select [Description] From UOM Where UOM = Items.UOM) , ''),      
 ...
 * IsNull(InvoiceDetail.SalePrice,0)),    
 "In KG" = sum(Case When InvoiceAbstract.InvoiceType>=4 and InvoiceAbstract.InvoiceType<=6 Then 0 - InvoiceDetail.Quantity  / (Case isnull(Items.reportingunit, 1)       
           When 0 Then 1 Else isnull(Items.reportingunit, 1) End)  Else InvoiceDetail.Quantity  / (Case isnull(Items.reportingunit, 1)     
     When 0 Then 1 Else isnull(Items.reportingunit, 1) End) End)    
    
-- "In KG" = dbo.sp_Get_ReportingUOMQty(InvoiceDetail.Product_Code, sum(Case     
--  When InvoiceAbstract.InvoiceType>=4 and InvoiceAbstract.InvoiceType<=6 Then 0 - InvoiceDetail.Quantity         
--  Else InvoiceDetail.Quantity           
--  End)),      
 ...
 GROUP BY ItemCategories.Category_Name, InvoiceDetail.Product_Code, Items.ProductName, InvoiceDetail.Batch_Number, Batch_Products.PKD,           
 Batch_Products.Expiry, Manufacturer.Manufacturer_Name, InvoiceDetail.SalePrice,      
 Items.ReportingUnit, Items.ReportingUOM, Items.UOM
end      
else       
begin      
 SELECT InvoiceDetail.Product_Code,   
