Create procedure spr_Vat_Disallowed_Report ( @FromDate Datetime, @ToDate Datetime, 
...
--update #tmpSales set [Disallowance Type]=T.DiscType from
--(Select IA.InvoiceID,Case When IA.AdditionalDiscount > 0 and sum(ID.DiscountPercentage) = 0 and IA.DiscountPercentage = 0
--then 'Trade Discount' else #tmpSales.[Disallowance Type] end as 'DiscType'
--from InvoiceAbstract IA,InvoiceDetail ID,#tmpSales
--Where IA.InvoiceID = ID.InvoiceID
--And #tmpSales.product_code=ID.Product_Code
--And #tmpSales.InvoiceID=IA.InvoiceID
--group by IA.invoiceid,#tmpSales.[Disallowance Type],IA.AdditionalDiscount,IA.DiscountPercentage
--) T
--Where T.InvoiceId=#tmpSales.Invoiceid
