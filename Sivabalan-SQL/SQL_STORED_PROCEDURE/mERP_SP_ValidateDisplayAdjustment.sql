Create Procedure mERP_SP_ValidateDisplayAdjustment (@TransactionType int,@DocumentID int)
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			/*If Display Scheme Credit Note is adjusted in a Collection and 
			...
			DocumentID in (Select CreditID from CreditNote where isnull(flag,0)=1 and PayoutID in
			(Select ID from tbl_merp_schemepayoutperiod where isnull(claimRFA,0)=1)))
			BEGIN
				/*Dont Allow*/
				Select 0
			END
		...
		BEGIN
			/*If Display Scheme Credit Note is adjusted in a INVOICEWISE Collection and 
		...
		BEGIN
			/*If Display Scheme Credit Note is adjusted in a MANUAL JOURNAL and 
			RFA is submitted then Dont allow MANUAL JOURNAL to Amend or Cancel*/
			if exists(Select * from Generaljournal GJ  
			where GJ.DocumentReference in(Select CreditID from CreditNote where isnull(flag,0)=1 and PayoutID in
			(Select ID from tbl_merp_schemepayoutperiod where isnull(claimRFA,0)=1))
