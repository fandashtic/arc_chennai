CREATE procedure Spr_list_SKULedger_Detail_GSK(@ItemCode nvarchar(400),@FromDate Datetime,@ToDate DateTime,@ItemCode1 nvarchar(400))  
as 
Begin
...
sum(ISNULL((IsNull(QuantityReceived, 0) + IsNull(FreeQty, 0) - IsNull(QuantityRejected, 0)),0)) From   
	...
	Left Outer Join Vendors V On ga.VendorID = V.VendorID Where dbo.StripDateFromTime(GRNDate) =@FromDate
    And Product_Code =@itemcode And IsNull(GRNStatus, 0) & 96 = 0 
...
Where DBO.StripDateFromTime(AdjustmentDate)=@FromDate
...
Left Outer Join  Customer C On inv.CustomerID =C.CustomerID Where Product_Code = @itemcode And dbo.StripDateFromTime(InvoiceDate) =@FromDate
...
Left Outer Join Customer C On da.CustomerID=C.CustomerID Where   
