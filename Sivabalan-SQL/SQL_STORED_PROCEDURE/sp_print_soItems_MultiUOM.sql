CREATE PROCEDURE [dbo].[sp_print_soItems_MultiUOM] (@SONumber int)  
AS  
SELECT "Item Code" = SODetail.Product_Code, "Item Name" = ProductName, 
"UOM2Quantity" = dbo.GetFirstLevelUOMQty(SODetail.Product_Code, Sum(SODetail.Quantity)),
"UOM2Description" = (Select UOM.Description from UOM Where UOM.UOM in( Select UOM2 from Items Where Items.Product_Code =  SODetail.Product_Code )),
"UOM1Quantity" = dbo.GetSecondLevelUOMQty(SODetail.Product_Code, Sum(SODetail.Quantity)),
"UOM1Description" = (Select UOM.Description from UOM Where UOM.UOM in( Select UOM1 from Items Where Items.Product_Code =  SODetail.Product_Code )),
"UOMQuantity" = dbo.GetLastLevelUOMQty(SODetail.Product_Code, Sum(SODetail.Quantity)),
"UOMDescription" = (Select UOM.Description from UOM Where UOM.UOM in( Select UOM from Items Where Items.Product_Code =  SODetail.Product_Code )),  

"Sale Price" = SalePrice,   
"Tax Applicable%" = Sum(ISNULL(SaleTax, 0) + ISNULL(TaxCode2, 0)),  
"Discount" = Discount, "UOM" = UOM1.Description, "Pending" = Sum(Pending),   
...
WHERE 
	SODetail.SONumber = @SONumber  
Group by SODetail.Product_Code, ProductName, SalePrice, Discount, UOM1.Description,   
Manufacturer.ManufacturerCode, Manufacturer.Manufacturer_Name, Brand.BrandName, ItemCategories.Category_Name,  
ConversionTable.ConversionUnit, Items.ConversionFactor,  RUOM.Description, Items.Description, TaxOnMRP
