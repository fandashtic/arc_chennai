CREATE Procedure sp_save_invoice_detail_CSP_van_pharmaMUOM_ITC 
(
	@INVOICE_ID int,
	...
	@GroupID int = 0,
	@BatchMRPPerPack Decimal(18,6) = 0,
	@TaxOnQty int = 0,
    @GSTTaxID int = 0,
    @GSTFlag int = 0,
    @GSTCSTaxCode int = 0,
...
SELECT @COST = Purchase_Price, @MRP = MRP, @SALEID = SaleID, @PTS = PTS, @PTR = PTR , @HSNNumber = isnull(HSNNumber,''), @CategorizationID = isnull(CategorizationID,0)
...
BEGIN
	...
	TaxSuffered, TaxSuffered2, PTS, PTR, UOM, UOMQTY, UOMPRICE, OtherCG_Item,QuotationID,MultipleSchemeID,TotSchemeAmount,MultipleSchemeDetails
	...
	@TAXSUFFERED2, @PTS, @PTR, @UOM, @UOMQTY, @UOMPRICE, @OtherCG_Item,@QuotationID, @MultiSchID, @TotSchAmount,@MultiSchemeDetail
	,@MultipleRebateID,@RebateRate,@MultipleRebateDet,@GroupID,@MRPPerPack,@TaxOnQty,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)
	GOTO ALL_SAID_AND_DONE
END
IF @TRACK_BATCHES = 1
	...
	IF @CUSTOMERTYPE = 1
		...
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		...
		VanStatementDetail.PTS, VanStatementDetail.PTR, VanStatementDetail.ECP
		...
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		END
	ELSE IF @CUSTOMERTYPE = 2
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.PTR, 0) = @GenericPTR And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack

		DECLARE ReleaseStocks CURSOR KEYSET FOR
		SELECT VanStatementDetail.Batch_Number, VanStatementDetail.Batch_Code, VanStatementDetail.Pending, 
		VanStatementDetail.PurchasePrice, [ID], VanStatementDetail.PTS, VanStatementDetail.PTR, VanStatementDetail.ECP 
		FROM VanStatementDetail, Batch_Products
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.PTR, 0) = @GenericPTR AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack	
		END
	ELSE IF @CUSTOMERTYPE = 3
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.SpecialPrice, 0) = @SALE_PRICE And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		...
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.SpecialPrice, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		END
	ELSE
		BEGIN
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.SalePrice, 0) = @SALE_PRICE And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		...
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.SalePrice, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		END
	END
ELSE
	...
	IF @CUSTOMERTYPE = 1
		...
		WHERE VanStatementDetail.Batch_Code = Batch_Products.Batch_Code And
		DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE AND 
		ISNULL(VanStatementDetail.PTS, 0) = @SALE_PRICE
		and isnull(Batch_Products.MRPPerPack, 0) = @BatchMRPPerPack
		
		DECLARE ReleaseStocks CURSOR KEYSET FOR
		SELECT VanStatementDetail.Batch_Number, VanStatementDetail.Batch_Code, Pending, 
		VanStatementDetail.PurchasePrice, [ID], VanStatementDetail.PTS, VanStatementDetail.PTR, VanStatementDetail.ECP FROM 
...
--	ELSE
		SET @MRPPerPack =  @BatchMRPPerPack 
	...
	@UOMPRICE, @OtherCG_Item,@QuotationID,@MultiSchID,@TotSchAmount,@MultiSchemeDetail
	,@MultipleRebateID,@RebateRate,@MultipleRebateDet,@GroupID, @MRPPerPack,@TaxOnQty,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)
	...
	@CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @PTS, @PTR, @UOM, @UOMQTY, @UOMPRICE, @OtherCG_Item,@QuotationID,@MultiSchID,@TotSchAmount,@MultiSchemeDetail
	,@MultipleRebateID,@RebateRate,@MultipleRebateDet,@GroupID, @MRPPerPack,@TaxOnQty,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)
	SET @TAXCODE = 0
