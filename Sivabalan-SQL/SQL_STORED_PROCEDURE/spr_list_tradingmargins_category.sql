CREATE PROCEDURE spr_list_tradingmargins_category (@ProductHierarchy nvarchar(256), @Category nvarchar(256),  @FROMDATE DATETIME, @TODATE DATETIME)  
AS  
Create Table #tempCategory (CategoryID int, Status int)                
Exec GetLeafCategories @ProductHierarchy, @Category              
  
Create table #temp(CategoryID int, CategoryName nvarchar(128) COLLATE SQL_Latin1_General_CP1_CI_AS, SalesValue Decimal(18,6), PurchaseValue Decimal(18,6), Margin Decimal(18,6))  
Insert into #temp  
...
select CategoryID, "Category Name" = CategoryName, "Sales Value (%c.)" = Sum(SalesValue), "Purchase Value (%c.)" = Sum(PurchaseValue), "Trading Margin (%c.)" = Sum(Margin)  
From #temp  
