CREATE PROCEDURE [dbo].[spr_list_billAbstract_Pidilite](@VENDOR nvarchar(2550),
				@FROMDATE datetime, 
				@TODATE datetime)
AS
...
SELECT  BillID, "Bill ID" = CASE 
	...
	"Trade Discount Amount" = Cast(((Select Sum(Quantity * PurchasePrice) - Billabstract.ProductDiscount  From BillDetail 
						Where BillDetail.BillID = BillAbstract.BillID) * Discount / 100) as Decimal(18,6)),
	"Product Discount" = Billabstract.ProductDiscount,
	"Additional Discount" = Billabstract.AddlDiscountAmount,
...
Inner Join  VoucherPrefix BillPrefix On BillPrefix.TranID = 'BILL' 
...
Inner Join  VoucherPrefix BillAPrefix On BillAPrefix.TranID = 'BILL AMENDMENT'
