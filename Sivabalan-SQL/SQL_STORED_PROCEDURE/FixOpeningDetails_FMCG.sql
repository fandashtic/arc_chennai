CREATE PROCEDURE FixOpeningDetails_FMCG        
AS        
...
OR (InvoiceDetail.SalePrice = 0 And InvoiceDetail.PurchasePrice <> 0)) 
And IsNull(InvoiceDetail.Batch_Code,0) = 0 
And (InvoiceAbstract.Status & 32) = 0 And (InvoiceAbstract.Status & 128) = 0      
...
BEGIN      
	...
	BEGIN      
		...
		IsNull(Damage_Opening_Quantity,0), IsNull(Damage_Opening_Value,0),     
		...
		begin      
			...
			@SalesReturnDamages = Sum(Case When (InvoiceAbstract.InvoiceType = 6) Then Quantity When ((InvoiceAbstract.InvoiceType = 4) AND (Status & 32) <> 0) Then Quantity Else 0 end),       
			@SalesReturnDamagesValue = Sum(Case When (InvoiceAbstract.InvoiceType = 6) Then PurchasePrice When ((InvoiceAbstract.InvoiceType = 4) AND (Status & 32) <> 0) Then PurchasePrice Else 0 end),      
		   ...
		   Set @NewFreeSaleable = IsNull(@FreeSaleable,0) + IsNull(@FreePurchases,0) 
			...
			BEGIN      
				...
				And Opening_Date = DateAdd(d, 1, @FirstTransactionDate) And      
				...
				Case 
				When (Select IsNull(Vat,0) from Items Where Items.Product_Code=BPT.Product_Code) = 1 Then 
					(Sum(Case IsNull(BPT.Vat_Locality,0) When 2 Then DBO.FN_GetTaxSufferedAmt_FMCG(IsNull(BPT.PurchasePrice,0), IsNull(Items.MRP,0), IsNull(BPT.Quantity, 0), IsNull(BPT.TaxSuffered,0), IsNull(BPT.ApplicableOn, 0), IsNull(BPT.PartOfPercentage, 0), 				
					BPT.Batch_Code) Else 0 End)	 /      
					Case When Sum(IsNull(BPT.Quantity, 0) *  IsNull(BPT.PurchasePrice, 0)) = 0 Then 1         
					Else Sum(IsNull(BPT.Quantity, 0) *  IsNull(BPT.PurchasePrice, 0)) End)*100     
				Else 0 End     
				FROM Batch_Products_Temp BPT, Items    
				Where Items.Product_Code = BPT.Product_Code    
