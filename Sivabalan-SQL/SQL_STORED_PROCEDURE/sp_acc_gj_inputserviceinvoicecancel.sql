CREATE Procedure sp_acc_gj_inputserviceinvoicecancel (@invoiceid INT,@BackDate DATETIME=Null)
AS
...
Set @AccountType = 152 --Input Service Invoice Cancellation
...
If @NetValue<>0
Begin
execute sp_acc_insertGJ @TransactionID,@PartyID,@Invoicedate,@NetValue,0,@invoiceid,@AccountType,"Input Service Invoice Cancellation",@DocumentNumber
...
begin
execute sp_acc_insertGJ @TransactionID,@AccountID,@InvoiceDate,0,@TaxableValue,@InvoiceID,@AccountType,"Input Service Invoice Cancellation",@DocumentNumber
...
begin
execute sp_acc_insertGJ @TransactionID,@AccountID,@InvoiceDate,0,@NetValue,@InvoiceID,@AccountType,"Input Service Invoice Cancellation",@DocumentNumber
Insert Into #TempBackdatedAccounts(AccountID) Values(@TaxableValue)
end
End
...
if @nGSTaxAmt <> 0
begin
select  @gst_payable	  = InputAccID
from	TaxComponentDetail(nolock)
where	TaxComponent_Code = @GSTaxComponent

if (isnull(@gst_payable,0)>0)
begin
execute sp_acc_insertGJ @TransactionID,@gst_payable,@InvoiceDate,0,@nGSTaxAmt,@InvoiceID,@AccountType,"Input Service Invoice Cancellation",@DocumentNumber
