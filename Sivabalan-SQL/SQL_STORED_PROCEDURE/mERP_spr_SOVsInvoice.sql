Create Procedure mERP_spr_SOVsInvoice
(
...
Where 
	...
	CH.ChannelType In (Select ChannelType From #tmpChannel) 
Group By
...
InvQty Decimal(18,6),InvValue Decimal(18,6),InvID int)
...
Select 
	...
	Else SONumber End ) ,
	InvoiceDate,case IsNULL(IA.GSTFlag ,0) when 0 then 
(@Inv + Cast(IA.DocumentID as nVarchar) ) else   IsNULL(IA.GSTFULLDOCID,'') end
,IA.CustomerID,DocReference,
	I.Product_Code ,
	...
	Else isNull(ID.Quantity,0) End) ,Sum(ID.Amount),IA.Invoiceid
...
Group By 
	SONumber,DocumentID,IA.InvoiceDate,(@Inv + Cast(IA.DocumentID as nVarchar) ),IA.GSTFLAG,IA.GSTFULLDOCID,
	I.Product_Code ,DocReference,IA.CustomerID, IA.InvoiceType, IA.ReferenceNumber,IA.Invoiceid  
...
Select 
	#tmpDisp.SoNumber,
	InvoiceDate,case IsNULL(IA.GSTFlag ,0) when 0 then 
(@Inv + Cast(IA.DocumentID as nVarchar) ) else   IsNULL(IA.GSTFullDocID,'') end,IA.CustomerID,DocReference,
	I.Product_Code ,
	Sum(Case @UOM When N'UOM1' Then isNull(ID.Quantity,0)/(Case When IsNull(I.UOM1_Conversion, 0) = 0 Then 1 Else I.UOM1_Conversion End) 	 
	When N'UOM2' Then isNull(ID.Quantity,0)/(Case When IsNull(I.UOM2_Conversion, 0) = 0 Then 1 Else I.UOM2_Conversion End) 	 		
	Else isNull(ID.Quantity,0) End) ,Sum(ID.Amount),IA.Invoiceid
...
Where 
	...
	I.CategoryID In (Select CategoryID From #tmpCatID) 
Group By 
	#tmpDisp.SONumber,IA.DocumentID,IA.InvoiceDate,(@Inv + Cast(IA.DocumentID as nVarchar) ),IA.GSTFLAG,IA.GSTFULLDOCID,
	I.Product_Code ,DocReference,IA.CustomerID,IA.Invoiceid
...
Begin
	...
	[Status] nVarChar(10) COLLATE SQL_Latin1_General_CP1_CI_AS,
	[Pending Qty] Decimal(18,6),
	[Order Item Value] Decimal(18,6),
	[Invoice Qty] Decimal(18,6),
	[Invoice Item Value] Decimal(18,6),CreditLimitExceed nVarChar(10) COLLATE SQL_Latin1_General_CP1_CI_AS,invid int)
	...
	Select 
		...
		InvDate,Inv.InvoiceID,DocRef,UOM,OrdQty,SO.OrdStatus,
	case when SO.OrdStatus <> 'Cancelled' then case when (IsNull(OrdQty, 0) - IsNull(InvQty, 0)) < 0 then 0 else (IsNull(OrdQty, 0) - IsNull(InvQty, 0)) end else 0 end, 
	IsNull(OrdValue, 0),IsNull(InvQty, 0), IsNull(InvValue, 0),'',inv.InvID --SO.OrdPending,
	From 
		#tmpSO SO
		Left Outer Join #tmpInv Inv On SO.SONumber = Inv.SONumber And SO.Item_Code = Inv.Prod_Code
	update T set CreditLimitExceed ='Yes' from #tmpOutput T,GT_invoice G where isnull([Invoice No],'') <> ''  
	and G.Invoiceid=T.InvID
	And G.Alerttype in (1,2,3,4)

	update #tmpOutput set CreditLimitExceed ='No' where isnull(CreditLimitExceed,'')='' and isnull([Invoice No],'') <> ''  
	
	update #tmpOutput set CreditLimitExceed ='' where isnull([Invoice No],'') = '' 
	
	Alter table #tmpOutput Drop Column InvID
