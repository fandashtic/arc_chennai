CREATE PROCEDURE sp_print_GRNitems_MultiUOM_serial(@GRNID int)      
AS      
...
SELECT  "Item Code" = GRNDetail.Product_Code, "Item Name" = Items.ProductName,    
 ...
 "Free" = Cast(Sum(FreeQty) As nvarchar), NULL,grndetail.Serial  
...
ItemCategories.Price_Option, Items.Track_Batches,grndetail.Serial  
UNION ALL  
...
"Free"= Case IsNull(Batch_Products.Free, 0) When 1 Then @FREE Else N'' End , "TaxSuffered" = Batch_Products.TaxSuffered,grndetail.Serial  
...
Batch_Products.PTS,Batch_Products.PTR, Batch_Products.ECP, Batch_Products.Company_Price, Batch_Products.Free  , Batch_Products.TaxSuffered,grndetail.Serial  
UNION ALL      
SELECT  "Item Code"= Batch_Products.Product_Code, "Item Name"= Items.ProductName,      
...
Case IsNull(Batch_Products.Free, 0) When 1 Then @FREE Else N'' End , "TaxSuffered" = Batch_Products.TaxSuffered,grndetail.Serial  
FROM Batch_Products, Items, ItemCategories,grndetail  
WHERE Batch_Products.GRN_ID = @GRNID AND Batch_Products.Product_Code = Items.Product_Code      
 AND ItemCategories.CategoryID = Items.CategoryID       
 AND GRNDetail.GRNID = Batch_Products.GRN_ID  
