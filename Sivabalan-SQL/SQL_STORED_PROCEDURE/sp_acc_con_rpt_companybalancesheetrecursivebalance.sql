CREATE procedure sp_acc_con_rpt_companybalancesheetrecursivebalance(@parentid integer,@fromdate datetime, @todate datetime,@Company nvarchar(128), @AccountMode Integer,@Fixed Integer,@balance decimal(18,2) output,@TotalDepAmt decimal(18,2) = 0 Output)
as
Set @balance=0
Set @TotalDepAmt=0
Declare @GroupID int
...
Begin
	...
	Begin
		...
		Insert into #temp values(@parentid,0)
	  	if (@AccountMode = 3 and @Parentid = 54) or (@AccountMode = 5 and @Parentid = 22) or (@AccountMode = 5 and @Parentid = 89)
	  	Begin
			...
			AccountID in (@OPENINGSTOCK,@TAXONOPENINGSTOCK) --and CompanyID =@Company and Date=@todate
		End
		Else
		Begin
			Declare scanrecursiveaccounts Cursor Keyset For
			Select AccountID from ConsolidateAccount where GroupID in (select groupid from #temp) and
			AccountID not in (@OPENINGSTOCK,@TAXONOPENINGSTOCK) --and CompanyID =@Company and Date=@todate
		End
...
Begin
	If IsNull(@Fixed,0)=0
	Begin
		if (@AccountMode = 5 and @Parentid = @OPENINGSTOCK) or (@AccountMode = 5 and @Parentid = @TAXONOPENINGSTOCK)
	   	Begin
			Declare scanrecursiveaccounts Cursor Keyset For
			Select AccountID from ConsolidateAccount where AccountID in (@OPENINGSTOCK,@TAXONOPENINGSTOCK) 
			and AccountID=@ParentID --and CompanyID =@Company and Date=@todate
		End
		Else
		Begin
			Declare scanrecursiveaccounts Cursor Keyset For
			Select AccountID from ConsolidateAccount where AccountID not in (@OPENINGSTOCK,@TAXONOPENINGSTOCK) 
			and AccountID=@ParentID --and CompanyID =@Company and Date=@todate
		End
		Open scanrecursiveaccounts
		Fetch From scanrecursiveaccounts Into @AccountID
	...
	Begin
		if (@AccountMode = 5 and @Parentid = @OPENINGSTOCK) or (@AccountMode = 5 and @Parentid = @TAXONOPENINGSTOCK)
	   	Begin
			Declare scanrecursiveaccounts Cursor Keyset For
			Select AccountID from ReceiveAccount where AccountID in (@OPENINGSTOCK,@TAXONOPENINGSTOCK) 
			and AccountID=@ParentID and CompanyID =@Company and Date=@todate
		End
		Else
		Begin
			Declare scanrecursiveaccounts Cursor Keyset For
			Select AccountID from ReceiveAccount where AccountID not in (@OPENINGSTOCK,@TAXONOPENINGSTOCK) 
			and AccountID=@ParentID and CompanyID =@Company and Date=@todate
		End
		Open scanrecursiveaccounts
		Fetch From scanrecursiveaccounts Into @AccountID
