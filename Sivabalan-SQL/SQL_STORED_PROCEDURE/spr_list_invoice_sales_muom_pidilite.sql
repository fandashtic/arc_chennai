CREATE procedure [dbo].[spr_list_invoice_sales_muom_pidilite](@ITEMCODE nvarchar(15),   
            @UOM nvarchar(100),  
                                 @FROMDATE DATETIME,  
                           @TODATE DATETIME)  
AS  
...
"Quantity" = CAST((Case @UOM When N'Sales UOM' Then Sum(InvoiceDetail.Quantity) 
                                When N'UOM1' Then dbo.sp_Get_ReportingQty(Sum(InvoiceDetail.Quantity), UOM1_Conversion)  
                                When N'UOM2' Then dbo.sp_Get_ReportingQty(Sum(InvoiceDetail.Quantity), UOM2_Conversion) End) AS nvarchar)  
+ N' ' + CAST(UOM.Description AS nvarchar),   
"Conversion Factor" = SUM(IsNull(InvoiceDetail.Quantity, 0) * IsNUll(Items.ConversionFactor, 0)),  
-- CAST(CAST(SUM(Quantity * Items.ConversionFactor) AS DECIMAL(18,6)) AS nvarchar)  
-- + ' ' + CAST(ConversionTable.ConversionUnit AS nvarchar),  
"Reporting UOM" = SUM(IsNull(InvoiceDetail.Quantity, 0) / Case IsNull(Items.ReportingUnit, 1) When 0 Then 1 Else IsNull(Items.ReportingUnit, 1) End),  
--   SubString(  
