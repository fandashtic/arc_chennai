...
Create Procedure [dbo].[sp_print_RetInvItems_RespectiveUOM_SR_ITC_ASM_TOQ](@INVNO INT,@MODE INT = 0)          
AS  
Begin  
...
"TaxDetails" = dbo.GetTaxCompInfo_TOQ(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code, 
										Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.STPayable)),
"TaxDetailsWithBreakup" = dbo.GetTaxCompInfoWithBreakup_TOQ(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code, 
								Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.STPayable)),
...
"Serial"= Min(InvoiceDetail.Serial) ,"M SQty" = Sum(InvoiceDetail.Quantity) 
Into  
 #TmpInvDet    
