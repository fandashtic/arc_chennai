Create Procedure [dbo].[sp_print_RetInvItems_RespectiveUOM_SR_ITC_CST_GUJARAT](@INVNO INT,@MODE INT = 0)          
AS  
Begin  
...
"TaxDetails" =  case when MAX(ISNULL(invoicedetail.CSTPayable,0)) >0 Then
dbo.GetTaxCompInfo_CST(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code, 
										Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.CSTPayable))
					else dbo.GetTaxCompInfo_CST(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code, 
										Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.STPayable))
										end,
"TaxDetailsWithBreakup" = 
case when MAX(ISNULL(invoicedetail.CSTPayable,0)) >0 Then
dbo.GetTaxCompInfoWithBreakup_CST(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code, 
								Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.CSTPayable)) 
else
dbo.GetTaxCompInfoWithBreakup_CST(Max(InvoiceDetail.InvoiceID),InvoiceDetail.Product_Code, 
								Max(InvoiceDetail.TaxID), Sum(InvoiceDetail.STPayable)) end,
...
"Item MRP" = Isnull(InvoiceDetail.MRPPerPack,0),
...
--InvoiceAbstract.TaxOnMRP,Items.TaxSuffered,Items.Sale_Tax,Items.MRPPerPack,  
 InvoiceAbstract.TaxOnMRP,Items.TaxSuffered,Items.Sale_Tax,Isnull(InvoiceDetail.MRPPerPack,0),	
 InvoiceDetail.TaxID,#Temp1.Invno,
 InvoiceDetail.UOM, Items.Soldas,Batch_Products.Batch_Number,Batch_Products.PKD,Batch_Products.Expiry  
Order By Serial
