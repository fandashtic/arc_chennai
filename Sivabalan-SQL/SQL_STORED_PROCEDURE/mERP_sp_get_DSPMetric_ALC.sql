Create Procedure mERP_sp_get_DSPMetric_ALC(@PMetricID Int, @ParamID Int, @EditLock Int = 0)
As
Begin 

		select T.SalesManID,S.SalesMan_Name,PMDS.DSType,isnull(T.Target,0) as Target,T.MaxPoints,T.PMID,T.PMDSTypeID,T.ParamID,--FocusSKU,
		T.DSTypeCGMapID,T.TargetDefnID,T.TargetDefnDate
		from tbl_merp_PMOutletAch_TargetDefn T,Salesman S,tbl_mERP_PMDSType PMDS,tbl_mERP_PMParam PMparam where T.Active = 1
		And T.SalesmanID = S.SalesmanID
		And T.PMID=@PMetricID
		And T.ParamID=@ParamID
		And PMparam.DSTypeID = PMDS.DSTypeID
		And PMDS.PMID=@PMetricID
		And PMParam.ParamID= @ParamID
		order by S.SalesMan_Name

--Below logic is commented as It is not required to recalculate everything. As per SRS once target saved for received PM then no changes can be done
...
set dateformat dmy
	...
	Begin
		...
		Create TABLE #AvgNetsales (SalesManid int, SalesValue Decimal(18,6),salesmonth int,ProductName Nvarchar(500) COLLATE SQL_Latin1_General_CP1_CI_AS)
		Create TABLE #FinalNetsales (SalesManid int, SalesValue Decimal(18,6))
		...
		Declare @Tempdata as Table (SalesManID int NULL,
			...
			SalesValue decimal(18, 6) NULL,GrowthPercentage decimal(18,6),ProposedTargetValue decimal(18,6),LastupdatedDate Datetime,AutoPostflag int,SaveFlag int)
	...
	Begin
		...
		--FocusSKU, 
		0 as FocusID,CGMapID, 0 TargetDefnID , NULL SalesValue,GrowthPercentage,0 ProposedTargetValue,NULL,0,0
		From @tmpDSTarget
		Union 
		...
		isnull(PMTar.ProposedTargetValue,0) ProposedTargetValue,PMTar.TargetDefnDate,isnull(PMTar.AutopostFlag,0),0
		...
		BEGIN
			...
			Create Table #tmpOutletwiseData(DSID int,Target decimal(18,6))
			...
			Update T set T.ProposedTargetValue = T1.Target
			From @Tempdata T, #tmpOutletwiseData T1
			Where T.SalesmanID = T1.DSID
			And T.saveflag=0
...
--			Update @Tempdata set ProposedTargetValue=Target
			
			Drop Table #tmpOutletwiseData
		END
		ELSE
		BEGIN
	...
	Begin
		...
		--Case IsNull(PMparam.isFocusParameter,0) When 0 Then N'Overall' Else PMFocus.ProdCat_Code End, 
		PMTar.FocusID, PMTar.DSTypeCGMapID, PMTar.TargetDefnID ,isnull(PMTar.Avgsales,0),isnull(PMTar.GrowthPerc,0),isnull(PMTar.ProposedTargetValue,0),PMTar.TargetDefnDate,isnull(PMTar.AutopostFlag,0),1
...
--		If (Select isnull(flag,0) from Tbl_Merp_ConfigAbstract where screencode='PMOutletTarget')=1
--		BEGIN
--			update @Tempdata set ProposedTargetValue = Isnull(Target,0)
--		END
	End
	If @EditLock=0
	BEGIN
		...
		BEGIN
			...
			CGMapID,TargetDefnID,SalesValue,GrowthPercentage,ProposedTargetValue,LastupdatedDate,AutoPostflag,SaveFlag
			from @Tempdata		
		END
		ELSE
		BEGIN
			select SalesManID,SalesMan_Name,DSTypeValue,isnull(Target,0) as Target,MaxPoints,PMID,PMDSTypeID,ParamID,--FocusSKU,
			CGMapID,TargetDefnID,SalesValue,GrowthPercentage,isnull(ProposedTargetValue,0) as ProposedTargetValue,LastupdatedDate,AutoPostflag,SaveFlag
			from @Tempdata		
		END
	END
	ELSE
