CREATE procedure sp_acc_rpt_userwisedenominations(@FromDate DateTime,@ToDate DateTime)    
as    
Declare @CASH Int    
...
Create table #TempDenominations (UserName nVarchar(255),Thousand Int,FiveHundred Int,    
...
and ContraAbstract.ContraID = ContraDetail.ContraID    
Open ScanDeno    
Fetch From ScanDeno Into @UserName,@Denomination    
While @@Fetch_Status = 0     
Begin    
 Insert #TempDenominations    
 Exec sp_acc_rpt_retrievedenominations @Denomination,@UserName    
     
 Fetch Next From ScanDeno Into @UserName,@Denomination    
End    
Close ScanDeno    
DeAllocate ScanDeno    
...
Insert #TempDenominations     
select UserName, Sum(IsNull(Thousand,0)), Sum(IsNull(FiveHundred,0)),    
...
from #TempDenominations Group by UserName    
...
Select 'User' = UserName,'1000'= IsNull(Thousand,0),    
'500' = IsNull(FiveHundred,0),'100' = IsNull(Hundred,0),'50' = IsNull(Fifty,0),    
...
'From Date' = @FromDate,'ToDate'=@ToDate,SerialNo from #TempDenominations Order By SerialNo    
