Create Procedure [dbo].[sp_print_DandDInvItems_RespectiveUOM_FMCG_Intra](@DnDID INT,@MODE INT=0)
AS
Begin
...
--into #TempInvDet2
--from DandDDetail ID
--Where ID = @DnDID and RFAQuantity > 0
...
"MRP" =  Cast(Case When Duplicate = 1 then Cast((Case When #TempTaxDet.MRPPerPack = 0 Then I.MRPPerPack Else #TempTaxDet.MRPPerPack End) as Decimal(18,2))  Else (Case @utgst_flag When 1 Then Cast(Max(UTGSTPer) as decimal(18,2)) Else Cast(Max( SGSTPer) as decimal(18,2)) End)
End As nVarchar),
"Sale Price" = Isnull(Case When Duplicate=1
Then Cast(cast(DD.UOMPTS As Decimal(18,2)) as nvarchar)
Else (Case @utgst_flag When 1 Then Cast(Cast(sum(#TempTaxDet.UTGSTAmt ) as decimal(18,2))as nVarchar) Else Cast(Cast(sum(#TempTaxDet.SGSTAmt) as decimal(18,2))as nVarchar) End)
End,''),
...
then Cast(Cast(Sum(DD.UOMRFAQty ) as decimal(18,2)) As nVarchar(50)) Else Cast(Cast(Cast(Sum(DD.RFAQuantity ) as Decimal(18,2)) As nVarchar(50)) + Cast((Select Isnull(Description,'') from UOM where UOM = I.UOM) As nVarchar(50)) As nVarchar(100))
End as nVarchar),''),
...
Else  Cast(Cast(Max(CGSTPer ) as decimal(18,2))as nVarchar) End,

"CGSTAmount" = 	Case When Duplicate = 1 then ''
Else  Cast(cast(Sum(#TempTaxDet.CGSTAmt ) as Decimal(18,2)) as nvarchar)  End,

"Salvage Value" = Isnull(cast(Case When Duplicate=1
then Cast(cast(Sum(DD.BatchSalvageValue )AS decimal(18,2)) as nvarchar)
...
Group By I.HSNNumber, DD.Product_code, I.ProductName,#TempTaxDet.MRPPerPack, I.MRPPerPack  , U.Description , DD.UOMPTS ,Duplicate,I.Product_Code,I.UOM
