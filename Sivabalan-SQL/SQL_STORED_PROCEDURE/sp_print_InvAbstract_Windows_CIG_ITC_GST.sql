Create Procedure [dbo].[sp_print_InvAbstract_Windows_CIG_ITC_GST](@INVNO INT)     
AS 
Set dateformat DMY         
...
exec sp_print_RetInvItems_RespectiveUOM_CIG_ITC_GST @INVNO,1
--Select  1
...
Where InvoiceId = @INVNo
and UOMQty > 0 and IsNull(FlagWord,0) <> 1
--Group by ID.Serial 
...
"Customer Name" = CASE When Isnull(GSTFlag,0) = 1 and isnull(Customer.PreDefFlag,0) = 1 Then Company_Name + ' - ' + isnull(AlternateCGCustomerName,'') Else Company_Name End,
...
,"FSSAINO" = Case when customer.TNGST  = '' then '' Else 'FSSAI No. : ' + customer.TNGST End,
"WDFSSAINO" = @WDFSSAINO
