CREATE PROCEDURE [dbo].[spr_List_Collection_DSWise_BeatWise_Abstract_ITC]    
(    
...
---------------------------------------------------------
-- 1
select "IDS" = ItemCategories.IDS, "CategoryID" = ItemCategories.CategoryID,
...
select * Into #tmtwoOne
from    
(           
   select "KEYPARAM" =  cast(isnull(#tmpCollInvoiceData.SalesmanID,0) as varchar(10)) + @Delimiter + cast(isnull(#tmpCollInvoiceData.BeatID,0) as varchar(10)) + @Delimiter + cast(isnull(#tmpCollInvoiceData.GroupID,0) as varchar(10)) + @Delimiter + cast(isnull(#tmpCollInvoiceData.HierarchyCatID,0) as varchar(10)),     
          "DS Name" = #tmpCollInvoiceData.Salesman_Name,     
          "Beat" = #tmpCollInvoiceData.Beat,    
          "Category Group" = #tmpCollInvoiceData.CategoryGroupName,     
   ...
   group by #tmpCollInvoiceData.SalesmanID,#tmpCollInvoiceData.BeatID,#tmpCollInvoiceData.Salesman_Name, #tmpCollInvoiceData.Beat,    
          ...
          #tmpCollInvoiceData.HierarchyCatID,#tmpCollInvoiceData.GroupID           
) t1,    
(    
...
select temp3.[KeyParam1],sum(temp3.[Invoice Adjustment]) as [Invoice Adjustment] from (select distinct cast(isnull(#tmpCollInvoiceData.SalesmanID,0) as varchar(10)) + @Delimiter + cast(isnull(#tmpCollInvoiceData.BeatID,0) as varchar(10)) + @Delimiter + cast(isnull(#tmpCollInvoiceData.GroupID,0) as varchar(10)) + @Delimiter + cast(isnull(#tmpCollInvoiceData.HierarchyCatID,0) as varchar(10)) as [KeyParam1],  
#tmpCollInvoiceData.InvoiceAdjustments as [Invoice Adjustment]  
