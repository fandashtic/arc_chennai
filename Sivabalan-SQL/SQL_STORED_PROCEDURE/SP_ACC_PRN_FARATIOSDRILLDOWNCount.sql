CREATE PROCedure SP_ACC_PRN_FARATIOSDRILLDOWNCount
(  
 	@FROMDATE DATETIME,  
	@TODATE   DATETIME,  
 	@RATIOID  NVARCHAR(50),
...
*/
	...
	IF @groupid <> 0 or @mode = 0
 		BEGIN  
			...
			IF @MODE = 3 
				BEGIN
					SELECT count(*) from #DRILLDOWNTABLE
				END
	...
	IF @RATIOID = 1 /* CURRENT RATIO */  
		BEGIN  
			...
			SELECT Count(*) + 4 FROM #DRILLDOWNTABLE  
-- -- 			where ltrim(rtrim(GroupName)) not in ( 'Ratio','Total') 
		END  
...
-- -- 			where ltrim(rtrim(GroupName)) not in ( 'Ratio','Total')
	...
	ELSE IF @RATIOID = 3
		BEGIN
			...
			SELECT @TOTAL = DEBIT - CREDIT FROM #TEMPREGISTER WHERE GROUPID = 0
				SET @VALUEOFGROUP2	= @TOTAL
			SELECT Count(*) + 3
			FROM #DRILLDOWNTABLE  --where ltrim(rtrim(GroupName)) not in ( 'Ratio','Total')
		END
			...
			UPDATE #DRILLDOWNTABLETEMP SET CREDIT = 0 WHERE CREDIT IS NULL AND GROUPID >= 1
				...
				ELSE IF @RATIOID = 5   -- ITS A NET LOSS/ PROFIT RATIO SO GET THE GROSS LOSS / PROFIT
					BEGIN
						...
						--SELECT @NETSALES	= DEBIT FROM #DRILLDOWNTABLETEMP WHERE LTRIM(RTRIM(GROUPNAME)) = dbo.LookupDictionaryItem('Sales Account',Default) 
						SELECT @NETSALES	= CASE WHEN ISNULL(DEBIT,0) = 0 THEN CREDIT ELSE DEBIT END FROM #DRILLDOWNTABLETEMP WHERE LTRIM(RTRIM(GROUPNAME)) = dbo.LookupDictionaryItem('Sales Account',Default) 
						IF @NETPROFIT > 0 
						...
						INSERT INTO #DRILLDOWNTABLETEMP(GROUPNAME,DUMMY1,COLORINFO2,GROUPID ) VALUES(dbo.LookupDictionaryItem('Ratio',Default) ,@PERCENTAGE ,@COLORBLUE,-1)
		 				...
		 				SELECT Count(*)
						FROM #DRILLDOWNTABLETEMP -- where ltrim(rtrim(isnull(GroupName,''))) not in ( dbo.LookupDictionaryItem('Ratio',Default) ,dbo.LookupDictionaryItem('Total',Default)  )
					END
	...
	ELSE IF @RATIOID = 7 
		BEGIN
			...
			SELECT Count(*) + 2
			FROM #DRILLDOWNTABLE --where ltrim(rtrim(isnull(GroupName,''))) not in ( dbo.LookupDictionaryItem('Ratio',Default) ,dbo.LookupDictionaryItem('Total',Default) )
		END
	...
	ELSE IF @RATIOID = 9 -- RETURN ON INVESTMENT (NET PROFIT / CAPITAL * 100)
		BEGIN
			...
			SELECT Count(*) + 2
		    FROM #DRILLDOWNTABLETEMP1 --where ltrim(rtrim(isnull(GroupName,''))) not in ( dbo.LookupDictionaryItem('Ratio',Default) ,dbo.LookupDictionaryItem('Total',Default) )
	...
	ELSE IF @RATIOID = 10 --RETURN ON WORKING CAPITAL -> PROFIT / WORKING CAPITAL * 100
		BEGIN
			...
			INSERT INTO #DRILLDOWNTABLETEMP2(GROUPNAME)VALUES(NULL)
			SELECT Count(*) + 4
			FROM #DRILLDOWNTABLETEMP2 --where ltrim(rtrim(isnull(GroupName,''))) not in ( 'Ratio','Total')
		END
