CREATE procedure [dbo].[spr_ser_list_manufacturerwise_stockmovement]            
              ( @Mfr Varchar(2550),                
                @FROMDATE datetime,                
                @TODATE datetime,        
...
Create Table #temp             
(Manu_Code Int,            
Manu_Name varchar(100) COLLATE SQL_Latin1_General_CP1_CI_AS,            
...
Issuedetail,IssueAbstract,JobcardAbstract,Items a
where IssueAbstract.IssueID =IssueDetail.IssueID And  
IssueAbstract.JobCardId = JobcardAbstract.JobcardID   
And Issuedetail.Sparecode = items.product_code
And a.Product_Code = items.product_code
...
AND (VanStatementAbstract.Status & 128) = 0               
And VanStatementDetail.Product_Code = Items.Product_Code 
And VanStatementDetail.SalePrice <> 0 and a.Product_Code = Items.Product_Code))              
end,                
