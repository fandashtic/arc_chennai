CREATE Procedure sp_acc_rpt_groupwiseledger(@fromdate datetime,@todate datetime ,@parentid  integer,@Hide0BalAC Int =0)
as        
...
If @parentgroup1 = 0
Begin
	Declare @AllGroupID Int
	Declare @AllGroupName nVarchar(250)
	create Table #TempRegisterAll
	(RowNum Integer Identity(1,1),
	GroupName nvarchar(255),
	Debit decimal(18,6),Credit decimal(18,6),Dummy1 Int,
	GroupID integer,FromDate Datetime,ToDate Datetime, 
	DocRef integer,DocType integer,Colorinfo1 integer,
	ColorInfo2 integer) 	
	
	Truncate Table #TempRegisterAll
	
	Declare GroupWiseLedgerAll Cursor Local For
	select GroupID,GroupName from AccountGroup where GroupID not in 
	(21,54,55,500) order by [GroupName] --Stock in Trade, Opening Stock, Closing Stock, User Account Group Start
	Open GroupWiseLedgerAll
	Fetch From GroupWiseLedgerAll into @AllGroupID,@AllGroupName
	
	While @@Fetch_Status = 0
	Begin
		Insert into #TempRegisterAll (GroupName,ColorInfo1,ColorInfo2)
		Values (@AllGroupName,1,1)
		Insert into #TempRegisterAll (GroupName ,Debit ,Credit ,Dummy1 ,
		GroupID ,FromDate ,ToDate ,DocRef ,DocType ,Colorinfo1 ,
		ColorInfo2 ) 	
		Execute sp_acc_rpt_groupwiseledger @fromdate,@todate,@AllGroupID
		Insert into #TempRegisterAll (ColorInfo1,ColorInfo2)
		Values (1,1)
		Fetch Next From GroupWiseLedgerAll into @AllGroupID,@AllGroupName
	End
	Close GroupWiseLedgerAll
	Deallocate GroupWiseLedgerAll
	Goto GetOut
End
create Table #TempRegister(RowNum int Identity(1,1),GroupID integer,GroupName nvarchar(255),Debit decimal(18,6),Credit decimal(18,6),FromDate datetime,ToDate datetime,DocRef integer,DocType integer,ColorInfo integer)        
 ...
 WHILE @@FETCH_STATUS =0        
  ...
  If @TotalDepAmt=0        
  Begin        
      If @groupid=@OPENINGSTOCKGROUP        
      Begin        
       INSERT INTO #TempRegister        
       SELECT 'GroupID'= @groupid,'GroupName'=@group,'Debit'= @balance,'Credit'= 0,@fromdate,@todate,0,0,@ACCOUNTGROUP             
      End        
      Else If @groupid=@CLOSINGSTOCKGROUP       
      Begin      
 	   INSERT INTO #TempRegister        
       SELECT 'GroupID'= @groupid,'GroupName'=@group,'Debit'=0,'Credit'= abs(@balance),@fromdate,@todate,0,0,@ACCOUNTGROUP             
      End        
      Else        
      Begin        
       INSERT INTO #TempRegister        
 ...
 DECLARE scangroupwiseaccounts CURSOR KEYSET FOR        
 select AccountID,AccountName from AccountsMaster where [GroupID]= @parentid --and isnull(Active,0)=1        
 And AccountID Not In (@OPENINGSTOCK,@TAXONOPENINGSTOCK) Order By AccountName
 OPEN scangroupwiseaccounts        
 FETCH FROM scangroupwiseaccounts into @AccountID,@AccountName        
 WHILE @@FETCH_STATUS=0        
 Begin        
...
--    End        
  ...
  @fromdate,@todate,0,0,@LEAFACCOUNT         
  */        
  FETCH NEXT FROM scangroupwiseaccounts into @AccountID,@AccountName        
 End        
 CLOSE scangroupwiseaccounts        
 DEALLOCATE scangroupwiseaccounts        
        
 select @totaldebit = SUM(ISNULL(Debit,0)),@totalcredit = SUM(ISNULL(Credit,0)) from #TempRegister        
         
 INSERT #TempRegister        
 select '','Total',isnull(@totaldebit,0),isnull(@totalcredit,0),@fromdate,@todate,0,0,@NONEXTLEVEL        
...
Begin
	...
	Begin
		select 'Account/Group'= GroupName,'Debit'=Debit,'Credit'=Credit,'', 
		'AccountID/GroupID'= CASE WHEN GroupID = 0 then '' else GroupID end,
		@fromdate,@todate,DocRef,DocType,ColorInfo1,ColorInfo2 from #TempRegisterAll         
		Order by Rownum
	End
	Else
	Begin
		...
		'AccountID/GroupID'= CASE WHEN GroupID = 0 then '' else GroupID end,
		@fromdate,@todate,DocRef,DocType,ColorInfo1,ColorInfo2 from #TempRegisterAll         
		where ((Isnull(Debit,0) + Isnull(Credit,0)) > 0 or isnull(colorinfo2,0) in (1,3))
		Order by Rownum
	End	
	drop Table #TempRegisterAll
End
