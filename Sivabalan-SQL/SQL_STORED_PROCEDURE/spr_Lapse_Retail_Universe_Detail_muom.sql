CREATE Procedure spr_Lapse_Retail_Universe_Detail_muom (@ProductInfo nvarchar(255),    
...
Declare @temp Table(CustomerID nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS, 
OLClassID Int, InvoiceDate Datetime, InvoiceValue Decimal(18, 2))
...
Right Outer Join #OLClassMapping olcm On olcm.CustomerID = Inv.CustomerID
...
(Select Distinct Inv.InvoiceID     
 ...
 Where InvoiceAbstract.InvoiceDate Between @RefFromDate And @RefToDate And InvoiceAbstract.InvoiceType In (1, 3, 4) And InvoiceAbstract.Status & 128 = 0 
  And  IsNull(Customer.ChannelType, 0) = @ChannelID And Items.CategoryID In (Select CategoryID From #tempCategory)    
 Group By InvoiceAbstract.CustomerID, Customer.Company_Name,  dbo.StripDateFromTime(InvoiceAbstract.InvoiceDate)) And 
...
Begin    
 ...
 "Invoice Value" = (Select InvoiceValue From @temp as tmp    
 Where tmp.CustomerID collate SQL_Latin1_General_Cp1_CI_AS  = InvoiceAbstract.CustomerID And    
 dbo.StripDateFromTime(tmp.InvoiceDate) = dbo.StripDateFromTime(InvoiceAbstract.InvoiceDate) And
 IsNull(tmp.OLClassID, 0) = IsNull(olcm.OLClassID, 0)),    
 ...
 InvoiceAbstract.CustomerID Not In (Select InvoiceAbstract.CustomerID    
