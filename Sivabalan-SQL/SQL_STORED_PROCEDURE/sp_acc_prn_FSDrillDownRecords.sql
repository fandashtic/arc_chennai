CREATE Procedure sp_acc_prn_FSDrillDownRecords(@fromdate datetime,@todate datetime ,@parentid  integer,@ReportHeader nVarchar(255) = Null,@Hide0BalAC Int =0)
	as
	...
	Set @TRIAL_BALANCE = dbo.LookupDictionaryItem('Trial Balance',Default) 
	Set @BALANCE_SHEET = dbo.LookupDictionaryItem('Balance Sheet',Default) 
	Set @TRADING_ACCOUNT = dbo.LookupDictionaryItem('Trading - Profit & Loss A/C',Default) 
	...
	Begin
		EXEC sp_acc_prn_FSDrillDownTradingACDetail @fromdate,@todate,@parentid,@Hide0BalAC
	End
	Else If @ReportHeader = @BALANCE_SHEET
	Begin
		EXEC sp_acc_prn_FSDrillDownBalanceSheet @fromdate,@todate,@parentid,@Hide0BalAC
	End
	Else
	BEGIN
	...
	*/
	...
	-- 		select '','Total',@totaldebit,@totalcredit,@fromdate,@todate,0,0,@NONEXTLEVEL
			...
			*/
			--select GroupName,Case When IsNull(Debit,0)=0 then ('Cr' + Cast(Credit as varchar(255))) Else ('Dr' + Cast(Debit as varchar(255))) End from #TempRegister --to match parameters column, extra colorinfo column added
			If @Hide0BalAC = 0
			Begin
