CREATE Procedure sp_save_vanstatementdetail(@Document_ID int,
...
IF @TRACK_INVENTORY = 0		  --No need for update the qty's only insertion
BEGIN
	SET @RETVAL = 1
	...
	begin
		...
		@PTS, @PTR, @ECP, @SPECIALPRICE, @REQUIRED_QUANTITY, @DocSerial, @Serial)
	End
...
BEGIN
	--Required qty avale in Ist batchcode itself means insertion operation finished and exited
 	--else the opr cursor continue to fetch the detail and updates
		  ...
		  Else					--type 0 and 3
			...
			if @TrnType<>2
				Begin
				    ...
				    PTR, ECP, SpecialPrice,TransferQty,VanTransferID,TransferItemSerial)
		...
		BEGIN
		   ...
		   --Godown to van
			...
			Begin
				...
				@SALE_PRICE, @TOTAL_AMOUNT, @BFQTY, @COST, @PTS, @PTR, @ECP, @SPECIALPRICE, 
				@QUANTITY, @DocSerial, @Serial)
			End
