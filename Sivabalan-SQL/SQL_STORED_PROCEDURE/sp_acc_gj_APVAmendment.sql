CREATE procedure sp_acc_gj_APVAmendment(@apvid integer,@BackDate DATETIME=Null)
as
...
Select @RefDocID = IsNull(RefDocID,0) from APVAbstract Where DocumentID = @apvid  
---------------------------Journal Entries For Ameded APV Document-------------------------  
select @apvdate = [APVDate],@amount = [AmountApproved],@partyid=[PartyAccountID],
@otheraccountid = isnull([OtherAccountID],0),@othervalue = isnull([OtherValue],0) from APVAbstract where [DocumentID]= @RefDocID  
...
Values(@documentid,@partyid,@apvdate,@amount,0,@RefDocID,@DOCUMENTTYPE,'Accounts Payable Voucher - Amended',@uniqueid,getdate())
Insert Into #TempBackdatedAccounts(AccountID) Values(@partyid)

declare scanapvdetail cursor keyset for 
select AccountID,Amount from APVDetail where [DocumentID]= @RefDocID
...
begin
	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
	Values(@documentid,@accountid,@apvdate,0,@value,@RefDocID,@DOCUMENTTYPE,'Accounts Payable Voucher - Amended',@uniqueid,getdate())
...
begin
	...
	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
	Values(@documentid,@otheraccountid,@apvdate,@othervalue,0,@RefDocID,@DOCUMENTTYPE,'Accounts Payable Voucher - Amended',@uniqueid,getdate())	
	Insert Into #TempBackdatedAccounts(AccountID) Values(@otheraccountid)

	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
	values(@documentid,@partyid,@apvdate,0,@othervalue,@RefDocID,@DOCUMENTTYPE,'Accounts Payable Voucher - Amended',@uniqueid,getdate())
	Insert Into #TempBackdatedAccounts(AccountID) Values(@partyid)
end
...
begin
	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
	Values(@documentid,@accountid,@apvdate,@value,0,@apvid,@DOCUMENTTYPE,'Accounts Payable Voucher - Amendment',@uniqueid,getdate())
	Insert Into #TempBackdatedAccounts(AccountID) Values(@accountid)
...
insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
Values(@documentid,@partyid,@apvdate,0,@amount,@apvid,@DOCUMENTTYPE,'Accounts Payable Voucher - Amendment',@uniqueid,getdate())
Insert Into #TempBackdatedAccounts(AccountID) Values(@partyid)
...
begin
	...
	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
	Values(@documentid,@partyid,@apvdate,@othervalue,0,@apvid,@DOCUMENTTYPE,'Accounts Payable Voucher - Amendment',@uniqueid,getdate())
	Insert Into #TempBackdatedAccounts(AccountID) Values(@partyid)

	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber],[CreationTime])
	Values(@documentid,@otheraccountid,@apvdate,0,@othervalue,@apvid,@DOCUMENTTYPE,'Accounts Payable Voucher - Amendment',@uniqueid,getdate())
	Insert Into #TempBackdatedAccounts(AccountID) Values(@otheraccountid)
end
