Create Procedure [dbo].[sp_print_InvAbstract_SR](@INVNO INT)     
AS          
...
Inner Join InvoiceAbstract  On  InvoiceAbstract.InvoiceID = InvoiceDetail.InvoiceID 
Left Outer Join Batch_Products On InvoiceDetail.Batch_Code = Batch_Products.Batch_Code
Where InvoiceDetail.InvoiceID = @INVNO And InvoiceDetail.Serial Not In(Select Serial From InvoiceDetail Where InvoiceID = @INVNO And SalePrice = 0 And Product_Code  In
...
From Collections
Inner Join BranchMaster On Collections.BranchCode = BranchMaster.BranchCode
...
"Adjustments" = Replace(dbo.GetAdjustments(cast(InvoiceAbstract.PaymentDetails as int), @INVNO),';',Char(13)),
"TaxDetails" = Replace(dbo.GetTaxDetails (@INVNO),';',Char(13)), 
"InvoiceOutstandingDetail" = Replace(dbo.GetCustomerOutStanding(@InvNo),';',Char(13)), 
...
--For ExciseDuty  
"Excise Duty" = InvoiceAbstract.ExciseDuty,"LT_vat12.5_Total_Value" = dbo.GetInvoiceTaxComponentTotalValue(@INVNO, 1 , 1),"LT_CESS_Total_Value" = dbo.GetInvoiceTaxComponentTotalValue(@INVNO, 2 , 1),"LT_VAT4_Total_Value" = dbo.GetInvoiceTaxComponentTotalValue(@INVNO, 3 , 1),"additional adjustment" = dbo.GetAdjustmentValue(@INVNO, 1),"Write Off" = dbo.GetAdjustmentValue(@INVNO, 2),
"Customer Points" =Isnull(CustomerPoints,0),
"DeliveryDate" = DeliveryDate,
"CurrentInvoicePoints" = IsNull((Select Sum(IsNull(Points, 0)) from tbl_mERP_OutletPoints op
                          Where op.InvoiceID = InvoiceAbstract.InvoiceID And op.Status = 0), 0)
