CREATE procedure sp_Adjust_BatchProducts_FMCG(@StockID integer, @ItemCode nvarchar(50), @BatchCode nvarchar(255), @BatchNumber nvarchar(255), @Qty Decimal(18,6),  @OldVal Decimal(18,6), @NewQty Decimal(18,6), @NewVal Decimal(18,6), @SaveAdjType Integer, @ReconcileDate DateTime, @Damage Int, @ActDamage Integer) 
As
...
IF @ActDamage = 1 
Begin
	SELECT @EXPIRY = Expiry, @PurPrice = Purchaseprice, @SalePrice = SalePrice, @PKD_DATE = PKD, @GRN_ID = GRN_ID, @TAXSUFFERED = TaxSuffered    
	...
	SalePrice, Damage, DamagesReason, PKD, GRN_ID, TaxSuffered)    
	Values(@BatchNumber, @EXPIRY, @ItemCode, @NewQty, @PurPrice,     
	@SalePrice, 1, 0, @PKD_DATE, @GRN_ID, @TAXSUFFERED)    
