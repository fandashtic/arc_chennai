CREATE procedure [dbo].[sp_print_RetInvItems_fmcg_RUOM_SR](@INVNO INT)    
AS  
...
"Item MRP" = isnull(Items.MRP,0),"LT_30%_Percentage" = dbo.GetInvoiceTaxComponentPercentage(@INVNO, InvoiceDetail.Product_Code, InvoiceDetail.TaxID, 1 , 1), "LT_30%_Value" = Case When (IsNull(Sum(#Temp1.SRQty * InvoiceDetail.STPayable), 0) + IsNull(Sum(#Temp1.SRQty * InvoiceDetail.CSTPayable), 0)) > 0 Then
 dbo.GetInvoiceTaxComponentValue(@INVNO, InvoiceDetail.Product_Code, InvoiceDetail.TaxID, 1 , 1) Else 0.000000 End      
