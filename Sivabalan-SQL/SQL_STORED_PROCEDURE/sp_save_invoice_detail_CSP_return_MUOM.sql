CREATE Procedure sp_save_invoice_detail_CSP_return_MUOM(@INVOICE_ID int,    
...
ELSE    
	...
	IF @CUSTOMER_TYPE = 1    
		...
		Expiry, PurchasePrice, ECP, Company_Price, PKD, TaxSuffered FROM Batch_Products    
		WHERE Product_Code = @ITEM_CODE     
		AND ISNULL(PTS, 0) = @SALE_PRICE And isnull(Free, 0) = @FreeRow    
		And (IsNull(Damage, 0) = @DAMAGES Or @DAMAGES = 1)    
		END    
	ELSE IF @CUSTOMER_TYPE = 2    
		BEGIN    
		...
		Expiry, PurchasePrice, ECP, Company_Price, PKD, TaxSuffered FROM Batch_Products    
		WHERE Product_Code = @ITEM_CODE     
		AND ISNULL(PTR, 0) = @SALE_PRICE And isnull(Free, 0) = @FreeRow    
		And (IsNull(Damage, 0) = @DAMAGES Or @DAMAGES = 1)    
		END    
	ELSE IF @CUSTOMER_TYPE = 3    
		BEGIN    
		...
		Expiry, PurchasePrice, ECP, Company_Price, PKD, TaxSuffered FROM Batch_Products    
		WHERE Product_Code = @ITEM_CODE     
		AND ISNULL(Company_Price, 0) = @SALE_PRICE And isnull(Free, 0) = @FreeRow    
		And (IsNull(Damage, 0) = @DAMAGES Or @DAMAGES = 1)    
		END     
	END    
...
ELSE    
	...
	ELSE    
		...
		FlagWord, SaleID, TaxCode2, CSTPayable, TaxSuffered, TaxSuffered2, ReasonID, UOM, UOMQty, UOMPrice)    
