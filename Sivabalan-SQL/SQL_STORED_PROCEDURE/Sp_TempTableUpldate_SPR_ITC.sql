Create Procedure Sp_TempTableUpldate_SPR_ITC (
...
If Not Exists (Select * From #TempExistColumn Where ColumnName=@Column)
Begin
  ...
  Begin
	  Set @Sql='Alter Table #TempConsolidate Add ' + @Column + ' Decimal(18,6)'
		Exec (@Sql)
		Insert Into #TempExistColumn (ColumnName) Values (@Column) 
  End
End
Else
Begin
	...
	Set @Sql='Update #TempConsolidate Set ' + @Column + ' = IsNull(' + @Column + ',0) + Cast(Cast(IsNull('+ @ColumnValue + ',0) as float) as Decimal(18,6)) Where'
