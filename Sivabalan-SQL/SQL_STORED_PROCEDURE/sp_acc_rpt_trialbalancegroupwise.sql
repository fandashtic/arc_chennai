CREATE Procedure sp_acc_rpt_trialbalancegroupwise (@fromdate datetime,@todate datetime ,@parentid  integer,  @TBType nvarchar(50) = null,@docref integer,@doctype integer,@mode integer,@Info nvarchar(4000) = Null,@State Int=0,@Hide0BalAC Int =0)
 as              
...
-- -- -- -- --  end      
 ...
 IF @TBType = 2    
 Begin    
  /* Set the Leaf Account variable to 5 , so tat the drilldown doesn't     
   go to Ledger Level, since this procedure will not be invoked for    
   Ledger level the condition @mode=@LEAFACCOUNT will not be affected in any way    
  */    
   SET @LEAFACCOUNT =5    
 End    
 Else    
...
BEGIN              
  ...
  */    
     ...
     Begin              
       Select @OpeningBalance=sum(isnull(Opening_Value,0)) from OpeningDetails,Items where Opening_Date=@FromDate And OpeningDetails.Product_Code = Items.Product_Code  
    ...
    End    
     ...
     Begin              
      Select @OpeningBalance = Sum(Case When IsNull(Items.VAT,0) = 1 Then     
...
-- -- --          end      
 ...
 Batch_Assets.ARVID=ARVAbstract.DocumentID and ARVAbstract.ARVDate > @ToDate and AccountID=@AccountID    
  ...
  If @Hide0BalAC = 0
  Begin
	  select 'Account/Group'= GroupName,'Debit'=Debit,'Credit'=Credit,@TBType,               
