CREATE Procedure sp_acc_rpt_FundsFlow_Recursivebalance(@parentid integer,@fromdate datetime, @todate datetime,@balance decimal(18,6) output,@TotalDepAmt decimal(18,6) = 0 output,@OpeningBalance Integer)            
as            
  
/*  
Purpose : To find the Opening / Closing Balance of any group or account  
How to find Balance :   
if @OpeningBalance is 0 then Get the closing balance  
if @OpeningBalance is 1 then Get the Opening balance  
*/  
...
Begin            
 If @AccountID = @CLOSINGSTOCK      
   ...
   if @OpeningBalance = 1     
    Begin  
       ...
       Begin          
         ...
         where Opening_Date= @Fromdate And OpeningDetails.Product_Code = Items.Product_Code  
     End            
       Else            
       Begin         
  ...
  Else if @AccountID=@DEPRECIATION            
   Begin            
   -- Calculate Depreciation but it is made 0 in toplevel  
