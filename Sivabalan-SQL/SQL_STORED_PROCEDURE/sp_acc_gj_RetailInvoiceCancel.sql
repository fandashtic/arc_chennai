CREATE Procedure sp_acc_gj_RetailInvoiceCancel(@InvoiceID INT, @BackDate DateTime=NULL)    
...
Declare @InvoiceMode INT  
/*Variables for Schemes Global Declaration*/  
...
Set @SecondarySchemeExpense = 39 /*SecondarySchemeExpense AccountID*/  
Set @ClaimsRecivable = 10 /*ClaimsRecivable AccountID*/  
...
If @Vat_Exists  = 1    
 Begin    
  ...
  @UserName=UserName,@PaymentDetails=IsNULL(PaymentDetails,N''),@ServiceChargeDetails=ServiceCharge,@AmountRecd=IsNULL(AmountRecd,0),@InvoiceMode=IsNULL(InvoiceType,0)  ,
...
If @InvoiceMode <> 5 And @InvoiceMode <> 6 /*5=SalesReturnSaleable, 6=SalesReturnDamage*/  
 Begin  
  ...
  If IsNULL(@RetailPaymentMode,0) = 0 /*Zero = Credit RetailInvoice*/    
   Begin    
    Set @InternalRemarks = 'Credit Retail Invoice Cancellation'    
   End    
  Else    
   Begin    
    Set @InternalRemarks = 'Retail Invoice Cancellation'    
   End    
  ...
  Execute sp_acc_insertGJ @TransactionID,@SalesTax_Account,@InvoiceDate,@TotalSalesTax,0,@InvoiceID,@InvoiceType,@InternalRemarks,@DocumentNumber                  
    ...
    If @TotalTaxSuffered <> 0    
     Begin -- Entry for Tax Suffered Account                  
      Execute sp_acc_insertGJ @TransactionID,@TaxSuffered_Account,@InvoiceDate,@TotalTaxSuffered,0,@InvoiceID,@InvoiceType,@InternalRemarks,@DocumentNumber                  
     ...
     Begin    
      If @nVatTaxamount <> 0      
       Begin -- Entry for VAT Tax Account                  
        Execute sp_acc_insertGJ @TransactionID,@VAT_Payable,@InvoiceDate,@nVatTaxamount,0,@InvoiceID,@InvoiceType,@InternalRemarks,@DocumentNumber                  
        Insert Into #TempBackdatedAccounts(AccountID) Values(@VAT_Payable)                  
       End    
     End    
    If @SalesValue <> 0                  
     Begin -- Entry for Sales Account                  
      Execute sp_acc_insertGJ @TransactionID,@Sales_Account,@InvoiceDate,@SalesValue,0,@InvoiceID,@InvoiceType,@InternalRemarks,@DocumentNumber                  
      Insert Into #TempBackdatedAccounts(AccountID) Values(@Sales_Account)                  
     End                  
    If @NetValue <> 0                  
     Begin -- Entry for Retail Customer Account                  
      Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,0,@NetValue,@InvoiceID,@InvoiceType,@InternalRemarks,@DocumentNumber                  
  ...
  -----------------------------Entry for Secondary Scheme--------------------------------------    
    ...
    If @SecSchemeValue <> 0                  
     Begin                  
      ...
      Execute sp_acc_insertGJ @TransactionID,@ClaimsRecivable,@InvoiceDate,@SecSchemeValue,0,@InvoiceID,@InvoiceType,@IntlRmksSecSch,@DocumentNumber                  
      -- Entry for SecondarySchemeExpense Account             
      Execute sp_acc_insertGJ @TransactionID,@SecondarySchemeExpense,@InvoiceDate,0,@SecSchemeValue,@InvoiceID,@InvoiceType,@IntlRmksSecSch,@DocumentNumber                  
  ...
  Select @PayModeID = PaymentModeID, @AmtRecd = Value, @ColtnPayMode = PaymentMode, @ServiceCharge = IsNULL(CustomerServiceCharge,0) from Collections Where DocumentID = @CollectionID    
         ...
         Begin    
          ...
          If isnull(@Total,0) <> 0     
           ...
           Commit Tran                  
            ...
            Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,@Total,0,@CollectionID,@CollectionCancelType,'Retail Invoice-Redeem Collection Cancellation',@DocumentNumber                  
            Insert Into #TempBackdatedAccounts(AccountID) Values(@Customer_AccountID)                  
            If isnull(@AdjustedAmount,0) > 0     
             Begin    
              Execute sp_acc_insertGJ @TransactionID,@Gift_Voucher,@InvoiceDate,0,@AdjustedAmount,@CollectionID,@CollectionCancelType,'Retail Invoice-Redeem Collection Cancellation',@DocumentNumber    
              Insert Into #TempBackdatedAccounts(AccountID) Values(@Gift_Voucher)                  
             End    
            If isnull(@Discount,0) > 0     
             Begin    
              Execute sp_acc_insertGJ @TransactionID,@Discount_Account,@InvoiceDate,0,@Discount,@CollectionID,@CollectionCancelType,'Retail Invoice-Redeem Collection Cancellation',@DocumentNumber    
        ...
        Else If IsNULL(@AmtRecd,0) <> 0 And @ColtnPayMode <> 6 /*6 = Credit Note Adjustments*/    
         Begin                  
          ...
          Commit Tran                  
          -- Entry for Customer Account    
          Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,@AmtRecd,0,@CollectionID,@CollectionCancelType,'Retail Invoice-Collection Cancellation',@DocumentNumber       
          -- Entry for Userwise PaymentMode    
          Execute sp_acc_insertGJ @TransactionID,@User_AccountID,@InvoiceDate,0,@AmtRecd,@CollectionID,@CollectionCancelType,'Retail Invoice-Collection Cancellation',@DocumentNumber    
          ...
          If IsNULL(@ServiceCharge,0) <> 0    
           Begin    
            ...
            Execute sp_acc_insertGJ @TransactionID,@ServiceCharge_AccountID,@InvoiceDate,@ServiceCharge,0,@CollectionID,@CollectionCancelType,'Retail Invoice Cancellation-Service Charge',@DocumentNumber                  
            -- Entry for Customer Account    
            Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,0,@ServiceCharge,@CollectionID,@CollectionCancelType,'Retail Invoice Cancellation-Service Charge',@DocumentNumber                  
            Insert Into #TempBackdatedAccounts(AccountID) Values(@Customer_AccountID)          
  ...
  -------------------------------Entry for Shortage Amount-------------------------------------    
      If (@Netvalue + @RoundOffAmount) > @SumAmtRecd                  
       Begin                  
        ...
        -- Entry for Customer Account    
        Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,@AdjustedAmount,0,@InvoiceID,@InvoiceType,'Retail Invoice Cancellation',@DocumentNumber    
        -- Entry for Discount Account(write off)    
        Execute sp_acc_insertGJ @TransactionID,@Discount_Account,@InvoiceDate,0,@AdjustedAmount,@InvoiceID,@InvoiceType,'Retail Invoice Cancellation',@DocumentNumber    
        Insert Into #TempBackdatedAccounts(AccountID) Values(@Discount_Account)                  
        Insert Into #TempBackdatedAccounts(AccountID) Values(@Customer_AccountID)                  
       End       
     End    
 ...
 End  
Else -----------------------------SalesReturn On RetailInvoice-------------------------------  
 Begin  
  -----------------------------------Set Internal Remarks------------------------------------  
  If @InvoiceMode = 5   
   Begin
    Set @InternalRemarks = 'RetailInvoice SalesReturn Saleable Cancellation'  
    Set @Sales_Account = 30
   End
  Else /*Always its 6 Only*/  
   Begin
    Set @InternalRemarks = 'RetailInvoice SalesReturn Damages Cancellation'  
    Set @Sales_Account = 32
   End
  Set @IntlRmksDisc = @InternalRemarks + N'-Discount On Sales' /*Internal Remarks for DiscountOnSales*/   
  Set @IntlRmksRndOff = @InternalRemarks + N'-RoundOff Amount' /*Internal Remarks for RoundOff Amount*/    
  Set @IntlRmksSecSch = @InternalRemarks + N'-Secondary Scheme' /*Internal Remarks for Secondary Scheme*/    
...
OLDIMPL: /*This will handle the Old Implementation*/    
 Exec sp_acc_gj_RetailInvoiceCancelOld @InvoiceID, @BackDate    
EXITPROC: 
