CREATE procedure [dbo].[spr_list_invoices_MUOM_ARU_Chevron]
(
	@FROMDATE datetime,            
 @TODATE datetime,            
...
SELECT  InvoiceID,             
 ...
 "Total SalesTax Value" = TotalTaxApplicable,
	"Segment Code" = Cs.SegmentCode
FROM InvoiceAbstract, Customer, CreditTerm, ClientInformation,Beat,Salesman,CustomerSegment CS          
WHERE   InvoiceType in (1,3) AND InvoiceDate BETWEEN @FROMDATE AND @TODATE AND            
 ...
 InvoiceAbstract.DocSerialType like @DocType         
	And Customer.SegmentID = CS.SegmentID
Order By InvoiceID
