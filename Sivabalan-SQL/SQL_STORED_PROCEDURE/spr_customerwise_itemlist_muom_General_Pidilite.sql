CREATE PROCEDURE [dbo].[spr_customerwise_itemlist_muom_General_Pidilite] (@ProductHierarchy nVarchar(255),                          
...
0 -  (Case @UOM When N'Sales UOM' Then InvoiceDetail.Quantity            
  ...
  When N'UOM2' Then  (case IsNull(UOM2_Conversion,0) when 0 then 0 else InvoiceDetail.Quantity / UOM2_Conversion end)       
  When N'Conversion Factor' Then (case IsNull(Items.ConversionFactor,0)  when 0 then 0 else InvoiceDetail.Quantity *  Items.ConversionFactor end )            
  When N'Reporting Uom' Then   (case IsNull(Items.ReportingUnit,0)  when 0 then 0 else InvoiceDetail.Quantity/ Items.ReportingUnit end)        
End)            
...
End                      
--Else InvoiceDetail.Quantity                      
Else (Case @UOM When N'Sales UOM' Then InvoiceDetail.Quantity          When N'UOM1' Then  (case IsNull(UOM1_Conversion,0) when 0 then 0 else InvoiceDetail.Quantity / UOM1_Conversion end)      
  When N'UOM2' Then  (case IsNull(UOM2_Conversion,0) when 0 then 0 else InvoiceDetail.Quantity / UOM2_Conversion end)       
  When N'Conversion Factor' Then (case IsNull(Items.ConversionFactor,0)  when 0 then 0 else InvoiceDetail.Quantity *  Items.ConversionFactor end )            
  When N'Reporting Uom' Then   (case IsNull(Items.ReportingUnit,0)  when 0 then 0 else InvoiceDetail.Quantity/ Items.ReportingUnit end)        
End)            
