CREATE procedure sp_acc_gj_internalcontracancellation(@contraid int,@BackDate DATETIME=Null)
as
...
Create Table #TempBackdatedinternalcontracancellation(AccountID Int) --for backdated operation
...
BEGIN
	...
	values(@documentid,@fromaccountid,@contradate,@amounttransferred,0,@contraid,@DOCUMENTTYPE,'Internal Contra Cancellation',@uniqueid)
	Insert Into #TempBackdatedinternalcontracancellation(AccountID) Values(@fromaccountid)  
	
	insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],
	[Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])	
	values(@documentid,@toaccountid,@contradate,0,@amounttransferred,@contraid,@DOCUMENTTYPE,'Internal Contra Cancellation',@uniqueid)
	Insert Into #TempBackdatedinternalcontracancellation(AccountID) Values(@toaccountid)  
...
Begin
	...
	Select AccountID From #TempBackdatedinternalcontracancellation
...
End
Drop Table #TempBackdatedinternalcontracancellation
