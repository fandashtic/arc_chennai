Create Procedure mERP_SP_Rpt_GSTPTRUpdate_VATStockReport(@Date nVarchar(25))
AS
BEGIN
SET DATEFORMAT DMY
...
update tbl_mERP_OtherReportsUpload set frequency=0 where ReportName='VAT Stock Report'        
...
Sum(case when Damage=0 then isnull(quantity,0) Else 0 end) as [Saleable Qty] 
,case when TaxType=1 then 'LST' 
when TaxType=2 then 'CST'
when TaxType=3 then 'FLST'
else '' end AS 'Tax Type',TOQ as'TOQ',
Sum(case when Damage<>0 then isnull(quantity,0) Else 0 end) as [Damage Qty],Items.HSNNumber as [HSNNumber]
--,Sum(isnull(Damage,0)) as Damage
From Items, ItemCategories, #temp4, Manufacturer, #tempCategory1, #tmpMfr, #tempItems,Uom,Batch_Products_GST_Backup
Where                                  
...
Batch_Products_GST_Backup.PTS,Batch_Products_GST_Backup.PTR,Batch_Products_GST_Backup.MRPPerPack,Batch_Products_GST_Backup.TaxSuffered,#tempCategory1.[IDs],
