Create Procedure mERP_sp_FindDispatch_SKUSchemeID_New 
...
--		(SOLC.OutLetClass = @TMDField4 Or SOLC.OutLetClass = N'All') 
	...
	Group By S.SchemeID
	    ...
	    select sp.SchemeID,sp.ProductScopeID from tbl_mERP_SchemeProductScopeMap tmsp,SchemeProducts sp
        Where  tmsp.SchemeID in (Select SchemeID From #tmpScheme) And  
			   tmsp.SchemeID = sp.SchemeID And tmsp.ProductScopeID = sp.ProductScopeID
			   And sp.Category = @Category And sp.Sub_Category = @SubCat And Sp.Market_SKU = @MarKetSKU
			   And Sp.Product_Code = @SKUCode And sp.Active = 1
