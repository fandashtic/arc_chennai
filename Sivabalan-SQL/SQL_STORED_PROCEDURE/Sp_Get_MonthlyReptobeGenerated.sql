CREATE Procedure Sp_Get_MonthlyReptobeGenerated(@Curdate DateTime = NULL)
As
	...
	Begin
		Select @LUploadDate=Cast(dbo.stripdatefromtime(Dateadd(d,1,LastUploadDate)) As nvarchar) From Reports_To_Upload Where ReportID = @Grepid
	...
	/* Delete the Capacity report which month end day close not happend*/
	delete from #temp1 where reportid = 1330 and gentodate > (select top 1 dbo.striptimefromdate(lastinventoryupload) from setup)
	--Added for GST Changes
	--delete from #temp1 where reportid = 1421 and gentodate >= (Select dbo.StripdateFromtime(Isnull(GSTDateEnabled,0)) GSTDateEnabled from Setup )
	...
	Node,"Frequency"=2,"DayOfMonthWeek"=DayWeek,"GenReportID"=Grepid,GPFlag, "LatestDocument" = LatestDoc
