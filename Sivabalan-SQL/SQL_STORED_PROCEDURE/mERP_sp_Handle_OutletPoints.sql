create procedure mERP_sp_Handle_OutletPoints(@InvId Int,@QPS int = 0,@Mode int = 0,@nSchemeId int = 0)
as
...
--	Delete from tbl_merp_outletpoints where schemeid = @nschemeid and InvoiceId = @InvId
...
Declare @TmpSKU table (ProductCode nvarchar(255) default '',Quantity decimal(18,6) default 0,SaleAmount decimal(18,6) default 0,TaxAmount decimal(18,6) default 0,Amount decimal(18,6) default 0,UOM1_Conversion decimal(18,6) default 0,UOM2_Conversion decimal(18,6) default 0,MarKetSKU nvarchar(255),SubCategoryID nVarchar(255),SubCat nVarchar(255),CategoryID nVarchar(255),Category nVarchar(255),Uom1_Qty decimal(18,6) default 0,Uom2_Qty decimal(18,6) default 0)
...
--		Begin
			...
			Select SchemeID, ProductScopeID From tbl_mERP_SchemeProductScopeMap --Where SchemeID = @SchemeID
			Where SchemeID In (Select SchemeID From @tmpScheme where SchemeType = 2)
			Open Cur_ScopeID
			Fetch Next From Cur_ScopeID Into @SchemeID, @ScopeID
			While @@Fetch_Status = 0
			Begin
...
Begin
	...
	(Select Sum(Quantity) Quantity,Sum(Amount) Amount,Sum(UOM1_CONVERSION) UOM1_CONVERSION,Sum(UOM2_CONVERSION) UOM2_CONVERSION,Sum(Uom1_Qty) UOM1_QTY,Sum(Uom2_Qty) UOM2_QTY from @tmpSKU where ProductCode in (Select SKUCode from dbo.mERP_fn_GetSplCategoryItems(@SchemeID)) group by ProductCode) x
	Open Cur_SKU 
