CREATE Procedure sp_acc_gj_retailinvoice(@InvoiceID INT, @BackDate DateTime=NULL)    
...
If @InvoiceMode <> 5 And @InvoiceMode <> 6 /*5=SalesReturnSaleable, 6=SalesReturnDamage*/  
 Begin  
  ...
  If IsNULL(@RetailPaymentMode,0) = 0 /*Zero = Credit RetailInvoice*/    
   Begin    
    Set @InternalRemarks = 'Credit Retail Invoice'    
   End    
  Else    
   Begin    
    Set @InternalRemarks = 'Retail Invoice'    
   End    
    ...
    If IsNULL(@RetailPaymentMode,0) <> 0    
     Begin    
          ...
          If IsNULL(@Total,0) <> 0     
           Begin    
            ...
            If IsNULL(@AdjustedAmount,0) > 0     
             Begin    
              Execute sp_acc_insertGJ @TransactionID,@Gift_Voucher,@InvoiceDate,@AdjustedAmount,0,@CollectionID,@CollectionType,'Retail Invoice-Redeem Collection',@DocumentNumber    
              Insert Into #TempBackdatedAccounts(AccountID) Values(@Gift_Voucher)                  
             End    
            If IsNULL(@Discount,0) > 0     
             Begin    
              Execute sp_acc_insertGJ @TransactionID,@Discount_Account,@InvoiceDate,@Discount,0,@CollectionID,@CollectionType,'Retail Invoice-Redeem Collection',@DocumentNumber    
              Insert Into #TempBackdatedAccounts(AccountID) Values(@Discount_Account)                  
             End    
            -- Entry for Customer Account    
            Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,0,@Total,@CollectionID,@CollectionType,'Retail Invoice-Redeem Collection',@DocumentNumber                  
            Insert Into #TempBackdatedAccounts(AccountID) Values(@Customer_AccountID)                  
           End    
         End    
      ...
      Set @GroupID = 19 -- CashInHand AccountGroup                   
          ...
          Commit Tran                  
          -- Entry for Userwise PaymentMode    
          Execute sp_acc_insertGJ @TransactionID,@User_AccountID,@InvoiceDate,@AmtRecd,0,@CollectionID,@CollectionType,'Retail Invoice-Collection',@DocumentNumber    
          -- Entry for Customer Account    
          Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,0,@AmtRecd,@CollectionID,@CollectionType,'Retail Invoice-Collection',@DocumentNumber                  
          ...
          If IsNULL(@ServiceCharge,0) <> 0    
           Begin    
            ...
            Commit Tran                  
            -- Entry for Customer Account    
            Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,@ServiceCharge,0,@CollectionID,@CollectionType,'Retail Invoice-Service Charge',@DocumentNumber                  
            -- Entry for Service Charge Account    
            Execute sp_acc_insertGJ @TransactionID,@ServiceCharge_AccountID,@InvoiceDate,0,@ServiceCharge,@CollectionID,@CollectionType,'Retail Invoice-Service Charge',@DocumentNumber                  
            Insert Into #TempBackdatedAccounts(AccountID) Values(@Customer_AccountID)                  
  ...
  -------------------------------Entry for Shortage Amount-------------------------------------    
      If (@Netvalue + @RoundOffAmount) > @SumAmtRecd                  
       Begin                  
        ...
        Execute sp_acc_insertGJ @TransactionID,@Discount_Account,@InvoiceDate,@AdjustedAmount,0,@InvoiceID,@InvoiceType,'Retail Invoice',@DocumentNumber    
        -- Entry for Customer Account    
        Execute sp_acc_insertGJ @TransactionID,@Customer_AccountID,@InvoiceDate,0,@AdjustedAmount,@InvoiceID,@InvoiceType,'Retail Invoice',@DocumentNumber    
        Insert Into #TempBackdatedAccounts(AccountID) Values(@Discount_Account)                  
        Insert Into #TempBackdatedAccounts(AccountID) Values(@Customer_AccountID)                  
       End                  
     End    
...
Else -----------------------------SalesReturn On RetailInvoice-------------------------------  
 Begin  
  ...
  If @InvoiceMode = 5   
   Begin
    Set @InternalRemarks = 'RetailInvoice SalesReturn Saleable'  
    Set @Sales_Account = 30
   End
  Else /*Always its 6 Only*/  
   Begin
    Set @InternalRemarks = 'RetailInvoice SalesReturn Damages'  
...
OLDIMPL: /*This will handle the Old Implementation*/    
 Exec sp_acc_gj_RetailInvoiceOld @InvoiceID, @BackDate    
EXITPROC: 
