Create Procedure sp_Get_GGRR_TargetvsActual_Daily(@Dtldata Nvarchar(Max),@fromdate datetime)
As
Begin
	Set DateFormat DMY
...
/* For Inculede Items */
	...
	While @@fetch_status =0
		Begin
			...
			Begin
				...
				Select Product_Code From #TmpTmpGGDRSKUDetails where ProdDefnID = @ProdDefnID  And CategoryGroup in (Select Distinct GroupName From @TmpCatGroup)
			End
			Else If @Product <> 'All'
			Begin
				If @level = 2
				Begin
					...
					Insert Into #TmpItems(Product_Code)
					Select Product_Code From #TmpTmpGGDRSKUDetails where ProdDefnID = @ProdDefnID And Division = @Product
				End
				Else If @level = 3
				Begin
					...
					Delete From #TmpItems
					Insert Into #TmpItems(Product_Code)
					Select Product_Code From #TmpTmpGGDRSKUDetails where ProdDefnID = @ProdDefnID And SubCategory = @Product
				End
				Else If @level = 4
				Begin
					Delete From #TmpItems
					Insert Into #TmpItems(Product_Code)
					Select Product_Code From #TmpTmpGGDRSKUDetails where ProdDefnID = @ProdDefnID And MarketSKU = @Product
				End
				Else If @level = 5
				Begin
					Delete From #TmpItems
					Insert Into #TmpItems(Product_Code)
					Select Product_Code From #TmpTmpGGDRSKUDetails where ProdDefnID = @ProdDefnID And Product_Code = @Product
				End
			End
			...
			from #TmpGGDRData G, #TmpItems TI,Items I	
			Where I.Product_Code = G.SystemSKU And TI.Product_Code = I.Product_Code 
			And Convert(Nvarchar(10),InvoiceDate,103) Between @A_Fromdate And @A_ToDate
	...
	Select 	[ProductCode],[Product Description],
		(Case
		...
		isnull([Actual],0) as [Actual], ProdDefnID From #TmpOut
