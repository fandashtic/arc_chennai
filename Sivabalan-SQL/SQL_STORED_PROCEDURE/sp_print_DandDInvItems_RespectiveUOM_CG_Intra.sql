Create Procedure [dbo].[sp_print_DandDInvItems_RespectiveUOM_CG_Intra](@DnDID INT,@MODE INT=0)
AS
Begin
...
"Gross Amount" = Case When Duplicate = 1 then isnull(cast(Case Sum(DD.UOMRFAQty  * DD.UOMPTS )
When 0 then	NULL
...
"Cess Rate" = 	Case When Duplicate = 1 then ''
Else  Cast(Cast(max(#TempTaxDet.CESSPer  ) as decimal(18,2)) as nvarchar)  End,

"Cess Amt" = 	Case When Duplicate = 1 then ''
Else  Cast(Cast(Sum(#TempTaxDet.CESSAmt  ) as decimal(18,2))  as nvarchar)  End,
...
Else Cast(Cast(sum(#TempTaxDet.CGSTAmt  ) as decimal(18,2))as nVarchar)
