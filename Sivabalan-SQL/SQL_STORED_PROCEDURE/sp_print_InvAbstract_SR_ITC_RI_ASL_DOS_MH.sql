...
Create Procedure [dbo].[sp_print_InvAbstract_SR_ITC_RI_ASL_DOS_MH](@INVNO INT)     
AS       
Set dateformat DMY   
...
Declare @PointsEarned as int--nvarchar(10)
Declare @TotPointsEarned as int--nvarchar(10)
...
Declare @WDTin as Nvarchar(15)
Declare @WDLBT as Nvarchar(30)
Declare @CustTin as Nvarchar(15)
Declare @CustLBT as Nvarchar(30)
Declare @WDTin_LBT as Nvarchar(50)
Declare @CustTin_LBT as Nvarchar(50)

select @WDTin = cast( ltrim(rtrim(isnull(Tin_Number,''))) as nvarchar(15)), @WDLBT = cast( ltrim(rtrim(isnull(LocalizedName,''))) as nvarchar(30)) from Setup

Set @CustCode=''
Select @CustCode=CustomerID from InvoiceAbstract where InvoiceID=@InvNo

select @CustTin = cast( ltrim(rtrim(isnull(Tin_Number,''))) as nvarchar(15)), @CustLBT = cast( ltrim(rtrim(isnull(Alternate_Name,''))) as nVarchar(30)) from customer where customerID=@CustCode

if (len(@WDTin) > 0 and len(@WDLBT) > 0 )
	set @WDTin_LBT = 'TIN:' + @WDTin + @WDLBT
else if (len(@WDTin) > 0)
	set @WDTin_LBT = 'TIN:' + @WDTin 
else if (len(@WDLBT) > 0)
	Begin
	If (substring(@WDLBT,1,1) = '\' or substring(@WDLBT,1,1) ='/' )
		set @WDTin_LBT = substring(@WDLBT,2,len(@WDLBT))
	else
		set @WDTin_LBT = @WDLBT
	End

if (len(@CustTin) > 0 and len(@CustLBT) > 0 )
	set @CustTin_LBT = 'TIN:' + @CustTin + @CustLBT
else if (len(@CustTin) > 0)
	set @CustTin_LBT = 'TIN:' + @CustTin 
else if (len(@CustLBT) > 0)
	Begin
	If (substring(@CustLBT,1,1) = '\' or substring(@CustLBT,1,1) ='/' )
		set @CustTin_LBT = substring(@CustLBT,2,len(@CustLBT))
	else
		set @CustTin_LBT = @CustLBT
	End
...
exec sp_print_RetInvItems_RespectiveUOM_SR_ITC_MH @INVNO,1
--Select  1
...
----And InvoiceDetail.Serial Not In(Select Serial From InvoiceDetail Where InvoiceID = @INVNO And SalePrice = 0 And Product_Code  In
----(Select Product_Code From InvoiceDetail Where InvoiceID = @INVNO and SalePrice <>0))
--Group By 
----InvoiceDetail.Serial,
--InvoiceDetail.Product_Code, InvoiceDetail.Batch_Number,         
--InvoiceDetail.SalePrice, 
----CAST(DATEPART(mm, Batch_Products.Expiry) AS nvarchar) + N'\'         
----+ SubString(CAST(DATEPART(yy, Batch_Products.Expiry) AS nvarchar), 3, 2),        
--InvoiceDetail.MRP, InvoiceDetail.SaleID,InvoiceAbstract.TaxOnMRP
...
"Doc Ref" = CASE InvoiceType WHEN 1 THEN Inv.Prefix WHEN 3 THEN InvA.Prefix WHEN 4 THEN SR.Prefix WHEN 5 THEN SR.Prefix END + CAST(DocumentID AS nvarchar) +'/'+ InvoiceAbstract.DocReference ,
"Serial No" = CASE InvoiceType WHEN 1 THEN Inv.Prefix WHEN 3 THEN InvA.Prefix WHEN 4 THEN SR.Prefix WHEN 5 THEN SR.Prefix END + CAST(DocumentID AS nvarchar),
...
"TaxDetailsWithBreakup" = Replace(dbo.GetTaxDetailsWithBreakup_DOS (@INVNO),';',Char(13)), 
"TaxDetails" = Replace(dbo.GetTaxDetails_DOS (@INVNO),';',Char(13)), 
"InvoiceOutstandingDetail" = Replace(dbo.GetCustomerOutStanding_DOS(@InvNo),';',Char(13)), 
...
"Rounded Net Value" = Case InvoiceAbstract.InvoiceType When 4 Then 0 - (NetValue + RoundOffAmount) Else NetValue + RoundOffAmount End,
"Net Amount Payable" = cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then 0 - ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as Nvarchar(10)),
...
"InvoiceTotals" = 
				  '|Inv.Sch.Disc.@ ' + space(5-len(Cast(Cast(isnull(DiscountPercentage,0) as decimal(18,2)) as nvarchar(10)))) +
									 + Cast(Cast(isnull(DiscountPercentage,0) as decimal(18,2)) as nvarchar(10)) + ' : ' +
									 + Space(11-len(cast(cast(isnull(DiscountValue,0) as decimal(18,2)) as nvarchar(10)))) +
									 + Cast(cast(isnull(DiscountValue,0) as decimal(18,2)) as nvarchar(10))  +  Char(13) + Char(10) + 
				  '|Trade Disc.@   ' + space(5-len(Cast(cast(isnull(AdditionalDiscount,0) as decimal(18,2)) as nvarchar(10)))) +
									 + Cast(cast(isnull(AdditionalDiscount,0) as decimal(18,2)) as nvarchar(10))  + ' : ' +
									 + space(11 - Len(cast(cast(isnull(AddlDiscountValue,0) as decimal(18,2)) as nvarchar(10)))) + 
								     + Cast(cast(isnull(AddlDiscountValue,0) as decimal(18,2)) as nvarchar(10))  + Char(13) + Char(10) +
				  '|Credit Adj.          :' + Space(12-len(cast(cast(isnull(@AdjustedValue,0) as decimal(18,2)) as Nvarchar(10))))
										    + cast(cast(isnull(@AdjustedValue,0) as decimal(18,2)) as Nvarchar(10)) + Char(13) + Char(10) +	
				  '|Round off Amt.       :' + Space(12-len(cast(cast(isnull(RoundOffAmount,0) as decimal(18,2)) as Nvarchar(10)))) 
										    + cast(cast(isnull(RoundOffAmount,0) as decimal(18,2)) as Nvarchar(10)) + Char(13) + Char(10) +	
				  '|Net Amt. Payable     :' + Space(14-Len(cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then 0 - ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as Nvarchar(10)) + Char(13) + Char(10)))
											+ cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then 0 - ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as Nvarchar(10)) + Char(13) + Char(10),
...
"TIN_LBT" = @CustTin_LBT ,
"WDTIN_LBT" = @WDTin_LBT ,
"DL 20" = Isnull(DLNumber,''),"DL 21" = IsNull(DLNumber21,'')
...
inner join Salesman2 on InvoiceAbstract.Salesman2 = Salesman2.SalesmanID
