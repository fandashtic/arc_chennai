CREATE PROCEDURE sp_Inv_Update_Others_MultiSplCatScheme(@RowNo int, @InvoiceID int,                   
...
--Update SPLCATSCHEMEID column to zero for multiple special category scheme selection  
--b'cos the splcatschemeid is stored to the separate table  
...
Select @SECONDARY_SCHEME = IsNull(SecondaryScheme,0) from Schemes Where SchemeID = @SPLCATSCHEMEID                    
 ...
 Insert Into SchemeSale(Product_Code, Quantity, Free, Value, Cost, Type, InvoiceID, Claimed, Pending, Flags, SpecialCategory, Serial,Sno)                     
 Values(@ITEM_CODE, 0, @ORIGINAL_QTY, @SALE_PRICE * @ORIGINAL_QTY, @SCHEME_COST, @SPLCATSCHEMEID, @INVOICEID, 0, @ORIGINAL_QTY, @SECONDARY_SCHEME, 1, @RowNo,@Sno)                    
END                    
