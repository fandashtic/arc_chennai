Create Procedure [dbo].[Sp_GGRRFinalDataPost_DayClose](@FromDate DateTime = Null,@ToDate DateTime = Null,@Receive Int = Null,@IsRepost Int =0)
As
Begin
Set DateFormat DMY
...
If Exists(Select Top 1 'x' From TmpGGDRSKUDetails Where ProdDefnID in (Select Distinct ProdDefnID From GGDRoutlet Where ReportFromdate >= @MinFromdate and ReportTodate <= @MaxTodate) And ((Isnull(MarketSKU,'') = '' Or Isnull(SubCategory,'') = '' Or Isnull(Division,'') = '' Or Isnull(CategoryGroup,'') = '')))
Begin
--Exec mERP_sp_RefereshGGDRSKUList

	/*Start Optim1*/
	Create Table #ProdDefnIDList (ProdDefnID Int)
	Create Table #TmpExNULItems (ProdDefnID Int,Product_Code Nvarchar(4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL)	

	Insert Into #ProdDefnIDList
	Select Distinct ProdDefnID  From TmpGGDRSKUDetails Where ProdDefnID in (Select Distinct ProdDefnID From GGDRoutlet Where ReportFromdate >= @MinFromdate and ReportTodate <= @MaxTodate) And ((Isnull(MarketSKU,'') = '' Or Isnull(SubCategory,'') = '' Or Isnull(Division,'') = '' Or Isnull(CategoryGroup,'') = ''))
	If IsNull(@OCGFlag,0) = 1
	Begin
		Delete From TmpGGDRSKUDetails Where ProdDefnID In (select Distinct ProdDefnID From #ProdDefnIDList)		
		Insert Into #TmpExNULItems(ProdDefnID,Product_Code)		
		...
		Where GGP.ProdDefnID  In (select Distinct ProdDefnID From #ProdDefnIDList)		
		And IsNull(OCGP.Exclusion,0) = 0
		And OCGP.SystemSKU Not In (Select Product_Code From #TmpExNULItems Where ProdDefnID = GGP.ProdDefnID)
	End
	Else
	Begin
		Delete From TmpGGDRSKUDetails Where ProdDefnID In (select Distinct ProdDefnID From #ProdDefnIDList)		
		...
		Where GGP.ProdDefnID  In (select Distinct ProdDefnID From #ProdDefnIDList)		
		And SysSKU.Product_Code Not In (Select Product_Code From #TmpExNULItems Where ProdDefnID = GGP.ProdDefnID)		
	End

	Drop Table #TmpExNULItems
	Drop Table #ProdDefnIDList
	...
	Where T.Product_code=G.SystemSKU And (IsNull(G.Division,'')='' Or ISNULL(G.SubCategory,'')='' Or IsNull(G.MarketSKU,'')='' )
	/*End Optim1*/
...
Select Distinct BS.SalesManId,BS.BeatID,BS.CustomerID 
From Beat_SalesMan BS, #TmpDS S, #TmpBeat B, 
...
Update DayCloseModules set DayCloseDate=@ToDate where module='GGDR Final Data'
