CREATE PROCEDURE FixOpening_Itemwise_Datewise (
...
Set DateFormat dmy
Set @ToDate = DateAdd(s, 0 - 1, DateAdd(hh, 24, dbo.StripDateFromTime(@FirstTransactionDate)))  
...
AND ConversionAbstract.ConversionType = 1
	...
	If Exists (Select * from PostItemOpening Where OpeningDate = @FirstTransactionDate)
	Update PostItemOpening 
	Set Purchase = IsNull(@Purchases,0)  - IsNull(@FreePurchases,0),
	FreePurchase = IsNull(@FreePurchases,0),
	TotalSalesReturn = IsNull(@SalesReturnSaleable,0),
	SalesReturnSaleable = IsNull(@SalesReturnSaleable,0)-IsNull(@SalesReturnDamages,0),
	SalesReturnDamages = IsNull(@SalesReturnDamages,0),
	Issues = IsNull(@Issues,0) - IsNull(@FreeIssues,0),
	FreeIssues = IsNull(@FreeIssues,0),
	PurchaseReturn = IsNull(@PurchaseReturn,0),
	Adjustments = IsNull(@AdjustmentOthers,0),
	StockTransferOut = IsNull(@StockTransferOut,0),
	StockTransferIn = IsNull(@StockTransferIn,0),
	StockDestruction = IsNull(@StockDestructQty,0)
	Where OpeningDate = @FirstTransactionDate

   IF IsNull(@BatchUpdate,0) = 0
   BEGIN  
		Set @OpenDate = DateAdd(d, 1, @FirstTransactionDate)
		If (Select Count(*) from OpeningDetails Where Product_Code = @ITEMCODE And Opening_Date = @OpenDate) > 1
		Begin
		...
		End
		Else If (Select Count(*) from OpeningDetails Where Product_Code = @ITEMCODE And Opening_Date = @OpenDate) = 0
			Insert Into OpeningDetails values (@ITEMCODE, @OpenDate, 0, 0, 0, 0, 0, 0, 0, 0)

		If Not Exists (Select * from PostItemOpening Where OpeningDate = DateAdd(d, 1, @FirstTransactionDate))
...
Select * From PostItemOpening Where OpeningDate = @FirstTransactionDate
