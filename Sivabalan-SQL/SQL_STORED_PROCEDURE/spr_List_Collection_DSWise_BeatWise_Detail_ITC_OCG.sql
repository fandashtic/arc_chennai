Create PROCEDURE [dbo].[spr_List_Collection_DSWise_BeatWise_Detail_ITC_OCG]      
(      
...
If @CATEGORY = '%'         
 Insert Into #tempCategoryName Select Category_Name From ItemCategories where CategoryId in ( select CatId from dbo.fn_GetCGFrmCGtype_Itc_RPT('%', @Hierarchy, @CategoryGrouptype,@InvFromDate,@InvToDate, default))
Else        
...
Select Distinct CatId Into #tmpCatId From dbo.fn_GetCGFrmCGtype_Itc_RPT(@Category_Group, @Hierarchy, @CategoryGrouptype,@InvFromDate,@InvToDate, Default)
...
Create table #ItmCat( IDS Int Identity(1,1), CategoryId Int, Category nvarchar(1000) COLLATE SQL_Latin1_General_CP1_CI_AS,
    GroupName nvarchar(1000) COLLATE SQL_Latin1_General_CP1_CI_AS, GroupId Int)

If @CategoryGrouptype = 'Regular'
BEGIN
    Insert Into #ItmCat( CategoryId, Category, GroupName, GroupId )
    select ItemCategories.CategoryID,ItemCategories.Category,      
