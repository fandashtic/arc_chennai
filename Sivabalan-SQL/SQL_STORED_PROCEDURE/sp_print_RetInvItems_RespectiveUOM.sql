CREATE PROCEDURE sp_print_RetInvItems_RespectiveUOM (@INVNO INT)          
AS          
...
SELECT  
 ...
 "Tax Suffered Desc" = (select Tax_description from Tax where tax_code = InvoiceDetail.TaxID),    
...
-- CAST(DATEPART(mm, Max(Batch_Products.Expiry)) AS nvarchar) + N'\'         
-- + SubString(CAST(DATEPART(yy, Max(Batch_Products.Expiry)) AS nvarchar), 3, 2),        
-- CAST(DATEPART(mm, Max(Batch_Products.PKD)) AS nvarchar) + N'\'        
-- + SubString(CAST(DATEPART(yy, Max(Batch_Products.PKD)) AS nvarchar), 3, 2),        
 ...
 Items.TaxSuffered, Items.Sale_Tax,InvoiceDetail.STCredit, Items.Soldas,
 InvoiceDetail.Serial
-- Having SUM(InvoiceDetail.UOMQty) > 0
