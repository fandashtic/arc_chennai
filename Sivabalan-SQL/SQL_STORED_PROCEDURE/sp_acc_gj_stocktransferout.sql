CREATE Procedure sp_acc_gj_stocktransferout (@DocSerial INT,@BackDate DATETIME=Null)
AS
...
Begin
	...
	Begin
		...
		execute sp_acc_insertGJ @TransactionID,@AccountID,@DocumentDate,@NetValue,0,@DocSerial,@AccountType,"Stock Transfer Out",@DocumentNumber
		Insert Into #TempBackdatedAccounts(AccountID) Values(@AccountID)
	End
	If @PurchaseValue<>0
	Begin
		-- Entry for StockTransferOut Account
		execute sp_acc_insertGJ @TransactionID,@AccountID1,@DocumentDate,0,@PurchaseValue,@DocSerial,@AccountType,"Stock Transfer Out",@DocumentNumber
		Insert Into #TempBackdatedAccounts(AccountID) Values(@AccountID1)
	End
	If @TaxAmount<>0
	Begin
		-- Entry for TaxonStocktransfer Account
		execute sp_acc_insertGJ @TransactionID,@AccountID2,@DocumentDate,0,@TaxAmount,@DocSerial,@AccountType,"Stock Transfer Out",@DocumentNumber
			...
			If @FLSTTaxType in(3,4) 
				Begin
					...
					Begin   
						...
						execute sp_acc_insertGJ @TransactionID,@VAT_Payable,@DocumentDate,0,@nVatTaxamount,@DocSerial,@AccountType,"Stock Transfer Out",@DocumentNumber
