Create Procedure dbo.sp_NOA_AutoPostProcess @Flag int = 0
as 
Begin
	...
	/* If Last day of the month is closed then it should not be processed */
		if Not Exists (Select 'x' from tbl_mERP_PMMaster where PMID in(Select Distinct PMID from TmpPMRepost_TLCNOA Where ParamType = 'NOA')
	...
	Begin
		Insert Into @TempPMID (PMID)
		Select Distinct PMID from TmpPMRepost_TLCNOA Where ParamType = 'NOA'
	End
...
-- Loop 1 Start:
	...
	While @@fetch_status =0
		Begin
			...
			Delete From #TempPMInfo Where ParameterTypeid <> 7
...
-- Loop 2 Start:
			...
			While @@fetch_status =0
				Begin
					Insert Into #TempPMTargetInfo (SalesManid,SalesManName,DSTypeValue,NOACount,PMID,PMDSTypeid,ParamID,CGMapid,TargetDefnID,LastUpdatedDate,AutoPostflag,SaveFlag,MaxPoints)
					Select * from dbo.mERP_FN_get_NOA_TargetDefn_AUTOPOST (@PMID,@ParamID,0)
...
-- Loop 3 Start:
	...
	select PMID,PMDsTypeID,ParamID,SalesManid,NOACount,CGMapid,TargetDefnID,'ITCUser',MaxPoints From #TempPMTargetInfo Where Isnull(AutoPostFlag,0) = 0 and Isnull(LastUpdatedDate ,'') = ''
	Open @cluFinelUpdate
	Fetch Next from @cluFinelUpdate into @PMetricID ,@PMDSTypeID,@PMParamID,@SalesmanID,@NOACount,@DSCGMapID,@PMTargetDefnID,@LogonUser,@MaxPoints
	While @@fetch_status =0
		Begin 
			Exec mERP_sp_Save_DSPMetric_TargetDefn_NOA @PMetricID,@PMDSTypeID,@PMParamID,@SalesmanID,@NOACount,@DSCGMapID,@PMTargetDefnID,@LogonUser,@MaxPoints
			Fetch Next from @cluFinelUpdate into @PMetricID ,@PMDSTypeID,@PMParamID,@SalesmanID,@NOACount,@DSCGMapID,@PMTargetDefnID,@LogonUser,@MaxPoints
		End
...
OUT:
	Update tbl_mERP_PMMaster set AutoPost_NOA = 1 Where PMID in (Select Distinct PMID From @TempPMID)
	Drop Table #TempPMInfo
	Drop Table #TempDSTypeID
	Drop Table #TempPMTargetInfo
THEEND:
	Delete From TmpPMRepost_TLCNOA Where ParamType = 'NOA'
End
