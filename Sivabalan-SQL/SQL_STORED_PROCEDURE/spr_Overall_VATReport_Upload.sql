CREATE procedure [dbo].[spr_Overall_VATReport_Upload]  
(            
	@FromDate Datetime,             
	@ToDate DateTime            
)             
...
Begin
	...
	Select @DayClosed = 0
	select @IsUpload = IsNull(Flag,0) from tbl_mERP_ConfigDetail where ScreenCode ='WLVFLAG' and XMLAttribute ='UPLOAD'   
...
--	Delete From #tmpVATRepDet Where SalesTaxPerc = 0 Or SalesTaxAmount = 0
	...
	Insert Into #tmpFinalVATRep(TaxRate, InputVAT, OutputVAT)
	Select TaxRate, Sum(isnull(InputVAT,0)), Sum(isnull(OutputVAT,0)) From #tmpVATRepAbs Group By TaxRate

	Select @WDCode [WD Code], @WDCode [WD Code],@WDDest [WD Dest],@FromDate [From Date], @ToDate [To Date],
		TaxRate [Tax Percentage], InputVAT [Input VAT], OutputVAT [Output VAT] From #tmpFinalVATRep
