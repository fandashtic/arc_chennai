CREATE procedure [dbo].[sp_insert_billcombo_amendment](@BillId int,@ComboId int,@Locality int,@comboQty Decimal(18,6),@Discount Decimal(18,6),@Flags int,@BillComboId int)    
as    
...
select t1.combo_item_code,t1.component_item_code,"Purchase Price"=case items.purchased_at when 1 then t1.Pts when 2 then t1.ptr end,t1.Received_Quantity,"Tax"=case t1.PurchasePrice when 0 then 0 else Isnull(percentage,0) end,t1.PTS,t1.PTR,t1.ECP,t1.specialPrice,items.taxsuffered,1 as TaxType from       
bill_combo_components t1 ,tax,items where        
...
Begin    
 ...
 if(@Flags=1)    
  set @comboTax=(@comboAmt*(@Tax/100))-@discount          
 else    
