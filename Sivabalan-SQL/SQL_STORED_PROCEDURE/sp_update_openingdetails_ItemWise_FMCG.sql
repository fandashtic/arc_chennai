create PROCEDURE sp_update_openingdetails_ItemWise_FMCG(@OPENING_DATE datetime, @ItemCode nvarchar(100))        
AS    
...
Where VanStatementDetail_Copy.Product_Code = @ItemCode
     ...
     and VanStatementDetail_Copy.Batch_Code=Batch_Products_Copy.Batch_Code 
...
/* Service module Issue opening detail*/  
 ...
 begin   
  ...
  Where Batch_Products_Copy.Product_Code = @ItemCode 	   
        and (Isnull(a.Status, 0) & 128) = 0 
        and (Isnull(JobcardAbstract.Status, 0) & 128) = 0  
 end  

SELECT @Product_Code = BPC.Product_Code, 
       @OpeningQty = SUM(BPC.Quantity), 
       @OpeningValue = SUM(ISNULL(BPC.Quantity, 0) * ISNULL(BPC.PurchasePrice, 0)),    
       @TaxSuffered= (Sum(Case IsNull(BPC.ApplicableOn,0)   
...
--Calculation for CST TaxSuffered Percentage  
       @CSTTaxSuffered = Case When (Select IsNull(Vat,0) from Items Where Items.Product_Code=BPC.Product_Code) = 1  
                         Then (Sum(Case IsNull(BPC.Vat_Locality,0) 
                         When 2 Then DBO.FN_GetTaxSufferedAmt_FMCG(IsNull(BPC.PurchasePrice,0), IsNull(Items.MRP,0), IsNull(BPC.Quantity, 0), IsNull(BPC.TaxSuffered,0), IsNull(BPC.ApplicableOn, 0), IsNull(BPC.PartOfPercentage, 0), BPC.Batch_Code) Else 0 End) /    
                         Case When Sum(IsNull(BPC.Quantity, 0) *  IsNull(BPC.PurchasePrice, 0)) = 0 Then 1       
                         Else Sum(IsNull(BPC.Quantity, 0) *  IsNull(BPC.PurchasePrice, 0)) End)*100   
                         Else 0 End   
