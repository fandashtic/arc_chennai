CREATE procedure sprc_get_items_under_beat    
  (  @BeatId int,     
    @FromDate Datetime,    
      @ToDate Datetime  )    
AS    
...
where  InvoiceAbstract.InvoiceId = InvoiceDetail.InvoiceID and    
 ...
 InvoiceAbstract.InvoiceType in (2)  and    
...
-- select  "Product_code" = Items.Product_Code ,     
--  "ProductName" = Items.ProductName,    
--  "SalesValue" =  isnull(sum(dispatchDetail.SalePrice * DispatchDetail.Quantity),0) ,    
--  "SalesQty (UOM)" = cast(isnull(sum(DispatchDetail.Quantity),0) as decimal (18,2)),    
--  "StockQty (CF)" = cast (ISNULL(SUM(DispatchDetail.Quantity * Items.ConversionFactor),0) as decimal (18,2)) ,    
--  "StockQty (RU)" = cast ( SUM(DispatchDetail.Quantity / (case ISNULL(Items.ReportingUnit, 0)      
--   when 0 then 1 else ISNULL(Items.ReportingUnit, 0) end)) as decimal(18,2) )     
-- from DispatchDetail, Items ,DispatchAbstract,beat_salesman    
-- where  DispatchAbstract.DispatchId = DispatchDetail.DispatchID and    
--  DispatchAbstract.CustomerID = beat_salesman.CustomerID and
-- Isnull(beat_salesman.BeatID, 0) = @BeatId and    
--  DispatchAbstract.DispatchDate between @FromDate and  @ToDate and     
--  Items.Product_Code = DispatchDetail.Product_Code  and    
-- (DispatchAbstract.Status & 64) = 0   
-- group by Items.Product_Code,     
--  Items.ProductName     
...
select  "Product_code" = #Sales.Product_Code ,     
 ...
 "SalesQtyUOM" = cast(isnull(sum(#Sales.SalesQtyUOM),0) as nvarchar) + N' ' + isnull((SELECT Description From UOM WHERE UOM = Items.UOM),N'') ,    
 "SalesQtyCF" = cast(isnull(sum(#Sales.SalesQtyCF),0 ) as nvarchar)  + N' ' + isnull((SELECT ConversionUnit From ConversionTable WHERE ConversionID = Items.ConversionUnit),N'') ,    
 "SalesQtyRU" = cast(isnull(sum(#Sales.SalesQtyRU),0) as nvarchar) + N' ' + isnull((SELECT Description From UOM WHERE UOM = Items.ReportingUOM),N'')    
from  #Sales ,  items    
