Create Procedure mERP_sp_Get_SchemeRFAClaimInfo_Expiry
As    
Begin    
 SET DATEFORMAT DMY 
 ...
 Insert Into #tmpSchemeDetail Select SA.SchemeID, SA.Description, SA.ActivityCode,      
  ...
  BEGIN
	  ...
	  --Where --(datediff(d,dateadd(m,6,PayoutTo),getdate())) > 0
  ...
  BEGIN
	  ...
	  PayoutFrom as PayoutPeriodFrom, PayoutTo as PayoutPeriodTo, SchType, PayoutID       
	  From #tmpSchemeDetail
	  -- To display only expired schemes, additional condition is validated below    
	  Where (datediff(d,dateadd(m,@Expiry,PayoutTo),getdate())) > 0
	  --And PayoutTo between dbo.stripdatefromtime(getdate()) and dateadd(m,-@Expiry,PayoutTo)
	  Order By ActivityCode, PayoutFrom, PayoutTo    
  END
