Create Procedure mERP_spr_GGRR_Target_UploadXML(@FromDate Datetime,@ToDate Datetime,@RecdDSType Nvarchar(4000),@DS Nvarchar(4000),@Beat Nvarchar(4000))
As
Begin
	Set DateFormat DMY
	...
	Begin
		IF ((Select dbo.StripTimeFromDate(LastInventoryUpload) From Setup) >= dbo.StripTimeFromDate(@ToDate) and (Select  dbo.StripTimeFromDate(DayCloseDate) From DayCloseModules Where Module = 'GGDR Final Data') >= dbo.StripTimeFromDate(@ToDate))
			Set @DayClosed = 1
	End
	IF @DayClosed = 0
		GoTo OvernOut

	Select @GGDRMonth = CAST(DATENAME(Month,@ToDate) as nvarchar(3)) + '-' + Right(Year(@ToDate),4)
	...
	Select * Into #TmpGGRRFinalData From GGRRFinalData Where [Month] = @GGDRMonth
	...
	Create Table #TmpOutput(ID Int Identity(1,1), DetailID nvarchar(500) COLLATE SQL_Latin1_General_CP1_CI_AS,
	...
	Select DetailID, ProdDefnID, Months, CustomerID, CustomerName, DSID, DSName, DSType, Convert(Datetime, LastDayCloseDate, 103) From #TmpAbstract --,Points,Flag  
	Open Cur
...
--		TargetUOM [Target UOM], Actual From #TmpOutput
...
update #TmpOutput set #TmpOutput.Points = #TmpGGRRFinalData.Points,#TmpOutput.Flag = #TmpGGRRFinalData.Flag
	...
	Select "_1" = isnull(@WDDest, ''),
			"_2" = isnull(Months, ''),
			"_3" = isnull(CustomerID, ''),
			"_4" = isnull(CustomerName, ''),
			"_5" = isnull(DSID, ''),
			"_6" = isnull(DSName, ''),
			"_7" = isnull(DSType, ''),
			"_8" = Convert(nVarchar(10),LastDayCloseDate,103),
			"_9" = isnull(ProductCode, ''),
			"_10" = isnull(Target, 0),
			"_11" = isnull(TargetUOM, ''),
			"_12" = isnull(Actual, 0),
			"_13" = ISNULL(Points,0),
			"_14" = ISNULL(Flag,'')			
	From #TmpOutput as Abstract For XML Auto, ROOT ('Root')
