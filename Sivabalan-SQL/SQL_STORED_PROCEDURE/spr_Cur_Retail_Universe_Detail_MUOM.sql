CREATE Procedure spr_Cur_Retail_Universe_Detail_MUOM (@ProductInfo nvarchar(255),                  
       @FromDate Datetime,                  
       @ToDate Datetime)                  
As                  
...
Declare @temp Table(CustomerID nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS, 
	OLClassID Int, InvoiceDate Datetime, InvoiceValue Decimal(18,6))
...
From InvoiceAbstract As Inv right outer join #OLClassMapping olcm on  olcm.CustomerID = Inv.CustomerID    
...
--Select * From @temp Where OLClassID is not null
...
left outer join ConversionTable  on Items.ConversionUnit = ConversionTable.ConversionID     
 ...
 "Date Of Purchase" = dbo.StripDateFromTime(InvoiceAbstract.InvoiceDate),            
 "Qty Of Purchase" = SUM(Quantity) * @UOMFactor,    
 "Conversion Units" = @UOMDescription,            
 ...
 "Product Value" = Sum(InvoiceDetail.Amount)            
-- From InvoiceAbstract, InvoiceDetail, Items, Customer, ConversionTable, #OLClassMapping olcm
