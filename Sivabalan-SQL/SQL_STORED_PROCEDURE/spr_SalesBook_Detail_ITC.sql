CREATE Procedure spr_SalesBook_Detail_ITC(@Inv Int)  
As
Begin
...
Select c.DocumentID, "Collection ID" = c.FullDocID, 
"Doc Ref" = (Case IsNull(c.DocReference,'') When '' Then (Select isnull(ReferenceNumber,'') From InvoiceWiseCollectionAbstract Where CollectionID
		    = (Select CollectionID From InvoiceWiseCollectionDetail Where DocumentID = c.DocumentID)) Else C.DocReference End),   
