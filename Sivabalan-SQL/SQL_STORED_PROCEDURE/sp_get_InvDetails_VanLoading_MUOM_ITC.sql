Create PROCEDURE sp_get_InvDetails_VanLoading_MUOM_ITC (@CustomerID NVARCHAR(15), @Inv_No INT)
AS
...
"SalesManName" =IsNull(SM.SalesMan_Name,''),
...
,dbo.fn_GetTaxCode(IsNull(MAX(InvDt.TaxSuffered), 0),Max(InvDt.TaxSuffApplicableOn),Max(InvDt.TaxSuffPartOff) ) as TaxCode ,Max(Isnull(InvDt.QuotationID,0)) as QuotationID,
IsNull(InvDt.MultipleSchemeID, '')'MultipleSchemeID', IsNull(InvDt.MultipleSplCatSchemeID, '')'MultipleSplCatSchemeID',
IsNull(InvDt.TOTSCHEMEAMOUNT, 0)'TOTSCHEMEAMOUNT', IsNull(InvDt.MultipleSchemeDetails, '')'MultipleSchemeDetails',
IsNull(InvDt.MultipleSplCategorySchDetail, '')'MultipleSplCategorySchDetail', InvDt.Serial,
"MultipleRebateID" = isNull(MultipleRebateID,''), "RebateRate" = isNull(RebateRate,0),
...
,"MRPPerPack" = isnull(BP.MRPPerPack, 0),"TAXONQTY" = Max(Isnull(InvDt.TAXONQTY,0))
...
Left Outer Join  #TmpBatchChannelPTR BP On BP.Batch_Code = VSD.Batch_Code
...
,InvDt.SalePriceBeforeExciseAmount, InvDt.ExciseDuty, InvDt.OtherCG_Item, IsNull(InvDt.MultipleSchemeID, ''),
IsNull(InvDt.MultipleSplCatSchemeID, ''), IsNull(InvDt.TOTSCHEMEAMOUNT, 0), IsNull(InvDt.MultipleSchemeDetails, ''),
IsNull(InvDt.MultipleSplCategorySchDetail, ''),isNull(MultipleRebateID,''),isNull(RebateRate,0),isNull(MultipleRebateDet,'')
