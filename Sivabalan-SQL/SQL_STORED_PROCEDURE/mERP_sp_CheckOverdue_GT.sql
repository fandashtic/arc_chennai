Create Procedure mERP_sp_CheckOverdue_GT(@CustomerID nVarchar(255), @GroupInfo as nVarchar(2000), @CurrDate as DateTime, @InvoiceID Int = 0)
As
	if (Select IsNull(CreditTerm,-1) From Customer Where CustomerID =@CustomerID) <>-1
	BEGIN
		Set DateFormat DMY	
		...
		Create Table #TempGroup(GroupID Int,GroupName nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS, OverdueValue Decimal(18,2))
		Insert Into #TempGroup(GroupID,OverdueValue) Select ItemValue, 0 From dbo.sp_SplitIn2Rows(@GroupInfo, '|')
		...
		If @GroupInfo = '0'
			Select 0,'All Categories', @OverdueAmount
		Else
		Begin
			...
			Begin
				...
				Begin
					Update #TempGroup set GroupName=(Select GroupName From ProductCategoryGroupAbstract Where GroupID = @TempGroupID)
				...
				Select * From #TempGroup where OverdueValue >0
			End
		End
