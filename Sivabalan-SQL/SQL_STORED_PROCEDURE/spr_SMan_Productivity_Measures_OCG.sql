Create Procedure spr_SMan_Productivity_Measures_OCG
(    
	@SmanName as nVarchar(4000),    
	@SmanType as nVarchar(4000),    
	@CategoryType as nVarchar(50),   
...
Begin  
	...
	IF OBJECT_ID('tempdb..#tmpSman') IS NOT NULL	
		Drop table #tmpSman
	Create Table  #tmpSman  (SalesmanID Int)
	IF OBJECT_ID('tempdb..#tmpSmanType') IS NOT NULL	
		Drop table #tmpSmanType
	--Declare @tmpSmanType As Table (DSType nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS)
	IF OBJECT_ID('tempdb..#tmpSmanType') IS NOT NULL	
		Drop table #tmpSmanType
	Create Table #tmpSmanType  (DSType nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS)
--	Declare @TmpCategory as Table (Category nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,level Int)
	Create Table  #TmpCategory (Category nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS,level Int)
	IF OBJECT_ID('tempdb..#CategoryGroup') IS NOT NULL	
		Drop table #CategoryGroup
	--Declare @CategoryGroup as Table (GroupName nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS)
	Create Table  #CategoryGroup (GroupName nVarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS)
...
-- If CategoryType = All Then.......
	...
	--Declare #TmpLines as Table(
	 IF OBJECT_ID('tempdb..#TmpLines1') IS NOT NULL	
		Drop table #TmpLines1
	  Create Table #TmpLines1(
		InvoiceID Int,
		SalesManID int,
		GroupName nvarchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS  NULL,
		Division nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
...
--	IF OBJECT_ID('tempdb..#TempCatDetails') IS NOT NULL	
--		Drop table #TempCatDetails
--	Create Table #TempCatDetails(
--		categoryID nvarchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
--		Category_Name nvarchar(255) COLLATE SQL_Latin1_General_CP1_CI_AS NULL)
--	
--	insert into #TempCatDetails
--	Select Distinct categoryID,Category_Name  From ItemCategories Where isnull(Level,0) = 2
--	and Categoryid in (select distinct CategoryID from #tmpCustMap)
--
--	Update T Set T.Division = IC.Category_Name From #tmpCustMap T,#TempCatDetails IC
--	Where T.CategoryID = IC.CategoryID 
--
--	delete From #TempCatDetails
--
--
--	insert into #TempCatDetails
--	Select Distinct categoryID,Category_Name  From ItemCategories Where isnull(Level,0) = 3
--	and Categoryid in (select distinct CategoryID from #tmpCustMap)
--	
--	Update T Set T.SubCategory = IC.Category_Name From #tmpCustMap T,#TempCatDetails IC
--	Where T.CategoryID = IC.CategoryID 
...
/* Remove Unwanted Salesman, Division, SubCategory,Category Groups */
	...
	Insert #TmpLines1 (InvoiceID,SalesManID,GroupName,Division,SubCategory,Product_Code)
...
--	Where S.SalesManID = IA.SalesManID
	...
	Begin
		...
		Begin
			...
			Select Distinct Top 1 0,@WDCode,@WDDestCode,Fromdate,Todate,Null,'All Salesman','All DS Types','NA','All Supervisor','All',0,'All' From #tmpSM
		End
	End
	Else
	Begin
	...
	Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
	Select Distinct 1,@WDCode,@WDDestCode,Fromdate,Todate,Null,'All Salesman','All DS Types','NA','All Supervisor',I.GroupName,0,@CatLevel From #tmpSM T,#TmpINV I Order By I.GroupName
	...
	Begin
		...
		Begin
			Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
			Select Distinct 0,@WDCode,@WDDestCode,Fromdate,Todate,SalesManID,SalesMan_name,Dstype,HH,Supervisor,'All',0,'All' From #tmpSM
		End
	End
	Else
	Begin
	...
	Begin
		...
		Begin
			Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
			Select Distinct 2,@WDCode,@WDDestCode,Fromdate,Todate,Null,'All Salesman','All DS Types','NA','All Supervisor',I.Division,0,'Division' From #tmpSM T,#TmpINV I Order By I.Division

			Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
			Select Distinct 3,@WDCode,@WDDestCode,Fromdate,Todate,Null,'All Salesman','All DS Types','NA','All Supervisor',I.SubCategory,0,'SubCategory' From #tmpSM T,#TmpINV I Order By I.SubCategory

			Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
			Select Distinct 2,@WDCode,@WDDestCode,Fromdate,Todate,SalesManID,SalesMan_name,Dstype,HH,Supervisor,Division,DivCount,'Division' From #tmpSM Order By Division

			Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
			Select Distinct 3,@WDCode,@WDDestCode,Fromdate,Todate,SalesManID,SalesMan_name,Dstype,HH,Supervisor,SubCategory,SubCatCount,'SubCategory' From #tmpSM Order By SubCategory
		End
	End
	Else
	Begin
	...
	Insert Into #tmpOutputdata([WDCode],[WD Code],[WD Dest Code],[From Date],[To Date],[Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[Category Type Level])
	Select Distinct 1,@WDCode,@WDDestCode,Fromdate,Todate,SalesManID,SalesMan_name,Dstype,HH,Supervisor,GroupName,CatGrpCount,@CatLevel From #tmpSM Order By GroupName

	If  @level = 3  
	Begin
		Delete From #tmpOutputdata Where [Category Type Level] = 'Division'
	End
	...
	(Select Count(Product_Code) cnt From #TmpLines1)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = 'All'
	And T.WDCode = 0

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select GroupName,Count(Product_Code) cnt From #TmpLines1 Group By GroupName)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = T1.GroupName
	And T.WDCode = 1

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select Division,Count(Product_Code) cnt From #TmpLines1 Group By Division)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = T1.Division
	And T.WDCode = 2

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select SubCategory,Count(Product_Code) cnt From #TmpLines1 Group By SubCategory)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = T1.SubCategory
	And T.WDCode = 3

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select SalesmanID,Count(Product_Code) cnt From #TmpLines1 Group By SalesmanID)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesmanID
	And T.[Category Level] = 'All'
	And T.WDCode = 0

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select SalesmanID,GroupName,Count(Product_Code) cnt From #TmpLines1 Group By SalesmanID,GroupName)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesmanID
	And T.[Category Level] = T1.GroupName
	And T.WDCode = 1

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select SalesmanID,Division,Count(Product_Code) cnt From #TmpLines1 Group By SalesmanID,Division)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesmanID
	And T.[Category Level] = T1.Division
	And T.WDCode = 2

	Update T Set T.[TTL No. of Lines] = T1.Cnt From #tmpOutputdata T,
	(Select SalesmanID,SubCategory,Count(Product_Code) cnt From #TmpLines1 Group By SalesmanID,SubCategory)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesmanID
	And T.[Category Level] = T1.SubCategory
	And T.WDCode = 3
	...
	(Select Count(Distinct Product_Code) cnt From #TmpLines1)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = 'All'
	And T.WDCode = 0

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select GroupName,Count(Distinct Product_Code) cnt From #TmpLines1 Group By GroupName)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = T1.GroupName
	And T.WDCode = 1

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select Division,Count(Distinct Product_Code) cnt From #TmpLines1 Group By Division)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = T1.Division
	And T.WDCode = 2

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select SubCategory,Count(Distinct Product_Code) cnt From #TmpLines1 Group By SubCategory)T1
	Where Isnull(T.[Salesman ID],0) = 0
	And T.[Category Level] = T1.SubCategory
	And T.WDCode = 3

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select SalesManID,Count(Distinct Product_Code) cnt From #TmpLines1 Group By SalesManID)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesManID
	And T.[Category Level] = 'All'
	And T.WDCode = 0

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select SalesManID,GroupName,Count(Distinct Product_Code) cnt From #TmpLines1 Group By SalesManID,GroupName)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesManID
	And T.[Category Level] = T1.GroupName
	And T.WDCode = 1

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select SalesManID,Division,Count(Distinct Product_Code) cnt From #TmpLines1 Group By SalesManID,Division)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesManID
	And T.[Category Level] = T1.Division
	And T.WDCode = 2

	Update T Set T.[TTL Unique Lines Cut] = T1.Cnt From #tmpOutputdata T,
	(Select SalesManID,SubCategory,Count(Distinct Product_Code) cnt From #TmpLines1 Group By SalesManID,SubCategory)T1
	Where Isnull(T.[Salesman ID],0) = T1.SalesManID
	And T.[Category Level] = T1.SubCategory
	And T.WDCode = 3
...
OUT:
	...
	Select Distinct [WDCode],[WD Code],[WD Dest Code], [From Date],[To Date], [Salesman ID],[Salesman Name],[Salesman Type],[Handheld DS],[Supervisor],[Category Level],[Total Outlets],[TTL No. of Bills],[Unique Outlets Billed],[Total Bill Value],[TTL No. of Lines],[TTL Unique Lines Cut],[CAlls Productivity],[Lines Productivity],[Average Bill Value],[No.Of Market Visit Days with HH],[Total No.Of Market Visit Days],[Category Type Level] From #tmpFinalOutputdata Order By [Salesman ID],[WDCode],[Category Type Level] Asc
