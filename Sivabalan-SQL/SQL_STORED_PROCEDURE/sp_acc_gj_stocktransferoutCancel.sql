CREATE Procedure sp_acc_gj_stocktransferoutCancel (@DocSerial INT,@BackDate DATETIME=Null)
AS
...
Begin
	...
	Begin
		...
		execute sp_acc_insertGJ @TransactionID,@AccountID1,@DocumentDate,@PurchaseValue,0,@DocSerial,@AccountType,"Stock Transfer Out Cancellation",@DocumentNumber
		Insert Into #TempBackdatedAccounts(AccountID) Values(@AccountID1)
	End
	If @TaxAmount<>0
	Begin
		-- Entry for TaxonStocktransfer Account
		execute sp_acc_insertGJ @TransactionID,@AccountID2,@DocumentDate,@TaxAmount,0,@DocSerial,@AccountType,"Stock Transfer Out Cancellation",@DocumentNumber
			...
			If @FLSTTaxType in(3,4) 
				Begin
					...
					Begin   
						...
						execute sp_acc_insertGJ @TransactionID,@VAT_Payable,@DocumentDate,@nVatTaxamount,0,@DocSerial,@AccountType,"Stock Transfer Out Cancellation",@DocumentNumber
	...
	Begin
		...
		execute sp_acc_insertGJ @TransactionID,@AccountID,@DocumentDate,0,@NetValue,@DocSerial,@AccountType,"Stock Transfer Out Cancellation",@DocumentNumber
