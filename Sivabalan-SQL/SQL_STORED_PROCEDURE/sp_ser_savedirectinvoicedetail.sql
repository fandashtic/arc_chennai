Create procedure sp_ser_savedirectinvoicedetail          
...
@BounceReason nvarchar(4000) = '',  @Chargeable int = Null, @LabourUnit Decimal(18,6) = 0,
@AppCharge Decimal(18,6) = 0 ,@MaxPerLU Int = 0,
@LabUnitCalc int  = null , @ActualCharge Decimal(18,6) = 0 , @ActualTaskRate Decimal(18,6) = 0 ,         
...
@VatExists int = 0, @CollectTaxSuffered int = 0)        
...
begin        
  ...
  ServiceTax_Percentage, ServiceTax, Amount, NetValue, EstimatedPrice, TaskType,      
  Chargeable , LabourUnit, ApportionedCharge, LabourUnitCalculation, MaxPermissibleLU, ActualTaskRate) Values           
  ...
  @Chargeable ,@LabourUnit, @AppCharge , @LabUnitCalc, @MaxPerLU, @ActualTaskRate)         
  Set @SerialNo = @@Identity          
 ...
 (SerialNo, TaxType, TaxCode, TaxComponent_Code, Tax_Percentage, Rate_Percentage, Tax_Value)        
  ...
  LastUpdatedTime, TaskType, StartWork)         
  ...
  TaskType, JobCardID_Bounced, Bouncecase_Reason, Chargeable, LabourUnit ,ApportionedCharge,       
  LabourUnitCalculation, ActualCharge,ServiceTax_Percentage, ServiceTax, Price, Amount, NetValue,MaxPermissibleLU) Values         
  (@JobCardID, @PRODUCT_CODE, @Product_Specification1, @Type, @TaskID,         
  @TaskType, @BounceJCID, @BounceReason, @Chargeable, @LabourUnit, @AppCharge, @LabUnitCalc, @ActualCharge,  
  @ServiceTax_Percentage, @ServiceTax, @ClaimPrice, @Price, @NETVALUE,@MaxPerLU)        
...
BEGIN         
  ...
  If isnull(@ClaimPrice, 0) = 0         
  begin        
 Select @ClaimPrice = (Case when @CUSTOMERTYPE = 3 then ISNULL(Company_Price, 0) else ISNULL(ECP, 0) end)         
 from Items where Product_Code = @SpareCode        
  ...
  Amount, NetValue, EstimatedPrice, SaleID, ItemDiscountPercentage, ItemDiscountValue, Flag, Claim_Price,         
  Vat_Exists, CollectTaxSuffered_Spares,    
  Chargeable , LabourUnit, ApportionedCharge, LabourUnitCalculation,MaxPermissibleLU) Values           
  ...
  @AMOUNT, @NETVALUE, @ClaimPrice, @SaleID, @ItemDiscountPercentage, @ItemDiscountValue, @Flag, @ClaimPrice,        
  @vatExists, @CollectTaxSuffered,    
  @Chargeable ,@LabourUnit, @AppCharge , @LabUnitCalc,@MaxPerLU)         
  Set @SerialNo = @@Identity        
  
  Insert into JobCarddetail (JobCardID, PRODUCT_CODE, Product_Specification1, Type, TaskID,         
  SpareCode, Quantity, Uom, UomQty, Warranty, WarrantyNo, DateofSale,  
  Price, LSTPayable, CSTPayable, SalesTax, TaxSuffered_Percentage, TaxSuffered,  
  ServiceTax_Percentage, ServiceTax, Amount, NetValue, Vat_Exists, CollectTaxSuffered_Spares,  
  Chargeable, LabourUnit, ApportionedCharge, LabourUnitCalculation,MaxPermissibleLU) Values         
  (@JobCardID, @Product_Code, @Product_Specification1, @Type, @TaskID,         
  @SpareCode, @UOMQty, @UOM, @UOMQty, @Warranty, @WarrantyNo, @DateofSale,  
  @ClaimPrice, @LSTPayable, @CSTPayable, @SaleTaxPercentage,@Tax_SufferedPercentage, @TAXSUFFERED,  
  @ServiceTax_Percentage, @ServiceTax, @Amount, @NetValue, @VatExists, @CollectTaxSuffered,  
  @Chargeable ,@LabourUnit, @AppCharge , @LabUnitCalc,@MaxPerLU)        
...
BEGIN /* @TRACK_INVENTORY = 1 */         
 ...
 Else if @CUSTOMERTYPE = 1 --Stockist
  SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products         
  WHERE Product_Code = @SpareCode AND         
  ISNULL(Batch_Number, '') = @Batch_Number AND         
  ISNULL(PTS, 0) = @SelectedPrice AND (Expiry >= GetDate() OR Expiry IS NULL)         
  And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow        
 Else if @CUSTOMERTYPE = 2 --Retailer
  SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products         
  WHERE Product_Code = @SpareCode AND         
  ISNULL(Batch_Number, '') = @Batch_Number AND         
  ISNULL(PTR, 0) = @SelectedPrice AND (Expiry >= GetDate() OR Expiry IS NULL)         
  And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow        
 Else if @CUSTOMERTYPE = 4 Or  @CUSTOMERTYPE = 5 --
  SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products         
  WHERE Product_Code = @SpareCode AND         
  ISNULL(Batch_Number, '') = @Batch_Number AND         
 ...
 BEGIN        
  ...
  and ISNULL(PTS, 0) = @SelectedPrice AND ISNULL(Quantity, 0) > 0         
 ...
 BEGIN        
  ...
  WHERE Product_Code = @SpareCode         
  and ISNULL(Batch_Number, '') = @Batch_Number         
  and ISNULL(PTR, 0) = @SelectedPrice AND ISNULL(Quantity, 0) > 0         
  AND (Expiry >= GetDate() OR Expiry IS NULL) And ISNULL(Damage, 0) = 0        
  And isnull(Free, 0) = @FreeRow        
 END        
 Else if @CUSTOMERTYPE = 4 or @CUSTOMERTYPE = 5
 ...
 IF @CUSTOMERTYPE = 3        
  ...
  SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products         
  WHERE Product_Code = @SpareCode AND ISNULL(PTS, 0) = @SelectedPrice         
  And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow        
 else if @CUSTOMERTYPE = 2
  SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products         
  WHERE Product_Code = @SpareCode AND ISNULL(PTR, 0) = @SelectedPrice         
  And ISNULL(Damage, 0) = 0 And isnull(Free, 0) = @FreeRow        
 else if @CUSTOMERTYPE = 4 or @CUSTOMERTYPE = 5
  SELECT @TOTAL_QUANTITY = ISNULL(SUM(Quantity), 0) FROM Batch_Products         
...
BEGIN        
 ...
 /* Net = Spare Amount + SaleTaxValue + TaxSuffereValue         
  ...
  Flag, Claim_Price, Vat_Exists, CollectTaxSuffered_Spares,    
  Chargeable , LabourUnit, ApportionedCharge, LabourUnitCalculation, MaxPermissibleLU) Values           
  (@InvoiceID, @PRODUCT_CODE, @Product_Specification1, @Type, @TaskID, @SpareCode,         
...
--    @BatchQty, @UOM, @UOMQty, @Warranty, @WarrantyNo, @DateofSale)        
   ...
   Chargeable , LabourUnit, ApportionedCharge, LabourUnitCalculation, MaxPermissibleLU) Values  
   (@JobCardID, @Product_Code, @Product_Specification1, @Type, @TaskID,         
   @SpareCode, @BatchQty, @UOM, @UOMQty, @Warranty, @WarrantyNo, @DateofSale,  
   @ClaimPrice, @LSTPayable, @CSTPayable, @SaleTaxPercentage,@Tax_SufferedPercentage, @TAXSUFFERED,  
   @ServiceTax_Percentage, @ServiceTax, @Amount, @NetValue, @VatExists, @CollectTaxSuffered,  
   @Chargeable ,@LabourUnit, @AppCharge , @LabUnitCalc, @MaxPerLU)  
