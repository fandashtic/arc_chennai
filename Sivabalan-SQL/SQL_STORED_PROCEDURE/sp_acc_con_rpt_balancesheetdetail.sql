CREATE procedure sp_acc_con_rpt_balancesheetdetail(@fromdate datetime,@todate datetime ,@parentid  integer,@docref integer,@doctype integer,@mode integer,@Info nvarchar(4000) = Null)
as
...
/* The following code is added specifically for handling Opening Stock */  
if @Parentid = @OPENINGSTOCKGROUP and @Mode = @ACCOUNTGROUP  
Begin  
 exec sp_acc_con_rpt_trialbalancegroupwise @fromdate ,@todate  ,@parentid  ,@docref ,@doctype ,@mode ,@Info  
 Goto Quit  
End  
...
BEGIN
	...
	select 'Account/Group'= GroupName,'Debit'=Debit,'Credit'=Credit,'', 'AccountID/GroupID'= CASE WHEN GroupID = 0 then N'' else GroupID end,@fromdate,@todate,DocRef,DocType,ColorInfo,ColorInfo from #TempRegister 
	Drop table #TempRegister
END
ELSE IF @Mode = @SPECIALCASE3 and @parentid=0
BEGIN
	exec sp_acc_con_rpt_tradingac @fromdate,@todate,9
END
Quit:
