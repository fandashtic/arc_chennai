CREATE Procedure sp_acc_gj_ser_IssueSpares(@IssueID INT,@BackDate DateTime = Null)  
AS  
----------------------------Journal entry for Issue Spares-----------------------------------  
...
Select @Value = Sum(IsNull(IssuedQty,0)*IsNull(PurchasePrice,0)) from IssueDetail where IssueID = @IssueID  
...
If @Value <> 0  
 Begin  
  ...
  Execute sp_acc_insertGJ @TransactionID,@BILLSRECEIVABLE,@IssueDate,@Value,0,@IssueID,@ISSUETYPE,'Issue Spares',@DocumentNumber  
  Insert Into #TempBackdatedAccountsIssueSpares(AccountID) Values(@BILLSRECEIVABLE)  
   
  Execute sp_acc_insertGJ @TransactionID,@SALESONDC,@IssueDate,0,@Value,@IssueID,@ISSUETYPE,'Issue Spares',@DocumentNumber  
