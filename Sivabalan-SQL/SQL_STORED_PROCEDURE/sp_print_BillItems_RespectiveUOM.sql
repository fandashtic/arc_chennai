CREATE Procedure sp_print_BillItems_RespectiveUOM(@Bill_ID Int)
As
Begin
...
Inner Join Items On  BillDetail.Product_Code = Items.Product_Code And IRDet.Product_Code = Items.Product_Code
...
Where IRDet.InvoiceID = @RecInvID And BillDetail.BillID = @Bill_ID And GRNDet.GRNID in (Select * from dbo.sp_SplitIn2Rows(@GRNLIST,','))
Group by BillDetail.Batch, BillDetail.Expiry, BillDetail.PKD, BillDetail.Serial, UOM.Description, BillDetail.Product_Code, Items.ProductName, Items.Description
...
Left Outer Join TAX On BillDetail.TaxCode = TAX.Tax_Code
Where BillDetail.BillID = @Bill_ID And GRNDet.GRNID in (Select * from dbo.sp_SplitIn2Rows(@GRNLIST,','))
Group by 
 BillDetail.Batch, BillDetail.Expiry,
