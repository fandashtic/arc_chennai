Create Procedure mERP_spr_CustomerFacingTimeSummaryRpt_ITC(
...
Set @CMonth = Cast(Datepart(yyyy, @DtMonth) As nVarchar) + Substring(@Month,4,2)
Set @i = 0
...
Begin
	...
	If @i = 1
	Begin
		Set @VToDate = Cast((N'1/' + Cast(Datepart(MM, @DtMonth) As nVarchar) + N'/' + Cast(Datepart(yyyy, @DtMonth) As nVarchar)) As Datetime)
		...
		Set @VFromDate = Cast((Cast((Case @FromWeek When 1 Then 1 
			...
			Cast(Datepart(MM, @DtMonth) As nVarchar) + N'/' + Cast(Datepart(yyyy, @DtMonth) As nVarchar)) As Datetime)
...
--		Set @PlannedWorkingDays = @PlannedWorkingDays - (@PlannedWorkingDays/7) 
		...
		Select @WDCode, "WDCode" = @WDCode, "WDDest" = @WDDest, "Month" = @CMonth, "From Week" = @FromWeek, 
			"To Week" = @ToWeek, "FromDate" = Convert(nVarchar(10),@VFromDate,103), "ToDate" = Convert(nVarchar(10),@VToDate,103), "Salesman_Code" = sm.SalesmanID, 
