CREATE procedure sp_acc_gj_paymentamendment(@paymentid integer,@BackDate DATETIME=Null)    
as    
...
@others = ISNULL([Others],0),@expenseid = isnull(ExpenseAccount,0),  
...
begin  
 execute  sp_acc_gj_AmendedOtherPayments @AmendedID,@BackDate  
end  
else   
begin     
...
-- --    set @currentdate = dbo.stripdatefromtime(getdate())    
 ...
 else if @accountid <> 0 and @npaymentmode = 4     
 begin    
  	select @bankaccount = ISNULL([AccountID],0) from bank    
    where [BankID]=@bankid       
...
if @others <> 0 or @expenseid <> 0  
begin  
 execute  sp_acc_gj_AmendmentOtherPayments @Paymentid,@BackDate  
end  
else   
begin      
  select @accountid=ISNULL([AccountID],0)    
