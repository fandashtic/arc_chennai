Create Procedure Sp_PM_GateUOB_Post_Monthly
As
BEGIN --Main
Set Dateformat DMY
...
Set @MonthTo  = DateAdd(dd,-1,Cast('01/' + Right(Convert(nVarChar(10),GETDATE(),103),7)  As DateTime))

IF @LastClosedDate >= @MonthTo
...
And PM.PMID not in (Select Distinct PMID from PM_GateUOB_MonthlyData Where PMMonth = @CurPeriod)
...
Set @MonthFrom = Cast('01/' + Right(Convert(nVarChar(10),@MonthTo,103),7)  As DateTime)

Select * Into #InvAbs From InvoiceAbstract IA
Where dbo.StripTimeFromDate(IA.InvoiceDate) Between dbo.StripTimeFromDate(@MonthFrom)  And dbo.StripTimeFromDate(@MonthTo)
...
Insert Into PM_GateUOB_MonthlyData (PMID,PMDSTypeID,PMParamID,DSID,PMMonth,Outletcount)
...
Update DayCloseModules Set DayCloseDate = (Select Top 1 LastInventoryUpload From Setup) Where Module = 'PM GateUOB Monthly' And Priority = 12
END --Main
