Create procedure mERP_SP_rpt_DSPointsreport_Detail (@DSType nvarchar(4000),@Rptmonth nvarchar(50),@DummyParam nvarchar(2000))
AS
BEGIN
	SET Dateformat DMY
	...
	create table #tmpoutputRpt (DSType nvarchar(4000) COLLATE SQL_Latin1_General_CP1_CI_AS)
	insert into #tmpoutputRpt values (@DSType)
	
	Create Table #final(DSId int,DSType nvarchar(4000),[Category group]  nvarchar(100),[Till Date Points Earned] decimal(18,6))
	...
	[Overall or Focus] nVarchar(100),Frequency nVarchar(25) COLLATE SQL_Latin1_General_CP1_CI_AS,
	...
	Select Distinct DSType From #tmpoutputRpt
...
--					Set @FromDate = (Select (DateAdd(DD, 21,  @FromDate)))
	...
	Create table #DSPMSalesman (SalesManid Int ,Salesman_Name Nvarchar(255),PMID Int,DSTypeValue Nvarchar(255),CurrentdsType Nvarchar(255),TargetStatus int, SalesStatus Int)
...
OvernOut:
	...
	Select * into #finalopt from
	(
	select F.DSID,F.DSID as [DS ID],S.Salesman_Name as [DS Name],T.DSType as [DS Type],F.[Category Group] as 'Cat Group',
	(Select count(distinct dbo.StripTimeFromDate(IA.InvoiceDate)) from invoiceabstract IA where  IsNull(IA.Status,0) & 128 = 0
		And dbo.StripTimeFromDate(IA.InvoiceDate) Between @MonthFirstDate And @TillDate 
		And IA.InvoiceType in(1,3) 
		And F.[Category Group] = dbo.mERP_FN_get_CategoryGroupDesc(IA.InvoiceID,F.[Category Group])
		and IA.salesmanid = F.DSID and IA.DSTypeID in (Select DSTypeID from DSType_Master where DSTypeValue = T.DSType)) as [No. of Days Worked],
	sum(F.[Till Date Points Earned]) as [Variable Points],'' as Fixed,'' as Mobility,1 as Performance_Flag from #tmpoutputRpt T,#final F,Salesman S
	where T.DSType = F.DSType
	And S.SalesmanID = F.DSID
	group by F.DSID,T.DSType,S.Salesman_Name,F.[Category Group] 
	)T

	Delete from #finalopt where isnull([Variable Points],0)=0 and isnull([No. of Days Worked],0) = 0

	Create Table #PCG(GroupID int,GroupName nvarchar(4000))
	insert into #PCG select GroupID,GroupName from ProductCategoryGroupAbstract where groupname in ('GR1','GR2','GR3')

	/* FOR NON PERFORMANCE ALLOTTED SALESMAN */
	insert into #finalopt 
	/* FOR GR1|GR3*/
	Select S.SalesmanID, S.SalesmanID, S.Salesman_Name,@DSType,'GR1|GR3',count(distinct dbo.StripTimeFromDate(IA.InvoiceDate))
	,0,'','',0 From Salesman S, InvoiceDetail ID, InvoiceAbstract IA
	Where IA.SalesmanID = S.SalesmanID
	And IA.InvoiceID =  ID.InvoiceID
	And dbo.StripTimeFromDate(IA.InvoiceDate) Between @MonthFirstDate And @TillDate 
	And IA.InvoiceType in(1,3)
	And isnull(IA.status,0) & 128 = 0
	And (Id.GroupID in (Select GroupID from #PCG where GroupName='GR1') or Id.GroupID in (Select GroupID from #PCG where GroupName='GR3'))
	--And 'GR1|GR3' = dbo.mERP_FN_get_CategoryGroupDesc(IA.InvoiceID,'GR1|GR3')
	And IA.DSTypeID in (Select DSTypeID from DSType_Master where DSTypeValue = @DSType)
	/*
	And S.SalesmanID in (Select SalesmanID from DSType_Details where DSTypeID in 
	(Select DSTypeID from DSType_Master where DSTypeValue = @DSType and Active = 1 And DSType_Master.DSTypeID = IA.DSTypeID))
	*/
--	And S.SalesmanID not in (Select DSID from #finalopt)
	Group by S.SalesmanID, S.Salesman_Name
	UNION
	/* FOR GR2*/
	Select S.SalesmanID, S.SalesmanID, S.Salesman_Name,@DSType,'GR2',count(distinct dbo.StripTimeFromDate(IA.InvoiceDate))
	,0,'','',0 From Salesman S, InvoiceDetail ID, InvoiceAbstract IA
	Where IA.SalesmanID = S.SalesmanID
	And IA.InvoiceID =  ID.InvoiceID
	And isnull(IA.status,0) & 128 = 0
	And dbo.StripTimeFromDate(IA.InvoiceDate) Between @MonthFirstDate And @TillDate 
	And IA.InvoiceType in(1,3)
	And Id.GroupID in (Select GroupID from #PCG where GroupName='GR2')
...
--	And S.SalesmanID not in (Select DSID from #finalopt)
	...
	if @DayCloseFlag = 0
		update #finalopt set [Variable Points] = NULL


	Declare @DID int
	Declare @CGroup nvarchar(2000)
	
	Declare AllInv cursor for select DSID,[Cat Group] from #finalopt where Performance_flag = 1
	open AllInv
	Fetch from AllInv into @DID,@CGroup
	While @@fetch_status = 0
	BEGIN
		Delete from #finalopt where DSID=@DID and [Cat Group]=@CGroup and Performance_flag = 0
		Fetch next from AllInv into @DID,@CGroup
	END
	Close AllInv
	Deallocate AllInv
	
	Select distinct DSID,DSID as[DS ID],[DS Name],[DS Type],[Cat Group],[No. of Days Worked],[Variable Points],Fixed,Mobility from #finalopt
	where isnull([No. of Days Worked],0)>0
