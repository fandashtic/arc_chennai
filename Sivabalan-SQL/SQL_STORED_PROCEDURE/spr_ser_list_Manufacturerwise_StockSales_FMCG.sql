CREATE Procedure spr_ser_list_Manufacturerwise_StockSales_FMCG(@Manufacturer Varchar(2550),
							@FromDate Datetime,
							@ToDate Datetime, 
...
Insert InTo #tmpone(SalesValue) 
Select Sum(Isnull(ServiceinvoiceDetail.Netvalue,0)) - (Sum(Isnull(Price,0) * Isnull(Quantity,0)) * Max(Isnull(ServiceInvoiceDetail.Tax_SufferedPercentage,0)) / 100) - SUM((Isnull(LSTpayable,0) + Isnull(CSTPayable,0)))  
