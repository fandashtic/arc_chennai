...
create Procedure dbo.sp_get_To_DocNo(@Mode int,@TranID nvarchar(255)=N'',@qry nvarchar(255),@Direction int = 0, @BookMark nvarchar(128) = N'')        
as        
...
Set @Query=@Query+N'From InvoiceAbstract,VoucherPrefix vp,VoucherPrefix ivp Where (IsNull(Status,0) & 128)=0 And Balance > 0 And InvoiceType in (1,3) '        
Set @Query=@Query+N'And vp.TranID=''INVOICE'' And ivp.TranID=''INVOICE AMENDMENT'' '+ @DocSerialType +' And LTrim(DocReference) like N'''+@qry+''''        
if @Mode=5        
...
If @TmpStr='D'    
Begin    
Set @Query=N'Select ''D''+cast(DebitID as varchar),DocumentReference,case IsNull(DocSerialType,'''') when '''' then dvp.Prefix else DocSerialType end,''Debit Note'' '         
...
Set @Query=@Query+N'And dvp.TranID=''DEBIT NOTE'' '+ @DocSerialType +' And LTrim(DocumentReference) like N'''+@qry+''''        
End        
