CREATE PROCEDURE [dbo].[spr_list_itemwise_receipt_MUOM](@ITEMCODE nvarchar(2550), @VENDOR nvarchar(2550),     
...
"Conversion Factor" = CAST(CAST(SUM((GRNDetail.QuantityReceived + IsNull(GRNDetail.FreeQty, 0)- GRNDetail.QuantityRejected) * Items.ConversionFactor) AS Decimal(18,6)) AS nvarchar)
 ...
 "Reporting UOM" = Cast(dbo.sp_Get_ReportingQty(SUM((GRNDetail.QuantityReceived + IsNull(GRNDetail.FreeQty, 0)- GRNDetail.QuantityRejected)), Items.ReportingUnit) As nvarchar) + N' ' + 
...
-- 	+ ' ' + CAST((SELECT Description FROM UOM WHERE UOM = Items.ReportingUOM) AS nvarchar)
FROM GRNAbstract
Inner Join GRNDetail on GRNAbstract.GRNID = GRNDetail.GRNID
...
Vendors.Vendor_Name in (select Vendor_Name COLLATE SQL_Latin1_General_CP1_CI_AS from #tmpVen)
...
-- 	+ ' ' + CAST((SELECT Description FROM UOM WHERE UOM = Items.ReportingUOM) AS nvarchar)
FROM GRNAbstract
--, GRNDetail, Items, UOM, ConversionTable, Vendors
Inner Join GRNDetail on GRNAbstract.GRNID = GRNDetail.GRNID
Inner Join Items on GRNDetail.Product_Code = Items.Product_Code
Left Outer Join UOM on Items.UOM = UOM.UOM
Left Outer Join ConversionTable on Items.ConversionUnit = ConversionTable.ConversionID
