Create Procedure mERP_SP_Get_MinQtyItems_SCHTLC_View
...
Declare @MQTYProduct_Code nvarchar(30)

Declare MQtyDetails Cursor For Select Product_code from #InvoiceProdAndUOM
Open MQtyDetails
Fetch From MQtyDetails into @MQTYProduct_Code
While @@Fetch_status=0
Begin
If (Select Count(*) from dbo.mERP_fn_Get_CSProductScope(@SchemeID) where Product_code = Isnull(@MQTYProduct_Code,'') ) = 0
Begin
Delete from #InvoiceProdAndUOM where Product_Code = Isnull(@MQTYProduct_Code,'')
End
Fetch From MQtyDetails into @MQTYProduct_Code
End
Close MQtyDetails
Deallocate MQtyDetails
...
--If (Select isnull(TLCFlag,0) from tbl_merp_schemeabstract where schemeid=@SchemeID)=0
