--exec spr_list_invoices_MUOM_Bottom1 416224
CREATE procedure [dbo].[spr_list_invoices_MUOM_Bottom1](@INVOICEID int)  
AS  
...
SUM(DiscountPercentage) / 100.00))) *  (@TRADEDIS / 100.00))), 2),"Total" = Round(SUM(Amount),2)  
...
.ReportingUnit) - SUM(InvoiceDetail.DiscountValue))
,"ADDL CESS"= dbo.[fn_GetTaxValueByComponent](15,@ADDLCESS)
FROM InvoiceDetail with (nolock), Items  with (nolock),Brand  with (nolock), UOM with (nolock)  
