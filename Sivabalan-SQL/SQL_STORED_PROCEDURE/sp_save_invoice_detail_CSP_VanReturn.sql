...
Create Procedure sp_save_invoice_detail_CSP_VanReturn(@INVOICE_ID int,      
...
BEGIN      
 SET @RETVAL = 1      
   ...
   Amount, PurchasePrice, STPayable, TaxID, FlagWord, SaleID, TaxCode2, CSTPayable, TaxSuffered, TaxSuffered2, ReasonID)      
   VALUES (@INVOICE_ID, @ITEM_CODE, 0, '', @REQUIRED_QUANTITY, @MODIFIED_PRICE, @TAXCODE, @DISCOUNTPERCENTAGE, @DISCOUNTAMOUNT, @AMOUNT,       
   @COST * @REQUIRED_QUANTITY, @STPAYABLE, @TAXID, @FLAG, @SALEID, @TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @REASON_ID)      
   GOTO ALL_SAID_AND_DONE      
END

IF @FREEROW=1 
    SELECT @CNT = Count(*) FROM VanStatementDetail  
    WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, '') = @BATCH_NUMBER  
AND DocSerial = @DOCSERIAL And IsNull(SalePrice,0) = 0 
ELSE  
    SELECT @CNT = Count(*) FROM VanStatementDetail  
    WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, '') = @BATCH_NUMBER  
       AND DocSerial = @DOCSERIAL And IsNull(SalePrice,0) = @SALE_PRICE 
...
BEGIN      
  ...
  IF @DAMAGES = 0      
	...
	INSERT INTO VanStatementDetail(DocSerial, Product_Code, Batch_Code, Batch_Number, 
		Quantity, Pending, SalePrice, Amount, PurchasePrice, BFQty, PTR, PTS, ECP, 
		SpecialPrice, TransferQty, VanTransferID, TransferItemSerial)  
	VALUES (@DOCSERIAL, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER, 		  
		...
		@SPECIAL_PRICE, 0, @DOCSERIAL,0)
	-- To update the Documentvalue in VanStatementAbstract
	UPDATE VanStatementAbstract SET DocumentValue = (SELECT Sum(Amount) From VanStatementDetail WHERE DocSerial = @DOCSERIAL)
	WHERE DocSerial = @DOCSERIAL and (Status & 192) = 0
	END
   INSERT INTO InvoiceDetail(InvoiceID, Product_Code, Batch_Code, Batch_Number,       
	...
	CSTPayable, TaxSuffered, TaxSuffered2, ReasonID)      
   VALUES (@INVOICE_ID, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER, @REQUIRED_QUANTITY,       
	...
	@TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @REASON_ID)      
   IF @DOCSERIAL > 0  
   BEGIN
