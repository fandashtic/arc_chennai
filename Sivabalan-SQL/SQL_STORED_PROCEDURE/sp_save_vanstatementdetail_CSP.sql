CREATE Procedure sp_save_vanstatementdetail_CSP(@DOCUMENT_ID int,  
...
BEGIN  
 ...
 begin  
  ...
  Values(  
    ...
    @REQUIRED_QUANTITY, @PTR,@TOTAL_AMOUNT,@Serial)  
 end  
 if @TrnType<>2  
 begin  
  ...
  @SPECIALPRICE,@REQUIRED_QUANTITY, @DocSerial, @Serial)  
 End  
...
IF @TRACK_BATCHES = 1  
 ...
 if (@TrnType=1 Or @TrnType=2)  
  ...
  AND DocSerial=@FromVan AND ISNULL(PTR,0)=@ORIG_PRICE  
 else  
 ...
 if (@TrnType = 1 or @TrnType=2)    --Cursor for VanstatemntDetail  
  ...
  WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, N'') = @BATCH_NUMBER   
  AND DocSerial=@FromVan AND ISNULL(Pending, 0) > 0 and ISNULL(PTR, 0) = @ORIG_PRICE   
 else  
  DECLARE ReleaseStocks CURSOR KEYSET FOR  
   ...
   set @REQUIRED_QUANTITY = @REQUIRED_QUANTITY - @QUANTITY  
    ...
    Begin  
     Insert into VanTransferDetail  
      ...
      Values(  
       @DocSerial, @ITEM_CODE, @BATCH_CODE, @BATCH_NUMBER,   
       @QUANTITY, @PTR,@TOTAL_AMOUNT,@Serial)  
    End  
    ...
    Begin  
     ...
     @PENDING_QTY, @SALE_PRICE, @TOTAL_AMOUNT, @BFQTY, @COST, @PTS, @PTR, @ECP,   
     @SPECIALPRICE,@QUANTITY, @DocSerial, @Serial)  
    End   
