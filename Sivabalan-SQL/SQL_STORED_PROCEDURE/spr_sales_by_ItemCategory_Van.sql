CREATE procedure [dbo].[spr_sales_by_ItemCategory_Van](  
...
-- InvoiceAbstract.VanNumber=@VAN And           
 ...
 Select Convert(nVarChar, Items.CategoryID),-- + Char(15) + Van.van,
"Category Name" = ItemCategories.Category_Name, --"Van"=Van.Van,             
 ...
 + ' ' + @ConversionUnit,            
 "Reporting UOM" = CAST(CAST(SUM(ISNULL(Quantity, 0) / (CASE Items.ReportingUnit WHEN 0 THEN 1 ELSE Items.ReportingUnit END)) AS Decimal(18,6)) AS nVARCHAR)            
  + ' ' + @ReportingUOM,            
--Cast(dbo.Sp_Get_ReportingQty(Sum(ISNULL(QUANTITY, 0)), (CASE Cast(IsNull(Items.ReportingUnit,0) as Int) WHEN 0 THEN 1 ELSE Cast(IsNull(Items.ReportingUnit,1) as Int) END)) As nVarChar)      
--  + ' ' + @ReportingUOM,            
