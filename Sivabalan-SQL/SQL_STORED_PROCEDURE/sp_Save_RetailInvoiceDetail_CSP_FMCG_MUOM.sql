CREATE procedure [dbo].[sp_Save_RetailInvoiceDetail_CSP_FMCG_MUOM] (@INVOICE_ID int,  
...
If @@Fetch_Status = 0 And @FreeRow = 1  
Begin  
	Close ReleaseStocks  
	...
	Begin  
		...
		SELECT Batch_Number, Batch_Code , PurchasePrice FROM Batch_Products  
		WHERE Product_Code = @ITEM_CODE and ISNULL(Batch_Number, N'') = @BATCH_NUMBER   
		and ISNULL(SalePrice, 0) = @BATCH_PRICE AND (Expiry >= GetDate() OR Expiry IS NULL) 
