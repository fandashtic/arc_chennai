CREATE Procedure sp_save_invoice_detail_CSP_van_pharma_MUOM(@INVOICE_ID int,
...
SELECT @COST = Purchase_Price, @MRP = MRP, @SALEID = SaleID, @PTS = PTS, @PTR = PTR 
...
BEGIN
	...
	TaxSuffered, TaxSuffered2, PTS, PTR, UOM, UOMQTY, UOMPRICE)
	...
	@TAXSUFFERED2, @PTS, @PTR, @UOM, @UOMQTY, @UOMPRICE)
...
IF @TRACK_BATCHES = 1
	...
	ELSE IF @CUSTOMERTYPE = 2
		...
		DocSerial = @VANSTMTID AND VanStatementDetail.Product_Code = @ITEM_CODE 
		AND ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER AND 
		ISNULL(VanStatementDetail.PTR, 0) = @SALE_PRICE And ISNULL(Batch_Products.Free,0) = @FreeRow
		...
		WHERE DocSerial = @VANSTMTID and VanStatementDetail.Product_Code = @ITEM_CODE and 
		ISNULL(VanStatementDetail.Batch_Number, N'') = @BATCH_NUMBER and 
		ISNULL(VanStatementDetail.PTR, 0) = @SALE_PRICE AND ISNULL(Pending, 0) > 0 And ISNULL(Batch_Products.Free,0) = @FreeRow
		END
...
BEGIN
	...
	@CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @PTS, @PTR, @UOM, @UOMQTY, @UOMPRICE)
	SET @TAXCODE = 0
