CREATE PROCEDURE spr_get_beatwise_balance_itc    
...
- Isnull(    
  (select sum(Balance) From Creditnote,Customer where      
   IsNull((Select DefaultBeatID From Customer Where CustomerID = CreditNote.CustomerID) ,0) = #tempBeat.Beatid and    
...
InvoiceAbstract.InvoiceDate between @Seven AND @One and    
InvoiceType = 4 And  (Status & 128) = 0), 0))         
- Isnull((select sum(Balance) From Creditnote,Customer where     IsNull((Select DefaultBeatID From Customer
