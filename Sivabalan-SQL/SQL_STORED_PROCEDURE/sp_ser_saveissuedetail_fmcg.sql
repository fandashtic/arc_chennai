CREATE Procedure sp_ser_saveissuedetail_fmcg(
	@IssueID int,
...
This variable is used only for that.
...
Declare @JCSPARESISSUEDQTY as decimal(18,6)
Declare @JCSPARESISSUEID as int 
...
Set @MRP = 0
SELECT @MRP = MRP FROM Items WHERE Product_code = @SpareCode	
If @SalePrice > 0  	
begin
...
BEGIN
	...
	@ClaimPrice = ISNULL(Sale_Price, 0) FROM Items WHERE Product_code = @SpareCode	
	SET @RETVAL = 1
	Set @UOMqty = @BaseQty/@UOMConvertor
	...
	Vat_Exists, CollectTaxSuffered_Spares, MRP)
	Values  
	...
	@VatExist, @CollectTaxSuffered, @MRP)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			AND ISNULL(SalePrice, 0) = @SelectedPrice 
			...
			SELECT Batch_Number, Batch_Code, Quantity, PurchasePrice, TaxSuffered, BatchReference
			FROM Batch_Products
			WHERE Product_Code = @SpareCode and ISNULL(Batch_Number, '') = @Batch_Number 
			AND ISNULL(Saleprice, 0) = @SelectedPrice 
			AND isnull(purchaseprice, 0) = isnull(@SelectedPurchasePrice, 0) 
...
FETCH FROM ReleaseStocks into @Batch_Number, @BATCH_CODE, @QUANTITY, @COST, 
	@BATCHTAXSUFFERED, @ReferenceBatch
...
BEGIN
	...
	begin
		select @ClaimPrice = ISNULL(SalePrice, 0) from Batch_Products Where Batch_Code = isnull(@ReferenceBatch, 0)
	end 
	...
	BEGIN 
		...
		(@IssueID, @Product_Code, @Product_Specification1, 
		@SpareCode, @Batch_Code, @Batch_Number, @Warranty, @WarrantyNo, @DateofSale, 
		@UOM, @UOMQty, (case  when Isnull(@Warranty, 0) <> 1 then @SalePrice else 0 end), @BATCHTAXSUFFERED, @SaleTaxPercentage, 
		@BaseQty,@UOMPrice, @SaleTaxCode, @JCSerialNo, @COST, @PersonnelID, @ClaimPrice, 
		@VatExist, @CollectTaxSuffered, @MRP)
