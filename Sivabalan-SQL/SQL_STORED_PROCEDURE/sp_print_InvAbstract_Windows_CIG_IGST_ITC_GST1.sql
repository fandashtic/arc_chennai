Create Procedure [dbo].[sp_print_InvAbstract_Windows_CIG_IGST_ITC_GST1](@INVNO INT)     
AS 
Set dateformat DMY         
...
"InvoiceTotals" = 
				  ...
				  '|Net Amt. Payable     :' + Space(14-Len(cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then  ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,
2)) as nvarchar(12)) + Char(13) + Char(10)))
											+ cast(cast(isnull(Case InvoiceAbstract.InvoiceType When 4 Then  ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) Else ((NetValue + RoundOffAmount) - (isnull(@AdjustedValue,0))) End ,0) as decimal(18,2)) as nvarchar(12)) + Char(13) + 
Char(10),
