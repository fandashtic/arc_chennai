Create Procedure [dbo].[sp_print_PurchaseReturn_CG_Windows](@PRNO INT)
AS
--Declare @PRNO INT
--set @PRNO = 2
...
exec sp_print_RetPurchaseItems_NonCG_Window @PRNO,1
...
CGTotal= Cast(SUM(UTGSTAmt+SGSTAmt+CGSTAmt+CESSAmt+ADDLCESSAmt) As Decimal(18,2)),
NONCGTotal=Cast(Sum(IGSTAmt+CESSAmt+ADDLCESSAmt) As Decimal(18,2))
Into #GSTaxSummary
From #TempInvDet2
...
Select	@TotalTaxValue = SUM(UTGSTAmt+SGSTAmt+CGSTAmt+CESSAmt+ADDLCESSAmt+IGSTAmt)
From #TempInvDet2


Declare @GSTTaxSumText nVarChar(25)
Declare @GSTTaxComp nVarChar(Max)
Declare @GSTTaxComp_Win nVarChar(Max)
Declare @GSTTaxCompText nVarChar(200)
Declare @GSTTaxCompText_Win nVarChar(200)

Declare @GSTTaxCompTextNC_Win nVarChar(200)
Declare @GSTTaxCompNC_Win nVarChar(Max)
...
Set @GSTTaxCompText_Win=  'Tax Summary/GST  Component' + space(62) + 'TaxableVal ' + Space(12) + 'CGST ' + Space(8) + Case when @UTGST_flag = 1 then 'UTGST' Else  ' SGST' End + Space(12) + 'Cess ' +  Space(4) + 'AddlCess '+ + Space(6) + 'Total Tax'
Set @GSTTaxComp = '  '+ Char(13) + Char(10)+'  |  Tax Summary/GST  Component' + space(62) + 'TaxableVal' + Space(10) + 'CGST' + Space(8) + Case when @UTGST_flag = 1 then 'UTGST' Else  '  SGST' End + Space(10) + 'Cess' +  Space(4) + 'AddlCess'+ + Space(6) + 'Total Tax' + Char(13) + Char(10)
...
+ ' ' + Replicate('  ',10-LEN(LTrim(Cast(Cast(CGTotal as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(CGTotal as Decimal(10,2)) As nVarChar(10)) + Char(13) + Char(10) -- + SPACE(1)
From #GSTaxSummary


Set @GSTTaxCompTextNC_Win=  '       Rate ' + Space(6) +'TaxableValue ' + Space(6) + 'CGST ' + Space(5) + Case when @UTGST_flag = 1 then 'UTGST' Else  ' SGST' End + Space(7) + 'Total Tax'

Set @GSTTaxComp_Win  = ''
Select @GSTTaxComp_Win  = @GSTTaxComp_Win
+ 'CGST' + Replicate('',5-LEN(Cast(Cast(CGSTPer  As Decimal(5,2)) as nVarChar(5)))) +Cast(Cast(CGSTPer  As Decimal(5,2)) as nVarChar(5))
+ '% + '+ Case when @UTGST_flag = 1 then 'UTGST' Else  'SGST ' End + Replicate(' ',5-LEN(CAST(Cast(SGSTPer  As Decimal(5,2)) + Cast(UTGSTPer  As Decimal(5,2)) As nVarChar(5)))) + CAST(Cast(SGSTPer As Decimal(5,2)) As nVarChar(5))
+ '% + Cess' + Replicate(' ',5-LEN(CAST(Cast(CESSPer  As Decimal(5,2)) As nVarChar(5)))) + CAST(Cast(CESSPer  As Decimal(5,2)) As nVarChar(5))
+ '% + AddlCess ' +Replicate(' ',5-LEN(LTrim(CAST(Cast(ADDLCESSPer  As Decimal(4)) As nVarChar(5))))) + CAST(Cast(ADDLCESSPer  As Decimal(4)) As nVarChar(5)) +'/M' +SPACE(2)
+ ' ' + Replicate(' ',20-LEN(LTrim(Cast(Cast(TaxableValue as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(TaxableValue as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate(' ',15-LEN(LTrim(Cast(Cast(CGSTAmt as Decimal(10,2))  As nVarChar(10))))) + Cast(Cast(CGSTAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate(' ',15-LEN(LTrim(Cast(Cast(SGSTAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(SGSTAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate(' ',15-LEN(LTrim(Cast(Cast(CESSAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(CESSAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate(' ',15-LEN(LTrim(Cast(Cast(ADDLCESSAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(ADDLCESSAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate(' ',15-LEN(LTrim(Cast(Cast(CGTotal as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(CGTotal as Decimal(10,2)) As nVarChar(10))  + SPACE(2)
From #GSTaxSummary


Set @GSTTaxCompNC_Win  = ''
Select @GSTTaxCompNC_Win  = @GSTTaxCompNC_Win
...
+ '  ' + Replicate(' ',10-LEN(LTrim(Cast(Cast(CGSTAmt as Decimal(10,2))  As nVarChar(10))))) + Cast(Cast(CGSTAmt as Decimal(10,2)) As nVarChar(10))
+ '  ' + Replicate(' ',10-LEN(LTrim(Cast(Cast(SGSTAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(SGSTAmt as Decimal(10,2)) As nVarChar(10))
+ '  ' + Replicate(' ',10-LEN(LTrim(Cast(Cast(CGTotal as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(CGTotal as Decimal(10,2)) As nVarChar(10))  + SPACE(2)
From #GSTaxSummary


Declare @GSTTaxCompT nVarchar(Max)

Set @GSTTaxCompT = '  '+ Char(13) + Char(10)+'  | Rate' + space(20) + 'TaxableVal' + Space(10) + 'CGST' + Space(10) + Case when @UTGST_flag = 1 then 'UTGST' Else  '  SGST' End +  Space(6) + 'Total Tax' + Char(13) + Char(10)
Select @GSTTaxCompT = @GSTTaxCompT
+ '  | ' + Replicate('  ',10-LEN(Cast(Cast(TotalIntraTaxPer  As Decimal(10,2)) as nVarChar(10)))) +Cast(Cast(TotalIntraTaxPer  As Decimal(10,2)) as nVarChar(10))
+ '%' +SPACE(2)
+ ' ' + Replicate('  ',10-LEN(LTrim(Cast(Cast(TaxableValue as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(TaxableValue as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate('  ',10-LEN(LTrim(Cast(Cast(CGSTAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(CGSTAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate('  ',10-LEN(LTrim(cast(Cast(SGSTAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(SGSTAmt as Decimal(10,2)) As nVarChar(10))
...
Set @IGSTTaxCompCG = '  '+ Char(13) + Char(10)+'  |  Tax Summary/GST  Component' + space(52) + 'TaxableVal' + Space(12) + 'IGST' + Space(20) + 'Cess' +  Space(16) + 'AddlCess'+ + Space(16) + 'Total Tax' + Char(13) + Char(10)
Select @IGSTTaxCompCG = @IGSTTaxCompCG
+ '  |  ICGST' + Replicate('  ',5-LEN(Cast(Cast(IGSTPer  As Decimal(5,2)) as nVarChar(5)))) +Cast(Cast(IGSTPer  As Decimal(5,2)) as nVarChar(5))
+ '% + Cess' + Replicate('  ',5-LEN(CAST(Cast(CESSPer  As Decimal(5,2)) As nVarChar(5)))) + CAST(Cast(CESSPer  As Decimal(5,2)) As nVarChar(5))
+ '% + AddlCess ' +Replicate('  ',5-LEN(LTrim(CAST(Cast(ADDLCESSPer  As Decimal(4)) As nVarChar(5))))) + CAST(Cast(ADDLCESSPer  As Decimal(4)) As nVarChar(5)) +'/M' +SPACE(2)
+ ' ' + Replicate('  ',18-LEN(LTrim(Cast(Cast(TaxableValue as Decimal(18,2)) As nVarChar(18))))) + Cast(Cast(TaxableValue as Decimal(18,2)) As nVarChar(18))
+ ' ' + Replicate('  ',10-LEN(LTrim(Cast(Cast(IGSTAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(IGSTAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate('  ',15-LEN(LTrim(cast(Cast(CESSAmt as Decimal(15,2)) As nVarChar(15))))) + Cast(Cast(CESSAmt as Decimal(15,2)) As nVarChar(15))
+ ' ' + Replicate('  ',15-LEN(LTrim(Cast(Cast(ADDLCESSAmt as Decimal(15,2)) As nVarChar(15))))) + Cast(Cast(ADDLCESSAmt as Decimal(15,2)) As nVarChar(15))
+ ' ' + Replicate('  ',15-LEN(LTrim(Cast(Cast(NONCGTotal as Decimal(15,2)) As nVarChar(15))))) + Cast(Cast(NONCGTotal as Decimal(15,2)) As nVarChar(15)) + Char(13) + Char(10) -- + SPACE(1)
From #GSTaxSummary

Set @IGSTTaxCompNonCG = '  '+ Char(13) + Char(10)+'  | Rate' + space(30) + 'TaxableVal' + Space(12) + 'IGST' + Space(10) +  Space(6) + 'Total Tax' + Char(13) + Char(10)
Select @IGSTTaxCompNonCG = @IGSTTaxCompNonCG
+ '  | ' + Replicate('  ',10-LEN(Cast(Cast(TotalInterTaxPer  As Decimal(10,2)) as nVarChar(10)))) +Cast(Cast(TotalInterTaxPer  As Decimal(10,2)) as nVarChar(10))
+ '%' +SPACE(2)
+ ' ' + Replicate('  ',15-LEN(LTrim(Cast(Cast(TaxableValue as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(TaxableValue as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate('  ',10-LEN(LTrim(Cast(Cast(IGSTAmt as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(IGSTAmt as Decimal(10,2)) As nVarChar(10))
+ ' ' + Replicate('  ',15-LEN(LTrim(Cast(Cast(NONCGTotal as Decimal(10,2)) As nVarChar(10))))) + Cast(Cast(NONCGTotal as Decimal(10,2)) As nVarChar(10)) + Char(13) + Char(10) -- + SPACE(1)
From #GSTaxSummary
...
"VendorAddress" = Replace(Replace(Isnull(V.Address,''),Char(10),' '),char(13),' '),
...
--Case when V.TNGST  = '' then '' Else 'FSSAI No. : ' + V.TNGST End,
...
"CG_GSTTax Summary Header" = @GSTTaxCompText_Win,
"CG_GSTTaxComp Details" = @GSTTaxComp,
"NonCG_GSTTax Summary Header" = @GSTTaxCompTextNC_Win,
"NonCG_GSTTaxComp Details" = @GSTTaxCompT,
