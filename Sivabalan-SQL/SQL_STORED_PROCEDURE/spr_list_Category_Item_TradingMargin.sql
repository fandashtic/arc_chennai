CREATE PROCEDURE spr_list_Category_Item_TradingMargin(@CATEGORYID INT,
		  			         @FROMDATE DATETIME, 
						 @TODATE DATETIME)
AS
...
Create Table #Margin (ProdCode nvarchar(255), ProdName nvarchar(255), Sales Decimal(18,6), Purchase Decimal(18,6), TradingMargin Decimal(18,6))
Insert into #Margin 
...
Select ProdCode, "Item Code" = ProdCode, "Item Name" = ProdName, "Sale Value (%c.)" = Sales, "Purchase Value (%c.)" = Purchase, "Trading Margin (%c.)" = TradingMargin, "% Contribution" = Case TradingMargin When 0 Then 0 Else Cast((TradingMargin * 100 / @TotalMargin) as Decimal(18,6)) End  
From #Margin Where Sales > 0
