...
Create Procedure [dbo].[sp_print_InvAbstract_SR_ITC_DOS_WB](@INVNO INT)     
AS       
Set dateformat DMY   
...
Set @Customerid = (select  Top 1 Customerid From InvoiceAbstract Where InvoiceID = @INVNO)
Set @InvoiceDate = (select  Top 1 dbo.stripTimeFromdate(InvoiceDate) From InvoiceAbstract Where InvoiceID = @INVNO)
set @ClosingPoints = isnull((select Dbo.Fn_Get_CurrentAchievementVal(@Customerid,@InvoiceDate)),'')
Set @TargetVsAchievement = isnull((select Dbo.Fn_Get_CurrentTarget_AchievementVal(@Customerid,@InvoiceDate)),'')
Set @CustCode=''
...
exec sp_print_RetInvItems_RespectiveUOM_SR_ITC_WB @INVNO,1
