CREATE procedure [dbo].[sp_print_RetInvItems_fmcg_MUOM_SR_Serial](@INVNO INT)    
AS    
...
"Amount" = case (SELECT TOP 1 Flags FROM InvoiceAbstract WHERE InvoiceID = #Temp1.Invno)    
...
"Tax Suffered Value" = IsNull((Sum(#Temp1.SRQty * InvoiceDetail.Quantity * InvoiceDetail.SalePrice) * Max(InvoiceDetail.TaxSuffered) / 100), 0),     
"PKD" = CAST(DATEPART(mm, Batch_Products.PKD) AS nvarchar) + N'\'    
+ SubString(CAST(DATEPART(yy, Batch_Products.PKD) AS nvarchar), 3, 2),    
"Amount" = Cast((case (SELECT TOP 1 Flags FROM InvoiceAbstract WHERE InvoiceID = #Temp1.Invno)    
    WHEN 0 THEN Round((SUM(#Temp1.SRQty * InvoiceDetail.Quantity) * InvoiceDetail.SalePrice)     
    - (SUM(#Temp1.SRQty * InvoiceDetail.Quantity) * InvoiceDetail.SalePrice     
    * MAX(InvoiceDetail.DiscountPercentage) / 100) +     
    (((SUM(#Temp1.SRQty * InvoiceDetail.Quantity) * InvoiceDetail.SalePrice)     
