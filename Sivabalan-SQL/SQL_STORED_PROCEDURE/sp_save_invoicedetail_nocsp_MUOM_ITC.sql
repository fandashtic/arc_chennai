CREATE Procedure sp_save_invoicedetail_nocsp_MUOM_ITC(@INVOICE_ID int,  
...
--		SELECT @TAXID = Tax_Code FROM Tax WHERE ISNULL(CST_Percentage, 0) = @TAXCODE2  
	...
	BEGIN  
		...
		TaxSuffered, TaxSuffered2, UOM, UOMQty, UOMPrice, OtherCG_Item,QuotationID,MultipleSchemeID,
		TotSchemeAmount,MultipleSchemeDetails,MultipleRebateID,RebateRate,MultipleRebateDet,GroupID,MRPPerPack,TAXONQTY,GSTFlag,GSTCSTaxCode,HSNNumber,CategorizationID)    
		...
		@CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice, @OtherCG_Item,@QuotationID,@MultiSchID,
		@TotSchAmount,@MultiSchIDAndCost,@MultipleRebateID,@RebateRate,@MultipleRebateDet,@GroupID,@MRPPERPACK,@TAXONQTY,@GSTFlag,@GSTCSTaxCode,@HSNNumber,@CategorizationID)
		GOTO ALL_SAID_AND_DONE  
	END  
	IF @TRACK_BATCHES = 1  
	BEGIN  
	...
	BEGIN  
		--SELECT @MRPPERPACK = ISNULL(MRPPerPack,0) FROM Batch_Products 
...
--		Select  @BATCHMRPPERPACK = IsNull(MRPPerPack,0) From Items Where Product_Code = @ITEM_CODE
		...
		BEGIN  
			...
			CSTPayable, TaxSuffered, TaxSuffered2, UOM, UOMQty, UOMPrice, OtherCG_Item,QuotationID,
			MultipleSchemeID,TotSchemeAmount,MultipleSchemeDetails,MultipleRebateID,RebateRate,MultipleRebateDet,GroupID,
			...
			@FLAG, @TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice, @OtherCG_Item,
	...
	BEGIN  
		...
		@TAXCODE2, @CSTPAYABLE, @TAXSUFFERED, @TAXSUFFERED2, @UOM, @UOMQty, @UOMPrice, 
		@OtherCG_Item,@QuotationID,@MultiSchID,@TotSchAmount,@MultiSchIDAndCost,@MultipleRebateID,@RebateRate
...
OVERNOUT:  
	...
	Begin
		...
		Begin
			if isNull(@MultiSchID,N'') <> N''
			Begin
	--			Insert Into tbl_mERP_SchemeSale(Product_Code, Quantity, Free, Value, Cost, Type, InvoiceID, Claimed, Pending, Flags,Sno,MultipleSchemeID)  
	--			Values(@ITEM_CODE, @PRIMARY_QUANTITY, @ORIGINAL_QTY, @SALE_PRICE * @ORIGINAL_QTY, @SCHEME_COST, @SCHEMEID, @INVOICE_ID, 0, @ORIGINAL_QTY, @SECONDARY_SCHEME,@Sno,@MultiSchID)  
