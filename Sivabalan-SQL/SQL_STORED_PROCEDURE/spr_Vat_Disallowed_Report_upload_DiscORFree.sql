Create procedure spr_Vat_Disallowed_Report_upload_DiscORFree ( @FromDate Datetime, @ToDate Datetime, 
...
select @IsUpload = IsNull(Flag,0) from tbl_mERP_ConfigDetail where ScreenCode ='DFRFLAG' and XMLAttribute ='UPLOAD'   
...
If @DayClosed = 0
    Goto OvernOut
/* Report should be generated only if the Upload Flag is enabled */
If @IsUpload = 0
    Goto OvernOut     
...
--select * from InvoiceDetail where SalePrice =0 and PTS > 0
--select * from InvoiceDetail where SalePrice >0 and PTS > 0
--TmpInvoiceabstract
Select Ia.* 
...
Select Id.InvoiceId, Id.product_code, max(Id.Batch_code) virtualBatch, max(Id.Batch_Number) Batch_Number, Id.saleprice,
  Itm.DivId, Itm.SubCtgId, Itm.MktSKUId, itm.UOM, itm.UOM1, itm.UOM2, itm.UOM1_Conversion,itm.UOM2_Conversion
...
, max(Id.stcredit) stcredit, max(Id.pts) pts, Ia.CustomerId, max(Id.taxId) taxId, Ia.status
...
, Ia.Docreference, Ia.InvoiceDate, Ia.InvoiceType, Ia.CustomerId, Ia.status, Id.saleprice ,  Itm.DivId, Itm.SubCtgId, Itm.MktSKUId
...
-- summary level Data for System-sku
    ...
    order by ItmD.Category_name, ItmS.Category_name, ItmM.Category_name,Itm.product_code
