CREATE PROCEDURE Sp_Update_Opening_TaxSuffered_Percentage_FMCG(
...
AS
--This FMCG Procedure is used to add or deduct taxsuffered percentage in openingdetails 
...
BEGIN
	...
	Select @MRP=MRP from Items Where Product_Code=@ITEMCODE
	Select @nTax = (@TaxPercentage * @TaxPartOf) / 100
	If @TaxApplicableOn = 1      
		Select @TaxSufferedAmt = (@Price * @Qty * (@nTax / 100))      
	Else If @TaxApplicableOn = 6      
		Select @TaxSufferedAmt = (@MRP * @Qty * (@nTax / 100))      
	Else
		Select @TaxSufferedAmt = 0   
	...
	Else
		...
		--to calculate taxsuffered percentage.
		--Because in bill module we don't update stock quantity.
		Select @Batch_Value = 0

	If @DEDUCT = 1	--Deduct TaxSuffered Amount and calculate TaxSuffered Per..
		Begin
		--Detect the Taxsuffered amount
