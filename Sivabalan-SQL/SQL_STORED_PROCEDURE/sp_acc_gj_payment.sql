CREATE procedure sp_acc_gj_payment(@paymentid integer,@BackDate DATETIME=Null)  
as  
...
-- --      set @currentdate = dbo.stripdatefromtime(getdate())  
     ...
     if (dbo.StripDateFromTime(@dpaymentDate) < @currentdate) and (dbo.StripDateFromTime(@dpaymentDate) < dbo.stripDateFromTime(@chequeDate)) and  (dbo.stripDateFromTime(@chequeDate) <= @currentdate)  
   begin  
     if @nvalue <> 0  
  begin  
       insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
 ...
 else if @accountid <> 0 and @npaymentmode = 4 /* Bank Transfer */
 begin  
   	select @bankaccount = ISNULL([AccountID],0) from bank  
    where [BankID]=@bankid     
 ...
 if @extracol > 0  
  begin  
   insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
      [Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])  
      Values(@documentid,@othercharges,@dpaymentdate,@extracol,0,@npaymentid,@ndoctype,'Extra Amount Paid',@uniqueid)  
      Insert Into #TempBackdatedpayments(AccountID) Values(@othercharges)  
    
      insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
      [Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])  
      Values(@documentid,@accountid,@dpaymentdate,0,@extracol,@npaymentid,@ndoctype,'Extra Amount Paid',@uniqueid)     
      Insert Into #TempBackdatedpayments(AccountID) Values(@accountid)  
  end  
  ...
  begin  
      insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
      [Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])  
      Values(@documentid,@accountid,@dpaymentdate,@adjamt,0,@npaymentid,@ndoctype,'Payment Adjusted with Shortage',@uniqueid)     
      Insert Into #TempBackdatedpayments(AccountID) Values(@accountid)   
       
      insert GeneralJournal ([TransactionID],[AccountID],[TransactionDate],  
      [Debit],[Credit],[DocumentReference],[DocumentType],[Remarks],[DocumentNumber])  
      Values(@documentid,@discount,@dpaymentdate,0,@adjamt,@npaymentid,@ndoctype,'Payment Adjusted with Shortage',@uniqueid)  
