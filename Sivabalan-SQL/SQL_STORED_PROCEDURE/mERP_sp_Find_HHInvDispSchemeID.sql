Create Procedure mERP_sp_Find_HHInvDispSchemeID
(
...
--	Declare @Channel nVarchar(255)
	...
	Insert Into #tmpScheme Select SA.SchemeID, SA.Description, Min(SO.GroupId)
		...
		And (dbo.stripTimeFromDate(@CreationDate) Between dbo.StripTimeFromDate(ActiveFrom) And dbo.StripTimeFromDate(ExpiryDate)) 
...
--		And (SOC.OutletClass = @OLClass Or SOC.OutletClass = N'ALL')
	...
	Begin 
		...
		Where SlabId = @SlabID
--		SchemeID = @SchemeID
--		And @InvoiceAmt Between SlabStart And SlabEnd
--		And SlabID = @SlabID
--		And GroupID = @GroupID
...
-- Mar 17 CS scheme
	...
	Select SchemeID, FreeProductCode
	from Scheme_Details where OrderNumber = @OrderNUmber 
	And IsNull(OrderedProductCode,'') = ''  


	Declare HHCursor Cursor For
	Select #tblHHData.SchemeID, #tblHHData.FreeProductCode from #tblHHData Order By schemeID
	Open HHCursor
	Fetch From HHCursor Into  @SchemeID, @FreeProductCode
	While @@FETCH_STATUS = 0
	Begin 
		If Exists ( Select SchemeID From #tmpDetail where SchemeID = @SchemeID)
		Begin
			Select @TSlabID = SlabID From #tmpDetail where SchemeID = @SchemeID
	...
	Select  TS.SchemeID 'SchemeID', Description as Description, IsNull(SchAmount, 0.00) as SchemeAmt, 
			IsNull(SchPercentage, 0) as SchemePer, IsNull(FreeUOM,0) as SchemeUOM, ISNull(Volume,0) as SchemeVolume, 
			SlabID as SlabID, SlabType as SlabType, ISNull(SKUList, '') as SKUList, IsNull(HH.FreeProductCode,'') As HHFreeProductCode 
	From #tmpDetail TS Left Outer Join #tblHHData HH
