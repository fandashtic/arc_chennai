CREATE procedure [dbo].[spr_ABC_pidilite](@Category nvarchar(2550),    
...
Where Category_Name In (select CategoryName from #tmpCat)      
While @Continue > 0    
Begin    
...
BEGIN    
 ...
 select InvoiceAbstract.CustomerID, Customer.Company_Name, SUM(Amount) from 
 InvoiceAbstract, Customer, InvoiceDetail, Items, ItemCategories    
...
BEGIN    
 Insert into #temp (CustomerID, Company_Name, TotalSales)     
 select CASE InvoiceAbstract.CustomerID WHEN N'0' THEN N'Other Customer' ELSE InvoiceAbstract.CustomerID END, IsNull(Customer.Company_Name, N'Other Customer'),     
