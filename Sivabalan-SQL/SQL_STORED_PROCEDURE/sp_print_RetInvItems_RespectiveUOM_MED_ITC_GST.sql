Create Procedure [dbo].[sp_print_RetInvItems_RespectiveUOM_MED_ITC_GST](@INVNO INT,@MODE INT=0)
AS
--Declare @INVNO INT
--Declare @MODE INT
...
"Quantity" = Case When Duplicate=1 Then Isnull(cast(Cast((Case When InvoiceDetail.UOMPrice <> 0
THEN Sum(#Temp1.SRQty * InvoiceDetail.UOMQty)
Else 0
End) as decimal(18,2))as nvarchar),'') Else cast(dbo.fn_dateMY(Batch_Products.Expiry) as nvarchar) End,
--GST_Changes starts here
...
--GST_Changes ends here
"UOM" = UOM.Description,
"Sale Price" = Case When Duplicate=1 Then Isnull(Cast(Case InvoiceDetail.UOMPrice
When 0 Then N'Free'
Else Cast(Cast(InvoiceDetail.UOMPrice As Decimal(18,2))As NVarChar)
End As nVarChar),'') Else cast(dbo.fn_dateMY(Batch_Products.PKD) as nvarchar) End,
...
else Cast(Cast(case when @utgst_flag = 1 then Max(UTGSTAmt) Else sum(SGSTAmt) End as decimal(18,2))as nVarchar)
end,
...
"Other Disc" = Case When Duplicate = 1
then Cast(sum((InvoiceDetail.Quantity * InvoiceDetail.SalePrice-InvoiceDetail.DiscountValue))*(InvoiceAbstract.AdditionalDiscount/100) as nvarchar)
...
+' | '+ Cast(Cast(Sum(InvoiceDetail.Quantity) As decimal(20,4)) As nvarchar(25)) + +' '+ Cast(InvoiceDetail.BaseUOMDescription  As nvarchar(5)) As nVarchar(50))
/*Cast(STUFF('000', 7-LEN(Cast(Sum(InvoiceDetail.Quantity) As decimal(5,3)))+1, LEN(Cast(Sum(InvoiceDetail.Quantity) As decimal(5,3))), Cast(Sum(InvoiceDetail.Quantity) As decimal(5,3))) As nVarchar(10)) +' '+
Cast(InvoiceDetail.BaseUOMDescription  As nvarchar(5))As nVarchar(50))*/
...
"Item MRP" =  cast(cast(case isnull(InvoiceDetail.MRPPerpack,0) when 0 then isnull(Items.MRPPerPack,0) else isnull(InvoiceDetail.MRPPerpack,0) end as decimal(18,2)) as nvarchar),
...
Left Outer Join Manufacturer On Items.ManufacturerID = Manufacturer.ManufacturerID--GST_Changes
...
,#tmpDuplicate --GST_Changes AND InvoiceDetail.UOMQty > 0
GROUP BY
...
UOM=[Batch No.],
--[Sale Price]=[Mfr. Dt.],Quantity= Expiry,
[Item Gross Value]=Taxablevalue,
Amount='',[Net Value]='',[Net Amount]='',Serial=0,[SNo]='',[Item MRP]='',TaxDetailsWithBreakup = '',
TaxDetails='',[Batch No.]='' ,[Total Tax] = '' Where Duplicate = 2
--GST_Changes ends here
