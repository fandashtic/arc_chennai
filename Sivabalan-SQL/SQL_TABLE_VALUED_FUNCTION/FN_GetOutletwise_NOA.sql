CREATE Function FN_GetOutletwise_NOA (@PMID int,@DSTypeID int,@ParamID int)
Returns @Result Table(DSID int,NOACount int)
AS
BEGIN
	...
	(select Distinct OutletID from PMOutletAchieve where PMID=@PMID And DSTYPEID = @DSTypeID and ParamID = @ParamID)

	insert into @Result(DSID,NOACount)
	Select DSID,count(OutletID) From @OutletDetails
	Group by DSID
