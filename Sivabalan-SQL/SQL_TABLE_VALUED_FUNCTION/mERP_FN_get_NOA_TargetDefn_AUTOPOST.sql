CREATE Function mERP_FN_get_NOA_TargetDefn_AUTOPOST(@PMetricID Int, @ParamID Int, @EditLock Int)
Returns @Result Table(SalesManID int NULL,
			...
			LastupdatedDate Datetime,AutoPostflag int,SaveFlag int,MaxPoints Decimal(18,6))
As
Begin 
	...
	Begin
		Declare @tmpDSTarget as Table(SalesmanID int, Salesman_Name nVarchar(510) COLLATE SQL_Latin1_General_CP1_CI_AS,
									  PMDSTypeID Int, DSTypeValue nVarchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS, CGMapID int,MaxPoints Decimal(18,6))
...
--		Set @i=1
		...
		if @OCG=1
		Begin
			Insert into @tmpDSTarget(SalesmanID, Salesman_Name, PMDSTypeID, DSTypeValue, MaxPoints)
			Select Distinct SM.SalesManID, SM.Salesman_Name, PMDS.DSTypeID, PMDS.DSType,PMParam.MaxPoints		
	...
	Begin
		...
		Select SalesManID, SalesMan_Name, DSTypeValue, 0 NOACount, @PMetricID as PMID, PMDSTypeID, @ParamID as ParamID, 		
		0 as FocusID,CGMapID, 0 TargetDefnID,NULL,0,0,MaxPoints
		From @tmpDSTarget
		...
		BEGIN
			...
			Declare @tmpOutletwiseData Table(DSID int,NOACount int)
			Insert into @tmpOutletwiseData(DSID,NOACount)
			select DSID,NOACount from  dbo.FN_GetOutletwise_NOA(@PMetricID,@DStypeID,@ParamID)
			
			Update T set T.NOACount = T1.NOACount
	...
	Begin
		...
		Insert Into @Tempdata 	  
		Select PMTar.SalesmanID, SM.SalesMan_Name, PMDS.DSType, PMTar.NOACount, PMTar.PMID, PMTar.PMDSTypeID, PMTar.ParamID, 		
		PMTar.FocusID, PMTar.DSTypeCGMapID, PMTar.TargetDefnID ,PMTar.TargetDefnDate,isnull(PMTar.AutopostFlag,0),1,PMTar.MaxPoints
		From tbl_merp_NOA_TargetDefn PMTar, Salesman SM, tbl_mERP_PMDSType PMDS,tbl_mERP_PMParam PMparam
	...
	End
	Insert into @Result
	select SalesManID,SalesMan_Name,DSTypeValue,NOACount,PMID,PMDSTypeID,ParamID,
	CGMapID,TargetDefnID,LastupdatedDate,AutoPostflag,SaveFlag,MaxPoints
	from @Tempdata
