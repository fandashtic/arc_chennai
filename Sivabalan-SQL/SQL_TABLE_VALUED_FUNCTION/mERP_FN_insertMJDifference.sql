...
Create function mERP_FN_insertMJDifference(@FD datetime,@TD datetime)  
Returns @temptable TABLE (TransactionID int,Voucher nvarchar(2000),VDate datetime,Ledgerfrom nvarchar(2000),DrAmount Decimal(18,4),CrAmount Decimal(18,4),Narration nvarchar(4000),  
AdjRef nvarchar(4000),AdjReftype nvarchar(250),AdjAmount decimal(18,6))    
AS  
  ...
  BEGIN  
   ...
   BEGIN  
    ...
    BEGIN  
     ...
     gj.TransactionDate,am.accountName,@Diff,0,Ltrim(Rtrim(gj.VoucherNo))+' '+Cast(gj.Remarks as Varchar(255)),'','New Ref',0  
   ...
   BEGIN  
    ...
    BEGIN  
     insert into @temptable  
     Select distinct @TransactionID,(Select Prefix From VoucherPrefix where TranID='MANUAL JOURNAL')+Rtrim(gj.DocumentNumber) +'-'+cast(gj.TransactionID as varchar(10)),  
     gj.TransactionDate,am.accountName,0,@Diff,Ltrim(Rtrim(gj.VoucherNo))+' '+Cast(gj.Remarks as Varchar(255)),'','New Ref',0  
     from Generaljournal gj,AccountsMaster am  where  am.AccountID=gj.AccountID  and TransactionID = @TransactionID and gj.accountid=@Accountid  
    END  
   END  
  END  
