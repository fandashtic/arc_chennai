Create Function mERP_FN_PointsforBLOCKBUSTER_View()
Returns  
...
/* End of Addition*/
	...
	Begin
		...
		Select Distinct ParamID,@WDCode as 'WDCode' ,@WDDest as 'WDDest',Salesman_Name as 'DSName',DSType  as 'DS Type',PMCode as 'Performance Metrics Code',PMDescription as 'Description',Replace(CGGroups,',','|') [Category Group],@FromDate [From Date],Convert(nVarchar(10), @ToDate, 103) [To Date],
		  (Case ParameterType When 1 Then N'Lines Cut' When 2 Then N'Bills Cut' When 3 Then N'Business Achievement' When 4 then 'Go Green OBJ' When 5 Then 'Reduce Red OBJ' When 9 then 'Blockbuster' End) 'Parameter',
...
/* GGDR Process Start :*/
	...
	Begin
		...
		Declare @TmpGGDRDSPointsData Table(
				...
				PointValue Decimal(18,6),Percentage Decimal(18,6))

		Delete From @TmpGGDRAbstract
		Insert Into @TmpGGDRAbstract 
		Select * from dbo.mERP_FN_TargetsforGGDR_View_Blockbuster()
...
--			Select ParamID,DSID,DSname,[DS Type],Parameter, case Isnull(Target,0) when 0 then 0 else (Isnull([Till date Actual],0)/ Isnull(Target,0)) * 100 end  From @tmpOutput Where Parameter In ('Blockbuster') And Isnull([Till date Actual] ,0) > 0
--
...
--	/* Points Calculation End. */
		...
		Where B.SalesmanId = T.DSID And T.Parameter In ('Blockbuster')  
		...
		Select DSId,[DS Type],[Category Group] From @tmpOutput Where Parameter In ('Blockbuster')
		Open Cur_GGDRtarget
...
--				And T.Parameter In ('Go Green OBJ')
				...
				And T.[DS Type] = @Tar_DSType
				And T.[Category Group] = @Tar_CatGroup
				And T.Parameter In ('Blockbuster')
...
--		If (Select dbo.StripTimeFromDate(LastInventoryUpload) From Setup) >= dbo.StripTimeFromDate(DateAdd(d,-1,DateAdd(m,1,Cast(('01/' + cast(@DateOrMonth as nvarchar)) as dateTime))))
		--Begin
		--	Insert Into @TmpGGDRDSPointsData (ParamID,DSID,DSname,DSType,Parameter,TilldateActual,Percentage)
		--	Select ParamID,DSID,DSname,[DS Type],Parameter,[Till date Actual], case Isnull(Target,0) when 0 then 0 else (Isnull([Till date Actual],0)/ Isnull(Target,0)) * 100 end  From @tmpOutput Where Parameter In ('Blockbuster') And Isnull([Till date Actual] ,0) > 0

		--	Declare @GGDRParamID as Int
		--	Declare @GGDRTilldateActual as Decimal(18,6)
		--	Declare @@GGDRPercentage as Decimal(18,6)
  --          Declare @DSID as int 
		--	Declare Cur_GGDRPoints Cursor for
		--	Select ParamID,TilldateActual,Percentage,DSID From @TmpGGDRDSPointsData
		--	Open Cur_GGDRPoints
		--	Fetch from Cur_GGDRPoints into @GGDRParamID,@GGDRTilldateActual,@@GGDRPercentage,@DSID
		--	While @@fetch_status =0
		--		Begin
		--			Update T Set T.SlabFrom = T1.Slab_Start,
		--						 T.SlabTo = T1.Slab_End,
		--						 T.ForEvery = T1.Slab_Every_Qty,
		--						 T.PointValue = T1.Slab_Value From @TmpGGDRDSPointsData T,
		--			(select ParamID,Slab_Start,Slab_End,Slab_Every_Qty,Slab_Value From tbl_mERP_PMParamSlab Where ParamID = @GGDRParamID and @@GGDRPercentage Between Slab_Start and Slab_End) T1
		--			Where T.paramID = T1.ParamID and T.DSID = @DSID

		--			Fetch Next from Cur_GGDRPoints into @GGDRParamID,@GGDRTilldateActual,@@GGDRPercentage,@DSID
		--		End
		--	Close Cur_GGDRPoints
		--	Deallocate Cur_GGDRPoints
...
--	Delete From @tmpOutput Where Parameter Not In ('Go Green OBJ','Reduce Red OBJ')
		Delete From @tmpOutput Where Parameter Not In ('Blockbuster')
	...
	(Case When Parameter = 'Blockbuster' Then 'BlockBuster' End )
