CREATE Function Fn_Get_MappedSalemanBeatAndCustomer_ITC    
(    
 @SalesManID Int =0,    
 @BeatID Int = 0,    
...
Begin    
 ...
 Declare @GSTEnabled int  
 Select @GSTEnabled = isnull(Flag, 0) From tbl_merp_ConfigAbstract Where ScreenCode = 'GSTaxEnabled'  
 ...
 Begin  
  ...
  If @SalesManID <> 0 And @BeatID = 0 And @CustomerID = ''    
   Insert Into @MappingTable    
   Select SalesManID, SalesMan_Name, BeatID, Description, CustomerID, Company_Name, Address From  
   (  
    Select     
      Beat_SalesMan.SalesManID,"SalesMan_Name"=dbo.GetSalesManNameFromID(SalesManID),    
      BeatID,"Description"=dbo.GetBeatDescriptionFromID(BeatID),  
    ...
    Select     
      "SalesManID" = @SalesManID,"SalesMan_Name"=dbo.GetSalesManNameFromID(@SalesManID),    
      B.BeatID,"Description"=B.Description,  
      C.CustomerID,"Company_Name"=C.Company_Name,     
      C.BillingAddress as Address    
    From     
      Customer C, Beat B  
    Where     
      C.DefaultBeatID = B.BeatID and isnull(C.PreDefFlag,0) = 1  
  
   ) A  
   Order By    
     SalesManID,SalesMan_Name,  
     BeatID, Description, CustomerID, Company_Name    
  ...
  Else    
   ...
   (  
    ...
    From     
      Customer C, Beat B  
    Where     
      B.BeatID = (Case When @BeatID <> 0 Then @BeatID Else B.BeatID End)      
      And C.CustomerID = (Case When @CustomerID <> '' Then @CustomerID Else C.CustomerID End)  
      and C.DefaultBeatID = B.BeatID and isnull(C.PreDefFlag,0) = 1  
