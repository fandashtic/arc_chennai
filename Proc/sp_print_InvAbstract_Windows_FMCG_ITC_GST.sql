--select * from InvoiceAbstract where GSTFullDocID like '%37152'
--exec sp_print_InvAbstract_Windows_FMCG_ITC_GST 65966
IF EXISTS(SELECT * FROM sys.objects WHERE Name = N'sp_print_InvAbstract_Windows_FMCG_ITC_GST')
BEGIN
    DROP PROC sp_print_InvAbstract_Windows_FMCG_ITC_GST
END
GO
Create Procedure [dbo].sp_print_InvAbstract_Windows_FMCG_ITC_GST(@INVNO INT)
AS
Set dateformat DMY
...
select @UTGST_flag = isnull(flag,0) from tbl_merp_configabstract  WITH (NOLOCK) where screencode = 'UTGST'

Set @CustCode=''
Set @CustCode=(Select CustomerID from InvoiceAbstract WITH (NOLOCK) where InvoiceID=@InvNo)

Set @InvoiceDate = (select  Top 1 dbo.stripTimeFromdate(InvoiceDate) From InvoiceAbstract WITH (NOLOCK) Where InvoiceID = @INVNO)
...
Set @PointsEarned=''
Set @PointsEarned=Cast(IsNull((Select Cast(Sum(IsNull(Points, 0)) as int) from tbl_mERP_OutletPoints op WITH (NOLOCK)
Where op.InvoiceID = @INVNO And op.Status = 0), 0) as int)
Set @TotPointsEarned=''
Set @TotPointsEarned=Cast(IsNull((Select Cast(Sum(IsNull(Points, 0)) as int) from tbl_mERP_OutletPoints op WITH (NOLOCK)
Where op.outletCode =@CustCode and op.InvoiceID <= @INVNO  And op.Status = 0), 0) as int)

Select @WDPhoneNumber=Telephone from Setup WITH (NOLOCK)
Select @CompanyGSTIN=GSTIN from Setup WITH (NOLOCK)
Select @CompanyPAN =PANNumber from Setup WITH (NOLOCK)
Select @CIN=CIN from Setup WITH (NOLOCK)
...
From InvoiceAbstract WITH (NOLOCK) Where InvoiceID = @INVNO
...
@SecondSales = (Select IsNull(Sum(STPayable + CSTPayable), 0) From InvoiceDetail WITH (NOLOCK)
...
from InvoiceDetail InvDet WITH (NOLOCK)
...
End
From InvoiceDetail WITH (NOLOCK)
Left Outer Join  Batch_Products WITH (NOLOCK) On InvoiceDetail.Batch_Code = Batch_Products.Batch_Code
Inner Join InvoiceAbstract WITH (NOLOCK)  On InvoiceAbstract.InvoiceID=InvoiceDetail.InvoiceID
...
exec sp_print_RetInvItems_RespectiveUOM_FMCG_ITC_GST @INVNO,1
--Select  1
...
Select @TaxSuffered = Sum(TaxSuffered) From #temp WITH (NOLOCK)
--@ItemCountWithoutFree = Sum(ItemCountWithoutFree) From #temp
Select @ItemCountWithoutFree=Count(Distinct Product_Code) from InvoiceDetail WITH (NOLOCK) where InvoiceID=@InvNo and SalePrice<>0
Select @ItemCount = max(ItemCount)*2 From #tempItemCount WITH (NOLOCK)
...
From GSTInvoiceTaxComponents ITC WITH (NOLOCK)
Join TaxComponentDetail TCD WITH (NOLOCK)
...
When 1 then (Select SGSTPer From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0)
When 0 then isnull(ID.TaxCode,0) + isnull(ID.TaxCode2,0)
End ,
SGSTAmt=Case GSTFlag
When 1 then (Select Sum(SGSTAmt) From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0)
When 0 then isnull(ID.stpayable,0)+ isnull(ID.cstpayable,0)
End,
CGSTPer=(Select CGSTPer From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
CGSTAmt=(Select Sum(CGSTAmt) From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
IGSTPer=(Select IGSTPer From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
IGSTAmt=(Select Sum(IGSTAmt) From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
UTGSTPer=(Select UTGSTPer From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
UTGSTAmt=(Select Sum(UTGSTAmt) From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
CESSPer=(Select CESSPer From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
CESSAmt=(Select Sum(CESSAmt) From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0),
ADDLCESSPer=(Select ADDLCESSPer From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0) ,
ADDLCESSAmt=(Select Sum(ADDLCESSAmt) From #TempTaxDet WITH (NOLOCK) Where InvoiceID = ID.InvoiceID And Product_Code = ID.Product_Code And SerialNo= ID.Serial and	UOMQty > 0)
Into #TempInvDet2
...
From #TempInvDet2 WITH (NOLOCK) --where Convert(Decimal(5,2),Case when @UTGST_flag = 1 then UTGSTPer Else  SGSTPer End )+Convert(Decimal(5,2) , CGSTPer) > 0
Group By Case when @UTGST_flag = 1 then UTGSTPer Else  SGSTPer End, CGSTPer

--Select * from  #GSTTaxCompDet
--Select LEN(Cast(Cast(Rate As Decimal(5,2))As nVarChar(5))) from #GSTTaxCompDet
...
+ ' ' + SPACE(10-LEN(Cast(Cast(CGSTAmt As Decimal(10,2)) As nVarChar(10)))) + Cast(Cast(CGSTAmt As Decimal(10,2)) As nVarChar(10))
+ ' ' + SPACE(10-LEN(Cast(Cast(SGSTAmt As Decimal(10,2)) As nVarChar(10)))) + Cast(Cast(SGSTAmt As Decimal(10,2)) As nVarChar(10))
...
IF Exists(Select 'x' From tbl_Merp_ConfigAbstract WITH (NOLOCK) Where ScreenCode = 'CLOCRPRINT' and Flag = 1)
Begin
Select @CLOAccountID = AccountID From Customer WITH (NOLOCK) Where CustomerID = @CLOCustomerID

Select @CLOMonth = [Value] From tbl_Merp_ConfigDetail WITH (NOLOCK) Where ScreenCode = 'CLOCRPRINT'
...
and CD.DocumentID in(Select CLOCreditID From tbl_mERP_CLOCreditPrint WITH (NOLOCK) Where CustomerID = @CLOCustomerID)
...
From GeneralJournal WITH (NOLOCK)
Where AccountID = @CLOAccountID
and DocumentReference in(Select CLOCreditID From tbl_mERP_CLOCreditPrint WITH (NOLOCK) Where CustomerID = @CLOCustomerID)
...
Delete D From #CLOTemp D WITH (NOLOCK) Where DocumentDate > @InvoiceDate

Select @CLOMaxDate = Max(DocumentDate) From #CLOTemp WITH (NOLOCK)
Set @CLOMaxDate = isnull(@CLOMaxDate, @CLOToDate)

Select Top 1 @CLOAdjustAmt = AdjustedAmount From #CLOTemp WITH (NOLOCK) Where DocumentDate = @CLOMaxDate Order By CLOType Asc, InvID Desc, DocID Desc
...
From CollectionDetail WITH (NOLOCK), InvoiceAbstract WITH (NOLOCK)
Where CollectionID = Cast(ISnull(PaymentDetails,0) as int)
And InvoiceAbstract.InvoiceID = @InvNo



Select @SalesTaxwithcess = Sum(STPayable) from InvoiceDetail WITH (NOLOCK) Where InvoiceID = @INVNO and  Isnull(TaxCode, 0) >= 5.00
Select @salestaxwithoutCESS = Sum(STPayable) from InvoiceDetail WITH (NOLOCK) Where InvoiceID = @INVNO and  Isnull(TaxCode,0) < 5.00

Select @DispRefNumber = case when PatIndex(N'%[^0-9]%', ReferenceNumber) = 0 then ReferenceNumber else null end From InvoiceAbstract WITH (NOLOCK) Where InvoiceID = @INVNO And Status & 1 <> 0
Select @SCRefNumber = case when PatIndex(N'%[^0-9]%', ReferenceNumber) = 0 then ReferenceNumber else null end From InvoiceAbstract WITH (NOLOCK) Where InvoiceID = @INVNO And Status & 4 <> 0

DECLARE DispInfo CURSOR FOR
Select RefNumber, NewRefNumber, Case When (Status & 6 <> 0) Then 0 Else 1 End
From DispatchAbstract WITH (NOLOCK)
...
Select PODocReference From SOAbstract WITH (NOLOCK) Where SONumber in
...
From Collections WITH (NOLOCK), BranchMaster WITH (NOLOCK), BankMaster WITH (NOLOCK)
...
"Cr.Note.Desc_GST" = dbo.MERP_FN_GetCreditNoteDetails_Windows_GST(@INVNO,1),
"Cr.Note.Val_GST" = dbo.MERP_FN_GetCreditNoteDetails_Windows_GST(@INVNO,2),
"Cr.Note.AdjVal_GST" = dbo.MERP_FN_GetCreditNoteDetails_Windows_GST(@INVNO,3),
"Cr.Note.BalVal_GST" = dbo.MERP_FN_GetCreditNoteDetails_Windows_GST(@INVNO,4),
"Cr.Note.Total_GST" = dbo.MERP_FN_GetCreditNoteDetails_Windows_GST(@INVNO,5),
...
'T A X   I N V O I C E' End From Customer cu WITH (NOLOCK) Where cu.CustomerID = Customer.CustomerID),
...
Inner Join VoucherPrefix SR WITH (NOLOCK) on SR.TranID = N'SALES RETURN'
Inner Join VoucherPrefix InvA WITH (NOLOCK) on InvA.TranID = N'INVOICE AMENDMENT'
Inner Join VoucherPrefix Inv WITH (NOLOCK) on Inv.TranID = N'INVOICE'
...
Left Outer Join Salesman2 WITH (NOLOCK) on InvoiceAbstract.Salesman2 = Salesman2.SalesmanID
Left Outer Join StateCode SCBilling WITH (NOLOCK) on InvoiceAbstract.ToStateCode  = SCBilling.StateID
Left Outer Join StateCode SCShipping WITH (NOLOCK) on customer.ShippingStateID = SCShipping.StateID
WHERE InvoiceID = @INVNO

Exec SP_ARC_LastPrintUpdate @INVNO
